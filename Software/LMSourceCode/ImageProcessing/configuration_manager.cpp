...
namespace {
    ...
    void yaml_to_ptree(const YAML::Node& node, boost::property_tree::ptree& pt,
        ...
        if (node.IsScalar()) {
            ...
            for (const auto& it : node) {
                std::string child_key = key.empty() ? it.first.as<std::string>()
...
namespace golf_sim {
    ...
    bool ConfigurationManager::Initialize(const std::string& json_config_file,
            ...
            if (yaml_file.empty()) {
                ...
                for (const auto& location : yaml_locations) {
                    if (std::filesystem::exists(location)) {
                        ...
                        GS_LOG_MSG(
                            warning,
                            "Please run 'pitrac config migrate-to-json' to convert to new format");
                        break;
                    }
                }
            }
    ...
    bool ConfigurationManager::ExportEffectiveConfig(const std::string& output_file,
    ...
    void ConfigurationManager::BuildReverseMappingCache() {
        ...
        try {
            ...
            GS_LOG_MSG(debug, "Built reverse mapping cache with " +
                                  std::to_string(json_to_yaml_map_.size()) + " entries");
        } catch (const std::exception& e) {
            GS_LOG_MSG(warning, "Failed to build reverse mapping cache: " + std::string(e.what()));
        }
    }

    std::string ConfigurationManager::ConvertToJson(const std::string& yaml_key,
    ...
    std::string ConfigurationManager::ConvertFromJson(const std::string& yaml_key,
                                                      const std::string& value) const {
        try {
            auto mapping = mappings_.get_child_optional("mappings." + yaml_key);
            if (mapping) {
    ...
    bool ConfigurationManager::ValidateValue(const std::string& key,
                                             const std::string& value) const {
        try {
            ...
            if (enum_values) {
                ...
                if (!found) {
                    validation_errors_.push_back(key + ": value '" + value +
            ...
            if (min || max) {
                try {
                    ...
                    if (min && num_val < *min) {
                        validation_errors_.push_back(key + ": value " + value + " below minimum " +
                                                     std::to_string(*min));
                        return false;
                    }

                    if (max && num_val > *max) {
                        validation_errors_.push_back(key + ": value " + value + " above maximum " +
                                                     std::to_string(*max));
                        return false;
                    }
                } catch (...) {
