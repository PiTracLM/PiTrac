...
namespace golf_sim {
    ...
    GsColorTriplet CvUtils::ConvertRgbToHsv(const GsColorTriplet& rgb) {
        GsColorTriplet hsvUnscaled =
            colorsys::rgb_to_hsv(GsColorTriplet(rgb[2] / 255, rgb[1] / 255, rgb[0] / 255));
        GsColorTriplet hsv = GsColorTriplet(hsvUnscaled[0] * (float)kOpenCvHueMax,
                                            hsvUnscaled[1] * (float)kOpenCvSatMax,
                                            hsvUnscaled[2] * (float)kOpenCvValMax);
    ...
    GsColorTriplet CvUtils::ConvertHsvToRgb(const GsColorTriplet& hsv) {
        GsColorTriplet rgbUnscaled = colorsys::hsv_to_rgb(
            GsColorTriplet(hsv[0] / (float)kOpenCvHueMax, hsv[1] / (float)kOpenCvSatMax,
                           hsv[2] / (float)kOpenCvValMax));
        GsColorTriplet rgb = GsColorTriplet(int(rgbUnscaled[2] * 255), int(rgbUnscaled[1] * 255),
                                            int(rgbUnscaled[0] * 255));

        return (rgb);
    }

    float CvUtils::ColorDistance(const GsColorTriplet& rgb1, const GsColorTriplet& rgb2) {
        float rgbDiff = sqrt((float)(pow((double)(rgb1[0] - rgb2[0]), 2.) +
                                     pow((double)(rgb1[1] - rgb2[1]), 2.) +
                                     pow((double)(rgb1[2] - rgb2[2]), 2.)));
        return (rgbDiff);
    }
    ...
    std::vector<GsColorTriplet> CvUtils::GetBallColorRgb(const cv::Mat& img,
                                                         const GsCircle& circle) {
        BOOST_LOG_FUNCTION();
        ...
        const double BOUNDED_BOX_RADIUS_RATIO =
            ((2.0 * r) * 0.707) /
            2.0;  // 1.6 (which is almost the whole ball may be resulting in too much averaging)
        int xmin = std::max(0, (int)round(x - BOUNDED_BOX_RADIUS_RATIO));
        int xmax = std::min(
            CvWidth(img), (int)round(x + BOUNDED_BOX_RADIUS_RATIO));  // Somehow, the box otherwise
                                                                      // seems too far to the left?
        int ymin =
            std::max(0, (int)round(y - BOUNDED_BOX_RADIUS_RATIO));  // Round up to focus more on the
                                                                    // better-lit top of the ball
        int ymax = std::min(CvHeight(img), (int)round(y + BOUNDED_BOX_RADIUS_RATIO));

        // GS_LOG_TRACE_MSG(trace, "GetBall_color: xmin,max, ymin,max = (" + std::to_string(xmin) +
        // "," + std::to_string(xmax) + ") : (" + std::to_string(ymin) + "," + std::to_string(ymax)
        // + ")");

        // LoggingTools::ShowRectangleOnImage("getBallColor area to average: ", img, cv::Point(xmin,
        // ymin), cv::Point(xmax, ymax));
        ...
        // GS_LOG_TRACE_MSG(trace, "Average is (RGB):" +
        ...
        GsColorTriplet avg_color_vec3f{(float)avg_color[0], (float)avg_color[1],
                                       (float)avg_color[2]};
        GsColorTriplet median_color_vec3f{(float)median_color[0], (float)median_color[1],
                                          (float)median_color[2]};
        GsColorTriplet std_color_vec3f{(float)std_color[0], (float)std_color[1],
                                       (float)std_color[2]};
    ...
    cv::Mat CvUtils::GetAreaMaskImage(int resolution_x_, int resolution_y_, int expected_ball_X,
                                      int expected_ball_Y, int mask_radius,
                                      cv::Rect& mask_dimensions, bool use_square) {
        BOOST_LOG_FUNCTION();
        ...
        if (!use_square) {
            ...
            cv::rectangle(
                maskImage, cv::Point(expected_ball_X - mask_radius, expected_ball_Y - mask_radius),
                cv::Point(expected_ball_X + mask_radius, expected_ball_Y + mask_radius), c1, -1);
        }

        // Return the rectangle around the ROI
        mask_dimensions = cv::Rect(expected_ball_X - mask_radius, expected_ball_Y - mask_radius,
        ...
        cv::GaussianBlur(area_mask_image_, area_mask_image_, cv::Size(5, 5),
                         0);  // nominal was 11x11
        cv::erode(area_mask_image_, area_mask_image_,
                  cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3)), cv::Point(-1, -1), 2);
        cv::dilate(area_mask_image_, area_mask_image_,
                   cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3)), cv::Point(-1, -1), 2);
    ...
    cv::Mat CvUtils::GetSubImage(const cv::Mat& full_image, cv::Rect& ball_ROI_rectInfull_image,
                                 cv::Point& offset_sub_to_full, cv::Point& offset_full_to_sub) {
        cv::Mat subImg;
        ...
        if (ball_ROI_rectInfull_image.tl().x < 0) {
            ...
            LoggingTools::Warning(
                "CvUtils::GetSubImage received invalid sub-image parameters.  tl.x = " +
                std::to_string(ball_ROI_rectInfull_image.tl().x) + ".  Corrected");
        }
        if (ball_ROI_rectInfull_image.tl().y < 0) {
            finalRoi.y = 0;
            LoggingTools::Warning(
                "CvUtils::GetSubImage received invalid sub-image parameters.  tl.y = " +
                std::to_string(ball_ROI_rectInfull_image.tl().y) + ".  Corrected");
        }
        if (ball_ROI_rectInfull_image.br().x >= full_image.cols) {
            finalRoi.width -= ball_ROI_rectInfull_image.br().x - full_image.cols;
            LoggingTools::Warning(
                "CvUtils::GetSubImage received invalid sub-image parameters.  br.x = " +
                std::to_string(ball_ROI_rectInfull_image.br().x) + ".  Corrected");
        }
    ...
    double CvUtils::GetDistance(const cv::Vec3d& location) {
        return std::sqrt(std::pow(location[0], 2.) + std::pow(location[1], 2.) +
                         std::pow(location[2], 2.));
    }
     ...
     */
    void CvUtils::BrightnessAndContrastAutoAlgo1(const cv::Mat& src, cv::Mat& dst,
