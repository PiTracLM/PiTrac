...
namespace golf_sim {
    ...
    bool GolfSimCamera::kUsePreImageSubtraction =
        false;  // Ultimately, this concept was not as helpful as hoped for
    ...
    int GolfSimCamera::kExternallyStrobedEnvPreHoughBlurSize =
        13;  // 6 for External Strobe, 9 for normal
    ...
    CameraHardware::CameraModel GolfSimCamera::kSystemSlot1CameraType =
        CameraHardware::CameraModel::PiGS;
    CameraHardware::CameraModel GolfSimCamera::kSystemSlot2CameraType =
        CameraHardware::CameraModel::PiGS;

    CameraHardware::LensType GolfSimCamera::kSystemSlot1LensType =
        CameraHardware::LensType::Lens_6mm;
    CameraHardware::LensType GolfSimCamera::kSystemSlot2LensType =
        CameraHardware::LensType::Lens_6mm;

    CameraHardware::CameraOrientation GolfSimCamera::kSystemSlot1CameraOrientation =
        CameraHardware::CameraOrientation::kUpsideUp;
    CameraHardware::CameraOrientation GolfSimCamera::kSystemSlot2CameraOrientation =
        CameraHardware::CameraOrientation::kUpsideUp;
    ...
    GolfSimCamera::GolfSimCamera() {
        ...
        // The following constants are only used internal to the GolfSimCamera class, and so can be
        ...
        ***/
        ...
        GolfSimConfiguration::SetConstant(
            "gs_config.ball_position.kTeedBallSearchAreaMaskRadiusRatio",
        ...
        GolfSimConfiguration::SetConstant("gs_config.ball_position.kMinRadiusRatio",
                                          kMinRadiusRatio);
        GolfSimConfiguration::SetConstant("gs_config.ball_position.kMaxRadiusRatio",
                                          kMaxRadiusRatio);
        GolfSimConfiguration::SetConstant("gs_config.ball_position.kMinRadiusOffset",
                                          kMinRadiusOffset);
        GolfSimConfiguration::SetConstant("gs_config.ball_position.kMaxRadiusOffset",
                                          kMaxRadiusOffset);
        ...
        GolfSimConfiguration::SetConstant(
            "gs_config.ball_exposure_selection.kUsePreImageSubtraction", kUsePreImageSubtraction);

        GolfSimConfiguration::SetConstant("gs_config.testing.kExternallyStrobedEnvFilterImage",
                                          kExternallyStrobedEnvFilterImage);
        GolfSimConfiguration::SetConstant(
            "gs_config.testing.kExternallyStrobedEnvBottomIgnoreHeight",
        ...
        GolfSimConfiguration::SetConstant(
            "gs_config.testing.kExternallyStrobedEnvMaximumHoughLineGap",
        ...
        GolfSimConfiguration::SetConstant("gs_config.cameras.kHLAOffsetAngleDegrees",
                                          kHLAOffsetAngleDegrees);
        GolfSimConfiguration::SetConstant("gs_config.cameras.kVLAOffsetAngleDegrees",
                                          kVLAOffsetAngleDegrees);
    }
    ...
    bool GolfSimCamera::DetermineHandedness(const std::vector<GolfBall>& input_balls,
        ...
        GS_LOG_TRACE_MSG(trace, "DetermineHandedness called with " +
                                    std::to_string(input_balls.size()) + " balls.");
        ...
        if ((int)input_balls.size() < balls_to_examine) {
            GS_LOG_TRACE_MSG(warning,
                             "DetermineHandedness - balls vector was empty.Exiting function.");
            return false;
        }
        ...
        std::sort(balls.begin(), balls.end(),
                  [](const GolfBall& a, const GolfBall& b) { return (a.x() < b.x()); });
        ...
        handedness = (positive_slope_count >= 0) ? GolferOrientation::kRightHanded
                                                 : GolferOrientation::kLeftHanded;
        std::string handedness_string =
            (handedness == GolferOrientation::kRightHanded) ? "right_handed" : "left_handed";
        GS_LOG_TRACE_MSG(
            trace, "DetermineHandedness result: " + handedness_string +
                       ", with positive_slope_count = " + std::to_string(positive_slope_count));

        return true;
    }

    int GolfSimCamera::GetExpectedBallRadiusPixelsUsingKnownFocalLength(
        const CameraHardware& camera_hardware, const int resolution_x_, const double distance) {
        GS_LOG_TRACE_MSG(
            trace, "GetExpectedBallRadiusPixelsUsingKnownFocalLength called with resolution: " +
                       std::to_string(resolution_x_) + ", distance: " + std::to_string(distance) +
                       ", and with expected camera_hardware.focal_length_ of: " +
                       std::to_string(camera_hardware.focal_length_));

        if (!camera_hardware.cameraInitialized || resolution_x_ <= 0) {
            GS_LOG_MSG(warning,
                       "Camera hardware (or resolution x value) not initialized in "
                       "GetExpectedBallRadiusPixelsUsingKnownFocalLength!");
            // TBD - For now, just ignore.    return false;
        }

        if (std::abs(distance) < 0.0001) {
            GS_LOG_MSG(error,
                       "GetExpectedBallRadiusPixelsUsingKnownFocalLength called with 0 distance.");
            return 0;
        }

        if (std::abs(camera_hardware.focal_length_) < 0.0001) {
            GS_LOG_MSG(error,
                       "GetExpectedBallRadiusPixelsUsingKnownFocalLength called with 0 "
                       "camera_hardware.focal_length_.");
            return 0;
        }

        if (std::abs(resolution_x_) < 0.0001) {
            GS_LOG_MSG(error,
                       "GetExpectedBallRadiusPixelsUsingKnownFocalLength when resolution_x_ is 0.");
            return 0;
        }
        ...
        GS_LOG_TRACE_MSG(trace, "GetExpectedBallRadiusPixelsUsingKnownFocalLength returning: " +
                                    std::to_string(radius_as_integer));

        return radius_as_integer;
    }

    int GolfSimCamera::GetExpectedBallRadiusPixels(const CameraHardware& camera_hardware,
                                                   const int resolution_x_, const double distance) {
        if (!camera_hardware.cameraInitialized || resolution_x_ <= 0) {
            GS_LOG_MSG(warning,
                       "Camera hardware (or resolution x value) not initialized in "
                       "GetExpectedBallRadiusPixels!");
            // TBD - For now, just ignore.    return false;
        }

        // Get a reasonable default in case (for some reason), the camera's expected radius has not
        // been set
        double radius = kExpectedBallRadiusPixelsAt40cm;

        // Get the camera's expected ball radius.  There might be two different cameras, or two
        // different lenses (e.g., one wide, one telephoto), with different respective radii.
        if (camera_hardware.expected_ball_radius_pixels_at_40cm_ > 0) {
            radius = camera_hardware.expected_ball_radius_pixels_at_40cm_;
        } else {
            GS_LOG_TRACE_MSG(
                warning,
                "expected_ball_radius_pixels_at_40cm_ not set in camera in "
                "GolfSimCamera::GetExpectedBallRadiusPixels().  Setting to default instead.");
        }

        GS_LOG_TRACE_MSG(
            trace, "GetExpectedBallRadiusPixels called with resolution: " +
                       std::to_string(resolution_x_) + ", distance: " + std::to_string(distance) +
        ...
        GS_LOG_TRACE_MSG(
            trace, "GetExpectedBallRadiusPixels returning: " + std::to_string(std::round(radius)));

        return (int)std::round(radius);
    }

    std::vector<GsColorTriplet> GolfSimCamera::GetBallHSVRange(
        const GsColorTriplet& ball_color_RGB) {
        ...
        // Note that the consumer of this function will need to loop around the 360 degree circle if
        // the widened hue numbers go belore zero
        ...
        float vmin =
            (float)std::max(0, (int)hsvAvg[2] - (int)(V_MIN_CAL_COLOR_WIDENING_AMOUNT * 0.9));
        ...
        GS_LOG_TRACE_MSG(trace, "GetBallHSVRange for average (" +
                                    LoggingTools::FormatGsColorTriplet(hsvAvg) +
                                    ") = " + LoggingTools::FormatGsColorTriplet(hsvMin) + " | " +
                                    LoggingTools::FormatGsColorTriplet(hsvMax));
    ...
    bool GolfSimCamera::GetCalibratedBall(const GolfSimCamera& camera, const cv::Mat& rgbImg,
                                          GolfBall& b, const cv::Vec2i& expectedBallCenter,
        ...
        if (GolfSimOptions::GetCommandLineOptions().GetCameraNumber() ==
            ...
            GS_LOG_TRACE_MSG(
                trace, "GetCalibratedBall called for camera2 (usually used for camera1 images).");
        }
        ...
        GS_LOG_TRACE_MSG(trace, "GetCalibratedBall using expected ball distance of: " +
                                    std::to_string(expected_distance));

        double expectedRadius =
            GetExpectedBallRadiusPixels(camera.camera_hardware_, rgbImg.cols, expected_distance);
        ...
        if (expected_ball_X == 0 && expected_ball_Y == 0) {
            ...
            double search_area_radius =
                max * 1.1;  // add a little buffer to make sure we can find things at the edge
        ...
        if (kTeedBallSearchAreaMaskRadiusRatio > 0.01) {
            ...
            GS_LOG_TRACE_MSG(trace,
                             "GetCalibratedBall using search mask_radius (computed using "
                             "kTeedBallSearchAreaMaskRadiusRatio=" +
                                 std::to_string(kTeedBallSearchAreaMaskRadiusRatio) +
                                 " of: " + std::to_string(mask_radius));
            ip->area_mask_image_ = CvUtils::GetAreaMaskImage(
                rgbImg.cols, rgbImg.rows, expected_ball_X, expected_ball_Y, mask_radius, roi);
            ...
            GS_LOG_TRACE_MSG(trace,
                             "GetCalibratedBall not using a search mask_radius (because "
                             "kTeedBallSearchAreaMaskRadiusRatio was = " +
                                 std::to_string(kTeedBallSearchAreaMaskRadiusRatio));
        }

        // The whole point here is that we don't know the color until we calibrate, so force a
        // very broad color range mask
        // NOTE - the entire color thing is pretty much deprecated now - signal that we don't know
        // the color
        ...
        bool result = ip->GetBall(rgbImg, b, return_balls, roi,
                                  BallImageProc::BallSearchMode::kFindPlacedBall,
                                  useLargestFoundBall, expectBall);
        ...
        GS_LOG_MSG(trace,
                   "GetBall() returned " + std::to_string(return_balls.size()) + " ball(s).");
        ShowAndLogBalls("GetBallReturnedBalls", rgbImg, return_balls,
                        kLogIntermediateExposureImagesToFile);
        ...
        ShowAndLogBalls("GetBallReturnedBalls Final Ball:", rgbImg, final_ball,
                        kLogIntermediateExposureImagesToFile);

        // We were able to discern a circle that the system thinks is a ball - return the ball with
        // the information corresponding to it inside

        // Setup a ball to return with all the pertinent information
        b.measured_radius_pixels_ = b.ball_circle_[2];

        // We might also try to force the user to put the ball at a specific distance from the
        // camera and calibrate from that Note that the distance being calculated here is not the
        // Z-axis only distance, but instead it is the distance directly to the ball, as the crow
        // flies.
        ...
        if (GolfSimOptions::GetCommandLineOptions().system_mode_ == SystemMode::kCamera1Calibrate ||
            ...
            // Note - since we are measuring the ball using the standard focal length of the camera
            // hardware, when we compute the length again here, it should be the same.  We'd use
            // this if we have the ball at a known, precise distance and then get the focal length
            // that makes the system find that distance.
            b.calibrated_focal_length_ = computeFocalDistanceFromBallData(
                camera, b.measured_radius_pixels_, distance_direct_to_ball);
            GS_LOG_MSG(info, "Calibrated focal length for distance " +
                                 std::to_string(distance_direct_to_ball) +
                                 " and Radius: " + std::to_string(b.measured_radius_pixels_) +
        ...
        // The golf ball may not be centered in the frame of the camera.  Determine the angle at
        // which the ball sits so that it can be taken into account for, e.g., ball rotation
        // perspectives
    ...
    bool GolfSimCamera::ComputeBallDeltas(GolfBall& ball1, GolfBall& ball2,
        ...
        if (!ComputeXyzDeltaDistances(ball1, ball2, ball2.position_deltas_ball_perspective_,
        ...
        // account for that here For example, if the second camera is to the right of the first
        // (looking at the ball), then that right-direction distance on the X axis should be added
        // to the distance delta in the X-axis of the ball.
    ...
    bool GolfSimCamera::ComputeSingleBallXYZOrthoCamPerspective(const GolfSimCamera& camera,
        ...
        // The golf ball may not be centered in the frame of the camera.  Determine the distances
        // and angle at which the ball sits so that it can be taken into account for, e.g., ball
        // rotation perspectives
        if (!ComputeXyzDistanceFromOrthoCamPerspective(camera, ball1,
        ...
        /* DEBUG
        GS_LOG_TRACE_MSG(trace, "Calculated X,Y angles (camera perspective) (in degrees) are: " +
        std::to_string(angles_camera_perspective[0]) + ", " +
            std::to_string(angles_camera_perspective[1]));

        GS_LOG_TRACE_MSG(trace, "Calculated X,Y angles (ball perspective) (in degrees) are: " +
        std::to_string(angles_ball_perspective[0]) + ", " +
    ...
    bool GolfSimCamera::GetCurrentBallLocation(const GolfSimCamera& camera, const cv::Mat& rgbImg,
                                               const GolfBall& calibrated_ball,
                                               GolfBall& foundBall) {
        GS_LOG_TRACE_MSG(
            trace, "GetCurrentBallLocation(ball).  calibrated_ball = " + calibrated_ball.Format());
        ...
        GS_LOG_TRACE_MSG(trace, "Original radius at calibration-time distance of " +
                                    std::to_string(distance_at_calibration_) +
                                    " was: " + std::to_string(radius_at_calibration_pixels_) +
        ...
        std::vector<GolfBall> return_balls;
        bool result =
            ip->GetBall(rgbImg, foundBall, return_balls, emptyROI,
                        BallImageProc::BallSearchMode::kFindPlacedBall, useLargestFoundBall);
        ...
        if (!ComputeSingleBallXYZOrthoCamPerspective(camera, foundBall)) {
            GS_LOG_MSG(error,
                       "GolfSimCamera::GetCurrentBallLocation failed to "
                       "ComputeSingleBallXYZOrthoCamPerspective.");
            return false;
        }
    ...
    cv::Vec2d GolfSimCamera::computeCameraAnglesToBallPlane(
        ...
        if (camera_height_above_ball < 0.0 || std::abs(zDistanceToBall) <= 0.001) {
            LoggingTools::Warning(
                "GolfSimCamera::computeCameraAnglesToBallPlane called, but "
                "camera_height_above_ball_ or zDistanceToBall <= 0 (and likely not set)");
        }
    ...
    bool GolfSimCamera::analyzeShotImages(
        const GolfSimCamera& camera, const cv::Mat& rgbImg1, const cv::Mat& rgbImg2,
        long timeDelayuS, const std::vector<cv::Vec3d>& camera_positions_from_origin,
        GolfBall& result_ball, const cv::Vec2i& expectedBallCenter) {
        if (timeDelayuS == 0) {
            GS_LOG_MSG(error, "In analyzeShotImages, timeDelayuS was " +
                                  std::to_string(timeDelayuS) + ".");
        }
        ...
        // The Vec2i is a hack to deal with the ball in the test images not being in the center
        ...
        if (GolfSimOptions::GetCommandLineOptions().system_mode_ == SystemMode::kCamera1Calibrate ||
            GolfSimOptions::GetCommandLineOptions().system_mode_ == SystemMode::kCamera2Calibrate) {
            ball1.calibrated_focal_length_ = computeFocalDistanceFromBallData(
                camera, ball1.measured_radius_pixels_, ball1.distance_to_z_plane_from_lens_);

            return true;
        }

        // Using the first ball for reference, determine where the second one is in relation to the
        // first
        ...
        cv::Vec3d rotationResults =
            BallImageProc::GetBallRotation(grayImg1, ball1, grayImg2, result_ball);
    ...
    void GolfSimCamera::CalculateBallSpinRates(GolfBall& b, const cv::Vec3d& rotation_results,
                                               long time_delay_us) {
        b.ball_rotation_angles_camera_ortho_perspective_ = rotation_results;
        b.rotation_speeds_RPM_[0] =
            60. * ((rotation_results[0] / 360.) / (double)(time_delay_us)) * 1000000.;
        b.rotation_speeds_RPM_[1] =
            60. * ((rotation_results[1] / 360.) / (double)(time_delay_us)) * 1000000.;
        b.rotation_speeds_RPM_[2] =
            60. * ((rotation_results[2] / 360.) / (double)(time_delay_us)) * 1000000.;
    }

    double GolfSimCamera::GetTotalDistance(cv::Vec3d& distance_deltas) {
        return sqrt(pow(distance_deltas[0], 2) + pow(distance_deltas[1], 2) +
                    pow(distance_deltas[2], 2));
    }
    ...
    double GolfSimCamera::computeFocalDistanceFromBallData(const GolfSimCamera& camera,
        ...
        // Could reasonably do this with either height or width parameters for the sensors that we
        // are using, but for at least the GS camera, the "correct" divisor is the X axis, not Y.
    ...
    double GolfSimCamera::convertXDistanceToMeters(const GolfSimCamera& camera,
                                                   double zDistanceMeters, double xDistancePixels) {
        ...
        double xDistanceMeters =
            halfWidthMeters * (xDistancePixels / (camera.camera_hardware_.resolution_x_ / 2.0));
        return (xDistanceMeters);
    }

    double GolfSimCamera::convertYDistanceToMeters(const GolfSimCamera& camera,
                                                   double zDistanceMeters, double yDistancePixels) {
        // Note that we are NOT using the calibrated_focal_length
        ...
        double yDistanceMeters =
            halfHeightMeters * (yDistancePixels / (camera.camera_hardware_.resolution_y_ / 2.0));
    ...
    double GolfSimCamera::ComputeDistanceToBallUsingRadius(const GolfSimCamera& camera,
        ...
        if (ball.ball_ellipse_.size.width > 0 && ball.ball_ellipse_.size.height > 0) {
            ...
            // If we have ellipse representation, then use it - it's probably more reliable than a
            // strict circle, but we need to figure out WHICH radius of the ellipse to use

            // TBD -- Seems to work correctly only if we use the (smaller) width.
            // We could also use the largest radius in the hope that it will be more accurate,
            // percentage-wise This should presumably be the height
            chosenRadiusPixels =
                std::min(ball.ball_ellipse_.size.width, ball.ball_ellipse_.size.height) / 2.0;

            // The ellipse may be at an angle.
            // Use the effective number of pixels on the sensor at that angle as a measuring stick
            // to determine the real-world distance
            double ellipseAngleDegrees = ball.ball_ellipse_.angle;

            // From vertical
            double ellipseAngleAtSensorCorner = CvUtils::RadiansToDegrees(atan(
                camera.camera_hardware_.sensor_height_ / camera.camera_hardware_.sensor_width_));
            ...
            // for details of rotatedRect angles.  They are non-intuitive.  Ellipse angle is to the
            // right from vertical
            if ((ellipseAngleDegrees > ellipseAngleAtSensorCorner &&
                ...
                effectiveSensorWidth =
                    std::abs(camera.camera_hardware_.sensor_width_ /
                             cos(CvUtils::DegreesToRadians(90 - ellipseAngleDegrees)));
                effectiveCameraResolution =
                    std::abs(camera.camera_hardware_.resolution_x_ /
                             cos(CvUtils::DegreesToRadians(90 - ellipseAngleDegrees)));
            } else {
                effectiveSensorWidth = std::abs(
                    (double)camera.camera_hardware_.sensor_height_ /
                    cos(CvUtils::DegreesToRadians(ellipseAngleDegrees)));  // 90-ellipse angle
                effectiveCameraResolution =
                    std::abs((double)camera.camera_hardware_.resolution_y_ /
                             cos(CvUtils::DegreesToRadians(ellipseAngleDegrees)));
            }
        ...
        // double ballRadiusMeters = (effectiveCameraResolution / (2.0 * chosenRadiusPixels)) * (2.0
        // * kBallRadiusMeters) * (camera.focal_length_ / effectiveSensorWidth);

        double distanceToBallMeters =
            (effectiveCameraResolution / (2.0 * chosenRadiusPixels)) *
            (2.0 * GolfBall::kBallRadiusMeters) *
            (camera.camera_hardware_.focal_length_ / effectiveSensorWidth);

        return distanceToBallMeters;

        // TBD - AlternativelyCreate an upright (non-rotated) rectangle that fits around the
        // ellipse, which is itself a rotated rect. Then, use the X and Y widths of the resulting
        // upright rectangle to figure out how wide/high the ellipse is. Finally, use the
        // pythagorean theory to figure out the diagonal's real-world length.

        std::vector<cv::Point> contours;  // ?? = std::vector<cv:Point>(ball.ball_ellipse_.re);
    }

    bool GolfSimCamera::ComputeXyzDeltaDistances(const GolfBall& b1, const GolfBall& b2,
        ...
        distance_deltas_camera_perspective[0] =
            b2.distances_ortho_camera_perspective_[0] - b1.distances_ortho_camera_perspective_[0];
        distance_deltas_camera_perspective[1] =
            b2.distances_ortho_camera_perspective_[1] - b1.distances_ortho_camera_perspective_[1];
        distance_deltas_camera_perspective[2] =
            b2.distances_ortho_camera_perspective_[2] - b1.distances_ortho_camera_perspective_[2];

        // Ball X is -Camera Z, Ball Y is Camera Y, Ball Z is Camera X  (TBD - earlier, this was
        ...
        position_deltas_ball_perspective[1] =
            b2.distances_ortho_camera_perspective_[1] - b1.distances_ortho_camera_perspective_[1];
    ...
    bool GolfSimCamera::AdjustXYZDistancesForCameraAngles(const cv::Vec2d& camera_angles,
        ...
        if (original_distances[kZ_index] == 0) {
            GS_LOG_MSG(error,
                       "GolfSimCamera::AdjustXYZDistancesForCameraAngles received original Z "
                       "distance of 0");
            return false;
        }

        GS_LOG_TRACE_MSG(
            trace, "X,Y camera angles (degrees) = " + std::to_string(camera_angles[kX_index]) +
                       ", " + std::to_string(camera_angles[kY_index]));
        GS_LOG_TRACE_MSG(
            trace, "X,Y distances (meters) = " + std::to_string(original_distances[kX_index]) +
        ...
        if (mode == SystemMode::kCamera1 || mode == SystemMode::kCamera1Calibrate ||
        ...
        cos(CvUtils::DegreesToRadians(camera_angles[kY_index])); adjusted_distances[kX_index] =
        hypotenuse * cos(a_prime_angle_radians) *
        ...
        a_prime_angle_radians = (kPi / 2.) + (CvUtils::DegreesToRadians(camera_angles[kY_index]) +
        b_angle_radians);
        ...
        */
        ...
        adjusted_distances[kY_index] =
            (original_distances[kY_index] * cos(camera_angle_x_radians)) -
            (original_distances[kZ_index] * sin(camera_angle_x_radians));

        adjusted_distances[kZ_index] =
            (original_distances[kY_index] * sin(camera_angle_x_radians)) +
            (original_distances[kZ_index] * cos(camera_angle_x_radians));

        // Perform Y-Axis rotation - NOTE - this re-uses and ajusts the Z distance already
        // calculated just above
        adjusted_distances[kX_index] =
            (original_distances[kX_index] * cos(camera_angle_y_radians)) +
            (adjusted_distances[kZ_index] * sin(camera_angle_y_radians));

        adjusted_distances[kZ_index] =
            (adjusted_distances[kZ_index] * cos(camera_angle_y_radians)) -
            (original_distances[kX_index] * sin(camera_angle_y_radians));

        // TBD - Not certain this is useful - we can make up for the offset of the tilted camera
        // from the center of the system in other ways
        adjusted_distances[kX_index] += camera_X_offset_for_tilt;
        adjusted_distances[kY_index] += camera_Y_offset_for_tilt;
    ...
    // The ball's direct distance to the lens, as well as the x,y coordinates from the picture
    // center origin both have to be set BEFORE calling this method. Axes are as perceived by the
    // camera -- z is close/far, x is left/right, and y is up/down NOTE that the Z distance is the
    // line-of-sight distance from the lens to the ball.
    bool GolfSimCamera::ComputeXyzDistanceFromOrthoCamPerspective(const GolfSimCamera& camera,
        ...
        if (std::abs(b1.distance_to_z_plane_from_lens_) <= 0.0001) {
            LoggingTools::Warning(
                "ComputeXyzDistanceFromOrthoCamPerspective called without setting the ball "
        ...
        double xDistanceFromCamCenter =
            convertXDistanceToMeters(camera, b1.distance_to_z_plane_from_lens_, xFromCameraCenter);
        camera_perspective_distances[0] =
            xDistanceFromCamCenter;  // X distance, negative means to the left of the camera

        double yDistanceFromCamCenter =
            convertYDistanceToMeters(camera, b1.distance_to_z_plane_from_lens_, yFromCameraCenter);

        camera_perspective_distances[1] =
            -yDistanceFromCamCenter;  // Y distance, positive is upward (smaller Y values)  // TBD -
                                      // sqrt(pow(b1.distance_to_z_plane_from_lens_, 2) -
                                      // pow(yDistanceFromCamCenter, 2));  // Y distance.  Positive
                                      // values are above the camera center

        camera_perspective_distances[2] =
            b1.distance_to_z_plane_from_lens_;  // FOR NON_DIRECT
                                                // sqrt(pow(b1.distance_to_z_plane_from_lens_, 2) -
                                                // pow(xDistanceFromCamCenter, 2));  // Z distance.
                                                // Only positive values in front of camera

        GS_LOG_TRACE_MSG(trace,
                         "GolfSimCamera::ComputeXyzDistanceFromOrthoCamPerspective computed "
                         "camera_perspective_distances of: " +
        ...
        // order to compute the x,y,z position of the ball.  The ball would be at the same
        // z-distance, and the x and y distances would just have to be added/subtracted to the point
        // in space directly out from the camera.
        //
        // However, because the camera is at an angle, we need to convert the x,y,z distances from
        // the camera's perspective into x,y,z from a perspective that is orthogonal to the launch
        // monitor. This will entail a polar (or spherical) coordinate system to a cartesian (x,y,z)
        // conversion So, Convert the distances from the camera into a set of angles from the
        // camera's perspective

        // Using the adjusted X and Y distances (lined up with the real world up/down and
        // left/right), determine the angles of the ball from the camera perspective Positive X
        // degrees is to the left looking out the camera Negative Y degrees is tilting down looking
        // out the camera
        cv::Vec2d deltaAnglesCameraPerspective;
        deltaAnglesCameraPerspective[0] = -CvUtils::RadiansToDegrees(
            atan(camera_perspective_distances[0] / b1.distance_to_z_plane_from_lens_));
        deltaAnglesCameraPerspective[1] = CvUtils::RadiansToDegrees(
            atan(camera_perspective_distances[1] / b1.distance_to_z_plane_from_lens_));
        ...
        // X angle here is positive in a counter-clockwise movement looking down at the LM from
        // above Y angle is negative as the azimuth angle goes down from horizontal
        cv::Vec2d deltaAnglesLMPerspective;
        deltaAnglesLMPerspective[0] =
            camera.camera_hardware_.camera_angles_[0] + deltaAnglesCameraPerspective[0];
        deltaAnglesLMPerspective[1] =
            camera.camera_hardware_.camera_angles_[1] + deltaAnglesCameraPerspective[1];

        GS_LOG_TRACE_MSG(trace,
                         "GolfSimCamera::ComputeXyzDistanceFromOrthoCamPerspective computed "
                         "(LM-perspective) X,Y angles of: " +
    ...
    bool GolfSimCamera::getXYDeltaAnglesBallPerspective(
        const cv::Vec3d& position_deltas_ball_perspective, cv::Vec2d& deltaAnglesBallPerspective) {
        ...
        // Determine from perspective of the camera x-axis, looking in the positive direction (the
        // ball's most natural perspective).

        // Negative degrees means clockwise looking down the z axis

        // X angle (up/down) = arctan(z/x) from ball perspective
        deltaAnglesBallPerspective[0] = CvUtils::RadiansToDegrees(
            atan(position_deltas_ball_perspective[0] / position_deltas_ball_perspective[2]));
        // Y angle (left/right) = arctan(y/xf)
        deltaAnglesBallPerspective[1] = CvUtils::RadiansToDegrees(
            atan((position_deltas_ball_perspective[1] / position_deltas_ball_perspective[2])));

        return true;
    }

    bool GolfSimCamera::ComputeBallXYAnglesFromCameraPerspective(
        const cv::Vec3d& distances_camera_perspective, cv::Vec2d& deltaAnglesCameraPerspective) {
        if (distances_camera_perspective[2] < 0.0001) {
            GS_LOG_MSG(error,
                       "ComputeBallXYAnglesFromCameraPerspective:  "
                       "b2.distance_to_z_plane_from_lens_ was 0!");
            return false;
        }

        /* First version is for when the distances_camera_perspective[2] was the direct-to-ball
        distance instead of just the z plane
        ...
        distances_camera_perspective[2])); double directDistance =
        sqrt(pow(distances_camera_perspective[2], 2) + pow(distances_camera_perspective[0], 2));
        deltaAnglesCameraPerspective[1] =
        -CvUtils::RadiansToDegrees(atan(-distances_camera_perspective[1] / directDistance));
        */

        // Positive X angle is counter-clockwise looking down on the camera/ball from above
        // Positive Y angle is looking up from level to the ball
        deltaAnglesCameraPerspective[0] = CvUtils::RadiansToDegrees(
            atan(distances_camera_perspective[0] / distances_camera_perspective[2]));
        deltaAnglesCameraPerspective[1] = -CvUtils::RadiansToDegrees(
            atan(-distances_camera_perspective[1] / distances_camera_perspective[2]));
    ...
    bool GolfSimCamera::ShowAndLogBalls(const std::string& title, const cv::Mat& img,
                                        std::vector<GolfBall>& balls, bool log_image_to_file,
                                        const int middle_ball_index, const int second_ball_index) {
        // It's expensive to clone an image, so make sure we're here to do at least something
        if (!log_image_to_file && !GolfSimOptions::GetCommandLineOptions().show_images_ &&
            GolfSimOptions::GetCommandLineOptions().artifact_save_level_ !=
                ArtifactSaveLevel::kAll) {
            return true;
        }
        ...
        if (log_image_to_file && GolfSimOptions::GetCommandLineOptions().artifact_save_level_ ==
                                     ArtifactSaveLevel::kAll) {
    ...
    int GolfSimCamera::GetMostCenteredBallIndex(const std::vector<GolfBall>& balls,
        ...
        int smallest_distance_from_center =
            10 * camera_hardware_.resolution_x_;  // Ensure first ball will be closer to the center
        ...
        for (size_t i = 0; i < balls.size(); i++) {
            ...
            int distance_from_center =
                (int)std::round(std::sqrt(x_distance * x_distance + y_distance * y_distance));
            ...
            if (distance_from_center < smallest_distance_from_center) {
                ...
                GS_LOG_TRACE_MSG(trace,
                                 "GetMostCenteredBallIndex - Best current candidate with distance "
                                 "from center of " +
    ...
    bool GolfSimCamera::GetBallDistancesAndRatios(const std::vector<GolfBall>& balls,
        ...
        if (balls.size() < 1) {
            GS_LOG_TRACE_MSG(
                warning, "GetBallDistancesAndRatios - balls vector was empty.  Exiting function.");
            return false;
        }
    ...
    bool GolfSimCamera::FindBestBallOnLineOfFlight(const std::vector<GolfBall>& balls,
                                                   ...
                                                   double& ball_1_distance, double& ball_2_distance,
                                                   const GolfBall& line_ball1,
                                                   const GolfBall& line_ball2) {
        /* This was the original code that used the balls around the ball being analyzed to
        determine the line if (balls.size() <= 3) {
        ...
        ball_1_distance = GetPerpendicularDistanceFromLine(ball1.x(), ball1.y(), left_line_ball.x(),
        left_line_ball.y(), right_line_ball.x(), right_line_ball.y()); ball_2_distance =
        GetPerpendicularDistanceFromLine(ball2.x(), ball2.y(), left_line_ball.x(),
        left_line_ball.y(), right_line_ball.x(), right_line_ball.y());
        */
        ...
        ball_1_distance = GetPerpendicularDistanceFromLine(
            ball1.x(), ball1.y(), line_ball1.x(), line_ball1.y(), line_ball2.x(), line_ball2.y());
        ball_2_distance = GetPerpendicularDistanceFromLine(
            ball2.x(), ball2.y(), line_ball1.x(), line_ball1.y(), line_ball2.x(), line_ball2.y());
    ...
    double GolfSimCamera::GetPerpendicularDistanceFromLine(double xc, double yc, double xa,
    ...
    void GolfSimCamera::RemoveLowScoringBalls(std::vector<GolfBall>& balls,
        ...
        if (adjusted_max_balls_to_retain >= (int)balls.size()) {
            GS_LOG_TRACE_MSG(trace,
                             "RemoveLowScoringBalls asked to remove more balls than were "
                             "identified.  max_balls_to_retain= " +
    ...
    void GolfSimCamera::RemoveUnlikelyRadiusChangeBalls(
        std::vector<GolfBall>& initial_balls, const double max_change_percent,
        const double max_overlapped_ball_radius_change_ratio,
        const bool preserve_high_quality_balls) {
        if (initial_balls.size() < 3) {
            GS_LOG_TRACE_MSG(trace,
                             "GolfSimCamera::RemoveUnlikelyRadiusChangeBalls found too few (< 3) "
                             "balls.  Not processing anything.");
            return;
        }
        ...
        // Identify any balls that are outside the expected radius range by retaining them only in
        // the initial ball vector
        for (int i = (int)initial_balls.size() - 3; i >= 0; i--) {
            ...
            double middle_to_right_ball_proximity_pixels =
                initial_balls[i + 1].PixelDistanceFromBall(initial_balls[i + 2]);
            double middle_to_left_ball_proximity_pixels =
                initial_balls[i].PixelDistanceFromBall(initial_balls[i + 1]);

            double middle_to_right_distance_adjustment =
                (middle_to_right_ball_proximity_pixels / 150.0) / 100.0;
            double middle_to_left_distance_adjustment =
                (middle_to_left_ball_proximity_pixels / 150.0) / 100.0;

            if ((b2_radius > (b1_radius * (1.0 + max_change_percent / 100. +
                                           middle_to_left_distance_adjustment)) &&
                 b2_radius > (b3_radius * (1.0 + max_change_percent / 100. +
                                           middle_to_right_distance_adjustment))) ||
                (b2_radius < (b1_radius * (1.0 - max_change_percent / 100. -
                                           middle_to_left_distance_adjustment)) &&
                 b2_radius < (b3_radius * (1.0 - max_change_percent / 100. -
                                           middle_to_right_distance_adjustment)))) {
                if (initial_balls[i + 1].quality_ranking >= kNumberHighQualityBallsToRetain_) {
                    GS_LOG_TRACE_MSG(
                        trace,
                        "RemoveUnlikelyRadiusChangeBalls removing ball " + std::to_string(i + 1) +
                            " because it was too much smaller/larger than both adjacent balls.");
                    initial_balls.erase(initial_balls.begin() + i + 1);
                } else {
                    GS_LOG_TRACE_MSG(trace, "RemoveUnlikelyRadiusChangeBalls NOT removing ball " +
                                                std::to_string(i + 1) +
                                                " because although it was larger than both "
                                                "adjacent balls, it was a high-quality circle.");
                }
                ...
                if (middle_to_right_ball_proximity_pixels < b3_radius &&
                    middle_to_right_ball_proximity_pixels <
                        middle_to_left_ball_proximity_pixels / 2) {
                    ...
                    if (right_radius_change >
                        ...
                        if (initial_balls[i + 2].quality_ranking >=
                                kNumberHighQualityBallsToRetain_ ||
                            preserve_high_quality_balls == false) {
                            GS_LOG_TRACE_MSG(trace,
                                             "RemoveUnlikelyRadiusChangeBalls removing ball " +
                                                 std::to_string(i + 2) +
                                                 " because it was much larger/smaller than the "
                                                 "ball it overlaps.");
                ...
                if (middle_to_left_ball_proximity_pixels < b1_radius &&
                    middle_to_left_ball_proximity_pixels <
                        middle_to_right_ball_proximity_pixels / 2) {
                    ...
                    if (left_radius_change >
                        ...
                        if (initial_balls[i].quality_ranking >
                                kNumberHighQualityBallsToRetain_ - 1 ||
                            preserve_high_quality_balls == false) {
                            GS_LOG_TRACE_MSG(trace,
                                             "RemoveUnlikelyRadiusChangeBalls removing ball " +
                                                 std::to_string(i) +
    ...
    void GolfSimCamera::RemoveOffTrajectoryBalls(std::vector<GolfBall>& initial_balls,
        ...
        if (initial_balls.size() < 1) {
            GS_LOG_TRACE_MSG(
                warning, "RemoveOffTrajectoryBalls - balls vector was empty.  Exiting function.");
            return;
        }
        ...
        for (int i = (int)initial_balls.size() - 1; i >= 0; i--) {
            ...
            double ball_distance =
                GetPerpendicularDistanceFromLine(b.x(), b.y(), best_ball.x(), best_ball.y(),
                                                 second_best_ball.x(), second_best_ball.y());

            if (ball_distance > max_distance_from_trajectory) {
                // GS_LOG_TRACE_MSG(trace, "Not analyzing ball " + std::to_string(i) + " due to it
                // having off - trajectory distance of : " + std::to_string(ball_distance));
                initial_balls.erase(initial_balls.begin() + i);
            }
        }
    }
    ...
    void GolfSimCamera::RemoveNearbyPoorQualityBalls(std::vector<GolfBall>& initial_balls,
        ...
        for (size_t outer_index = 0; outer_index < balls_copy.size(); outer_index++) {
            ...
            for (int i = (int)initial_balls.size() - 1; i > (int)outer_index; i--) {
                ...
                // NCNN detections lack HoughCircles quality scores, so use spatial ordering as a
                // proxy
                ...
                if (b.ball_color_ == GolfBall::BallColor::kModelDetected &&
                    ...
                    quality_difference =
                        std::abs(i - (int)outer_index);  // Position difference in sorted list
                } else {
                    // Legacy HoughCircles quality ranking
                    quality_difference = b.quality_ranking - current_ball.quality_ranking;
                }

                if (ball_distance < max_ball_proximity &&
                    quality_difference > max_quality_difference) {
                    GS_LOG_TRACE_MSG(
                        trace, "Not analyzing ball " + std::to_string(i) +
                                   " due to its proximity of : " + std::to_string(ball_distance) +
                                   " and quality difference of " +
                                   std::to_string(quality_difference));
                    initial_balls.erase(initial_balls.begin() + i);
                }
            }
        }
    ...
    bool GolfSimCamera::BallIsInBallVector(const GolfBall& search_ball,
                                           const GsBallsAndTimingVector& ball_vector) {
    ...
    uint GolfSimCamera::RemoveOverlappingBalls(const std::vector<GolfBall>& initial_balls,
        ...
        for (int i = (int)initial_balls.size() - 1; i >= 0; i--) {
            ...
            if (i == 0) {
                ...
                // and the loop did not drop out on the last iteration, so there was no overlap with
                // this ball, so retain the current ball and the one to it's left and stop the
                // iteration
            ...
            double proximity_limit =
                ((1. - ball_proximity_margin_percent / 100.) *
                 (next_closer_ball.measured_radius_pixels_ + ball.measured_radius_pixels_));
            ...
            if (ball_proximity_pixels < proximity_limit) {
                if (attempt_removal_of_off_trajectory_balls) {
                    ...
                    // Thus, BOTH balls (including the 'current' ball) are overlapping and may have
                    // to be ignored for the purpose of for example, spin analysis

                    // However, if we can determine that one of the balls is clearly a false
                    // positive, such as because that ball is clearly off the line-of-flight defined
                    // by the other balls, then we can just drop the false positive and keep the
                    // other ball.
                    double ball_1_distance = -1;
                    double ball_2_distance = -1;

                    FindBestBallOnLineOfFlight(initial_balls, i - 1, i, ball_1_distance,
                                               ball_2_distance, best_ball, second_best_ball);

                    // If both balls are on the trajectory (or really close), then assume they were
                    // both valid and remove both of them. Otherwise, assume one was a
                    // mis-identification and remove only it TBD - We might even want to increase
                    // the maximum off-trajectory now that we're doing quite a bit of trajectory
                    // checks before this?
                    ...
                    if (ball_1_distance < kMaximumOffTrajectoryDistance_ &&
                        ...
                        // Effectively get rid of both the current (overlapped) ball and the one to
                        // the left by skipping to the next ball to the left (if it exists).  They
                        // are both unlikely to be useful for spin analysis (and possibly position)
                        ...
                        if (ball.quality_ranking < kNumberHighQualityBallsToRetain_ &&
                            ...
                            number_removed++;
                            continue;
                        } else if (next_closer_ball.quality_ranking <
                                       kNumberHighQualityBallsToRetain_ &&
                                   preserve_high_quality_balls == true) {
                            // Preserve the left-most ball by simply skipping inserting the
                            // right-most ball in the return vector.  Assume the right-most is not a
                            // 'real' ball.
                            continue;
                        } else {
                            ...
                            // special case - if we got rid of both balls and there's only one more
                            // ball that we would be looking at, there's no way to compare that ball
                            // to the ones we have just removed.  But, knowing that the spacing is
                            // even closer as we move to the left, we'll just effectively remove
                            // that last ball as well since it likely overlapped with the current
                            // left-side ball.
                            if (i == 1) {
                                break;
                            } else {
                        ...
                        // At least one of the balls is a misidentified ball.  Assume one ball is
                        // real. Get rid of the ball that was furthest from the assumed line of
                        // flight unless that ball is one of the highest-ranked, in which case, lose
                        // the other one. The off-track ball is the one that is furthest from the
                        // line defined by the end points determined earlier
                        if (ball_1_distance > ball_2_distance) {
                            // The next ball to the left was the ball that is furthest from the line
                            // of flight Save the current ball (to the right of the other) and
                            // effectively skip the other We think that we only removed ONE real
                            // ball, however
                            ...
                            // The next ball to the left was the ball that is closest to the line of
                            // flight Effectively skip the current ball and move on to looking at
                            // the next ball to the left In this case, we don't believe we've
                            // removed a real ball, so don't count it
                            continue;
                        }
                    }
                } else {
                    ...
                    i--;
                    number_removed += 2;
                    // See speciai case comment above for an explanation for potentially breaking
    ...
    void GolfSimCamera::RemoveTooSmallOrBigBalls(std::vector<GolfBall>& initial_balls,
        ...
        if (initial_balls.size() < 1) {
            GS_LOG_TRACE_MSG(
                warning, "RemoveTooSmallOrBigBalls - balls vector was empty.  Exiting function.");
            return;
        }
        ...
        int min_strobed_ball_radius =
            int(expected_best_ball.measured_radius_pixels_ * kMinStrobedBallRadiusRation);
        int max_strobed_ball_radius =
            int(expected_best_ball.measured_radius_pixels_ * kMaxStrobedBallRadiusRation);
        ...
        for (int i = (int)initial_balls.size() - 1; i >= 0; i--) {
            ...
            if (radius < min_strobed_ball_radius || radius > max_strobed_ball_radius) {
                GS_LOG_TRACE_MSG(trace, "  Not analyzing found ball due to it having radius = {" +
    ...
    void GolfSimCamera::DetermineSecondBall(std::vector<GolfBall>& return_balls,
        ...
        // One question is how to determine which OTHER ball should be used to compare with the
        // face_ball Should it be the nearest or the furthest?  Closest to the center (other than
        // the middle ball)? Answer:  We will focus on the spin analysis Most centered will be
        // better to compare for spin, as the face-on angle will be the most similar.  For high
        // speed spins, the closer the ball images, the higher the recoverable speed, because the
        // ball doesn't have as much time to spin.
    ...
    void GolfSimCamera::RemoveUnlikelyAngleLowerQualityBalls(std::vector<GolfBall>& initial_balls) {
        ...
        if (initial_balls.size() < 2) {
            GS_LOG_TRACE_MSG(
                warning, "initial_balls vector was empty or had only 1 ball.  Exiting function.");
            return;
        }
        ...
        // Examine each of the search balls and remove any near-by balls that are at an unreasonable
        // angle. This process takes care of the (likely) situation when the top (position 0)
        // quality ball is near another high-quality ball (e.g., position 1), but the second ball is
        // a mistake and is at a weird angle below/above the higher-quality ball.
        ...
        while ((outer_index < initial_balls.size() - 1) && (initial_balls.size() > 0)) {
            ...
            for (size_t i = initial_balls.size() - 1;
                 (initial_balls.size() > 0) && (i > outer_index); i--) {
                GolfBall& b = initial_balls[i];
                ...
                if (x_distance_pixels > kUnlikelyAngleMinimumDistancePixels) {
                    ...
                    // GS_LOG_TRACE_MSG(trace, "RemoveUnlikelyAngleLowerQualityBalls not analyzing
                    // ball " + std::to_string(i) + " because its X coordinate is too far from ball
                    // number " + std::to_string(outer_index)); If the balls are right above/below
                    // each other, just pick a very big angle to avoid doing a divide by zero The
                    // large angle should ensure that the 'bad' ball is removed
                    ball_angle_degrees = 89;
                } else {
                    // Calculate angle so that it doesn't matter which ball is to the left
                    ball_angle_degrees =
                        CvUtils::RadiansToDegrees(atan((double)(b.y() - current_ball.y()) /
                                                       (double)std::abs(b.x() - current_ball.x())));
                    ;
                }

                if (b.x() > current_ball.x()) {
                    // The ball to be compared to the outer loop ball is to the right of the outer
                    // loop
                    ball_angle_degrees = -ball_angle_degrees;
                } else {
                    // The ball to be compared to the outer loop ball is to the left of the outer
                    // loop Leave the angles alone
                }
    ...
    void GolfSimCamera::RemoveWrongColorBalls(const cv::Mat& rgbImg,
        ...
        std::vector<GsColorTriplet> statistics =
            CvUtils::GetBallColorRgb(rgbImg, expected_best_ball.ball_circle_);
        ...
        // Expect that the expected_best_ball will not be removed from the vector, as its
        ...
        for (int i = (int)initial_balls.size() - 1; i >= 0; i--) {
            ...
            if (b.ball_color_ == GolfBall::BallColor::kModelDetected) {
                GS_LOG_TRACE_MSG(
                    trace, "Skipping color analysis for model-detected ball " + std::to_string(i));
                continue;
            }

            std::vector<GsColorTriplet> statistics =
                CvUtils::GetBallColorRgb(rgbImg, b.ball_circle_);
            ...
            GS_LOG_TRACE_MSG(
                trace, "\n\nExamining circle No. " + std::to_string(i) + ".  Radius " +
                           std::to_string(b.measured_radius_pixels_) + " pixels. Average RGB is{ " +
                           LoggingTools::FormatGsColorTriplet(avg_RGB) + ". Average HSV is{ " +
                           LoggingTools::FormatGsColorTriplet(CvUtils::ConvertRgbToHsv(avg_RGB)));
            ...
            float rgb_median_diff =
                CvUtils::ColorDistance(median_RGB, expectedBallRGBMedian);             // TBD
            float rgb_std_diff = CvUtils::ColorDistance(std_RGB, expectedBallRGBStd);  // TBD

            // Even if a potential ball has a really close median color, if the STD is even a little
            // off, we want to down - grade it The following works to mix the three statistics
            // together appropriately Will also penalize balls that are found toward the tail end of
            // the list
            //  NOMINAL - large StdDiff was throwing off? float calculated_color_difference =
            //  rgb_avg_diff + (float)(100. * pow(rgb_std_diff, 2.));
            // TBD - this needs to be optimized and should probably be different than the code in
            // the BallImageProc class.
            //                    double calculated_color_difference = pow(rgb_avg_diff,2) +
            //                    (float)(2. * pow(rgb_std_diff, 2.)) + (float)(125. * pow(i, 4.));
            // NOTE - if the flash-times are different for the ball we are using for the color, this
            // is likely to pick the wrong thing.

            // We are primarily concerned about situations where the median RGB went UP, because
            // that's usually what happens when a ball overlaps with another ball(s).  We are going
            // to weigh that situation more heavily
            ...
            if (CvUtils::IsDarker(avg_RGB, expectedBallRGBMedian)) {
                ...
                std_difference_component = (double)(kColorDifferenceStdPostMultiplierForLighter *
                                                    pow(2.0 * rgb_std_diff, 2.));

                calculated_color_difference = rgb_difference_component + std_difference_component;
            }

            GS_LOG_TRACE_MSG(
                trace, "Found " + brightness + " circle number " + std::to_string(i) + "(x,y) = (" +
            ...
            // Identify any balls that are outside the expected radius range by retaining them only
            // in the initial ball vector
            if (calculated_color_difference > max_strobed_ball_color_difference) {
                GS_LOG_TRACE_MSG(
                    trace, "  Not analyzing found ball No. " + std::to_string(i) +
                ...
                GS_LOG_TRACE_MSG(trace, " rgb_difference_component was " +
    ...
    void GolfSimCamera::RemoveWrongRadiusBalls(std::vector<GolfBall>& initial_balls,
        ...
        // Expect that the expected_best_ball will not be removed from the vector, as it's
        // differences should be zero.
        ...
        for (int i = (int)initial_balls.size() - 1; i > 0; i--) {
            ...
            double max_radius_different =
                nominal_radius * (kMaxRadiusDifferencePercentageFromBest / 100.0);

            // Remove any balls that are too far away from the expected radius range
            if (radius_difference > max_radius_different) {
                GS_LOG_TRACE_MSG(trace, "  Not analyzing found ball No. " + std::to_string(i) +
                                            " due to it having too different a radius from best "
                                            "ball ( difference was " +
                                            std::to_string(radius_difference) +
                                            "), and the max was " +
    ...
    void GolfSimCamera::ReportBallSearchError(const int number_balls_found) {
        ...
        if (number_balls_found == 0) {
            ...
            root_cause_str =
                "Unable to find at least two ball exposures after ball hit.  It's possible the "
                "ball was hit faster or slower than the system can handle.";
        } else {
            root_cause_str =
    ...
    bool GolfSimCamera::AnalyzeStrobedBalls(
        const cv::Mat& strobed_balls_color_image, const cv::Mat& strobed_balls_gray_image,
        const GolfBall& calibrated_ball, GsBallsAndTimingVector& return_balls_and_timings,
        GsBallsAndTimingVector& non_overlapping_balls_and_timing, GolfBall& face_ball,
        GolfBall& ball2, long& time_between_ball_images_uS) {
        GS_LOG_TRACE_MSG(
            trace, "AnalyzeStrobedBalls(ball).  calibrated_ball = " + calibrated_ball.Format());
        ...
        double expected_strobed_ball_radius =
            calibrated_ball.radius_at_calibration_pixels_ *
        ...
        bool result = ip->GetBall(strobed_balls_color_image, non_const_ball, initial_balls, roi,
                                  processing_mode, useLargestFoundBall, dontReportErrors);
        ...
        ShowAndLogBalls("AnalyzeStrobedBall_Initial_Candidate_Balls", strobed_balls_color_image,
                        initial_balls, kLogIntermediateExposureImagesToFile);
        ...
        double max_color_difference = (GolfSimClubs::GetCurrentClubType() == GolfSimClubs::kPutter)
                                          ? kMaxPuttingBallColorDifferenceRelaxed
                                          : kMaxStrobedBallColorDifferenceRelaxed;
        ...
        for (auto& ball : initial_balls) {
            if (ball.ball_color_ == GolfBall::BallColor::kModelDetected) {
                ...
                if (!ComputeSingleBallXYZOrthoCamPerspective(*this, ball)) {
                    GS_LOG_MSG(warning,
                               "Failed to compute spatial physics for model-detected ball - "
                               "continuing anyway");
                }
                ...
                GS_LOG_TRACE_MSG(
                    trace, "Model-detected ball physics complete: DistFromLens=" +
                               std::to_string(ball.distance_to_z_plane_from_lens_) +
                               "m, CalFocLen=" + std::to_string(ball.calibrated_focal_length_));
            }
        }

        RemoveWrongColorBalls(strobed_balls_color_image, initial_balls, expected_best_ball,
                              max_color_difference);
        ShowAndLogBalls("AnalyzeStrobedBall_After_RemoveWrongColorBalls", strobed_balls_color_image,
                        initial_balls, kLogIntermediateExposureImagesToFile);
        LoggingTools::Trace("Initial_balls after RemoveWrongColorBalls: ", initial_balls);

        // Seems like wrong radius balls can be a better and more-ball-removing early filter than
        // UnlikelyAngle balls
        ...
        initial_balls.erase(initial_balls.begin() + 2);
        ShowAndLogBalls("initial_balls after hack for testing", strobed_balls_color_image,
        initial_balls, kLogIntermediateExposureImagesToFile);
        ***/
        ...
        // TBD - Am putting this back in because we're still getting too many balls with the new
        // edge detector
        RemoveLowScoringBalls(initial_balls, kMaxBallsToRetain);
        ShowAndLogBalls("AnalyzeStrobedBall_After_RemoveLowScoringBalls", strobed_balls_color_image,
                        initial_balls, kLogIntermediateExposureImagesToFile);
        ...
        // Especially if we are using a loose Hough for initial identification, this could get rid
        // of 100 or more ball candidates
        ...
        // A frequent problem because of the very-broad net we cast is that crappy balls end up
        // determined right next to a good one.  Get rid of 'em
        if (number_of_initial_balls > 20) {
            ShowAndLogBalls("AnalyzeStrobedBall Balls before RemoveNearbyPoorQualityBalls",
                            strobed_balls_color_image, initial_balls,
                            kLogIntermediateExposureImagesToFile);
            RemoveNearbyPoorQualityBalls(initial_balls, ip->min_ball_radius_,
        ...
        if (initial_balls.size() == 1) {
            GS_LOG_MSG(warning,
                       "GetBall() found only one ball after initial filtering.  Ball velocity may "
                       "have been too high or very slow.");
            return false;
        }
        ...
        RemoveUnlikelyRadiusChangeBalls(initial_balls, max_intermediate_ball_radius_change_percent,
                                        kMaxOverlappedBallRadiusChangeRatio);
        ...
        // Identify any balls that are overlapping with other balls.  Such balls are unlikely to be
        // useful for spin calculations.
        ...
        ShowAndLogBalls("AnalyzeStrobedBall_After_1stRemoveOffTrajectoryBalls",
                        strobed_balls_color_image, initial_balls, false);

        // This should be using a tighter trajectory limit, but we're trying to accomodate some slow
        // balls right now that end up with curved trajectories.  TBD
        SortBallsByXPosition(initial_balls);
        RemoveOffTrajectoryBalls(initial_balls, kMaxDistanceFromTrajectory, best_ball,
                                 second_best_ball);
        ...
        // the color-filter.  That way, we can later determine a set of strictly non-overlapping
        // balls for purposes of, e.g., spin analysis. Otherwise, we might remove balls that were
        // actually overlaps and then not be able to later remove any (strictly) overlapping balls
        // for spin analysis.
        ...
        int number_overlapping_balls_removed =
            RemoveOverlappingBalls(initial_balls, kBallProximityMarginPercentRelaxed, true,
        ...
        ShowAndLogBalls("AnalyzeStrobedBall_After_1stRemoveOverlappingBalls",
                        strobed_balls_color_image, first_pass_balls,
                        kLogIntermediateExposureImagesToFile);

        // TBD -Trying these two steps twice to deal with really close balls that remain after the
        // first pass THIS time, do not preserve high-quality balls that look sketchy
        RemoveUnlikelyRadiusChangeBalls(first_pass_balls,
                                        max_intermediate_ball_radius_change_percent,
                                        kMaxOverlappedBallRadiusChangeRatio, false);
        ShowAndLogBalls("AnalyzeStrobedBall_After_3rdRemoveUnlikelyRadiusChangeBalls",
                        strobed_balls_color_image, first_pass_balls,
                        kLogIntermediateExposureImagesToFile);
        ...
        std::vector<GolfBall> return_balls;
        number_overlapping_balls_removed +=
            RemoveOverlappingBalls(first_pass_balls, kBallProximityMarginPercentRelaxed, true,
        ...
        ShowAndLogBalls("AnalyzeStrobedBall Candidates after RemoveOverlappingBalls",
                        strobed_balls_color_image, return_balls,
                        kLogIntermediateExposureImagesToFile);
        GS_LOG_TRACE_MSG(trace, "Number of Return_balls Candidates after RemoveOverlappingBalls: " +
                                    std::to_string(return_balls.size()));

        if (return_balls.size() < 2) {
            GS_LOG_MSG(error,
                       "GetBall() found only one ball after color filtering.  Ball velocity may "
                       "have been too high or very slow.");
            return false;
        }
        ...
        if (return_balls.size() < 2) {
            GS_LOG_MSG(error,
                       "GetBall() found fewer than two balls after removing overlapping ball "
                       "images.  Ball velocity may have been too slow.");
            return false;
        }

        // Determine which ball is closest to the center of the image.  That will be the best one to
        // use for spin determination
        SortBallsByXPosition(return_balls);

        ShowAndLogBalls("AnalyzeStrobedBall_After_Size_Color_Filtered_Candidates",
                        strobed_balls_color_image, return_balls,
                        kLogIntermediateExposureImagesToFile);
        GS_LOG_TRACE_MSG(trace, "Return_balls after color and size filter" +
                                    std::to_string(return_balls.size()));

        int most_centered_ball_index = GetMostCenteredBallIndex(return_balls);

        GS_LOG_TRACE_MSG(trace, "The closest-to-center-screen ball index was " +
        ...
        // NOTE - At this point we cannot re-sort the ball vector or else our indexes may not make
        // sense any more.
        ...
        ***/
        ...
        if (BallImageProc::kUseBestCircleRefinement && perform_best_circle_fit_when_putting) {
            for (size_t first_index = 0; first_index < return_balls.size(); first_index++) {
                ...
                // TBD - Still trying to figure out of the largest circle (among the top few) is the
                // best?
                if (!BallImageProc::DetermineBestCircle(strobed_balls_gray_image, original_ball,
                                                        BallImageProc::kUseBestCircleLargestCircle,
                                                        best_circle)) {
                    GS_LOG_MSG(warning,
                               "GolfSimCamera::AnalyzeStrobedBalls - failed to DetermineBestCircle "
                               "spin ball number " +
                                   std::to_string(first_index) +
                                   " .Using originally - found ball.");
                    continue;
                }
                ...
                // Also find this ball in the possibly_overlapping_balls_before_color_filter and
                // update it's circle information
        ...
        ShowAndLogBalls("AnalyzeStrobedBall - improved ball set", strobed_balls_color_image,
                        return_balls, kLogIntermediateExposureImagesToFile);
        ...
        DetermineStrobeIntervals(return_balls, most_centered_ball_index, second_ball_index,
                                 time_between_ball_images_uS, return_balls_and_timings);

        GS_LOG_MSG(info, "Time between center-most images: " +
                             std::to_string((double)time_between_ball_images_uS / 1000.0) + "ms");

        // This is a "final" image, so we want to store it
        ShowAndLogBalls("AnalyzeStrobedBall_Final_Candidate_Balls", strobed_balls_color_image,
                        return_balls, true, most_centered_ball_index, second_ball_index);

        face_ball = return_balls[most_centered_ball_index];
        ball2 = return_balls[second_ball_index];

        // Also return a set of fairly-strictly-non-overlapping balls for use with, e.g., spin
        // analysis NOTE - if there is a negative %, it is here to make sure the balls are
        // well-separated. Otherwise, we sometimes get some overlap with a ball that was missed.
        ...
        ShowAndLogBalls(
            "AnalyzeStrobedBall - possibly_overlapping_balls_before_color_filter before "
            "RemoveOverlappingBalls",
            strobed_balls_color_image, possibly_overlapping_balls_before_color_filter, false);
        LoggingTools::Trace(
            "AnalyzeStrobedBall - possibly_overlapping_balls_before_color_filter before "
            "RemoveOverlappingBalls: ",
        ...
        ShowAndLogBalls("AnalyzeStrobedBall - balls after final RemoveUnlikelyRadiusChangeBalls",
                        strobed_balls_color_image, possibly_overlapping_balls_before_color_filter,
                        false);

        // TBD - For some reason, we end up creating the possibly-overlapping-balls vector at a time
        // before the poor quality balls are necessarily removed. So, do so here
        ...
        RemoveOverlappingBalls(possibly_overlapping_balls_before_color_filter,
                               ...
                               non_overlapping_balls, best_ball, second_best_ball,
                               false /* preserve_high_quality_balls */);

        ShowAndLogBalls("AnalyzeStrobedBall - balls after strictly-overlapping are removed",
                        strobed_balls_color_image, non_overlapping_balls,
                        kLogIntermediateExposureImagesToFile);

        RemoveWrongColorBalls(strobed_balls_color_image, non_overlapping_balls, expected_best_ball,
                              kMaxStrobedBallColorDifferenceStrict);
        ...
        for (int i = (int)(non_overlapping_balls_and_timing.size() - 1); i >= 0; i--) {
            ...
            for (GolfBall& non_overlapping_ball : non_overlapping_balls) {
                if (be.ball.x() == non_overlapping_ball.x() &&
                    be.ball.y() == non_overlapping_ball.y()) {
            ...
            if (!ball_is_in_non_overlapping_vector) {
                ...
                if (i < (int)(non_overlapping_balls_and_timing.size() - 1)) {
                    non_overlapping_balls_and_timing[i + 1].time_interval_before_ball_us +=
                        be.time_interval_before_ball_us;
                }

                non_overlapping_balls_and_timing.erase(non_overlapping_balls_and_timing.begin() +
                                                       i);
            }
        }
    ...
    double GolfSimCamera::AdjustDistanceForSlowing(const double initial_right_distance) {
        ...
        if (GolfSimOptions::GetCommandLineOptions().practice_ball_) {
            kBallConstantSpeedAdjustmentPercentage =
                kPracticeBallSpeedSlowdownPercentage;  // percent
        } else if (GolfSimClubs::GetCurrentClubType() == GolfSimClubs::kPutter) {
            kBallConstantSpeedAdjustmentPercentage =
                kPuttingBallSpeedSlowdownPercentage;  // percent
        } else {
            kBallConstantSpeedAdjustmentPercentage =
                kStandardBallSpeedSlowdownPercentage;  // percent
        }

        double right_distance =
            initial_right_distance * (1.0 + kBallConstantSpeedAdjustmentPercentage / 100.0);
    ...
    int GolfSimCamera::FindClosestRatioPatternMatchOffset(const std::vector<double> distance_ratios,
        ...
        for (int distance_pattern_offset = 0;
            ...
            double difference_in_ratios =
                ComputeRatioDistance(distance_ratios, pulse_ratios, distance_pattern_offset);

            GS_LOG_TRACE_MSG(trace, "difference_in_ratios for offset " +
                                        std::to_string(distance_pattern_offset) +
                                        " is: " + std::to_string(difference_in_ratios));
    ...
    // Returns a score of the closeness of the vector of distance_ratios within the pulse_ratios at
    // an offset of the distance_ratios from the beginning of the pulse_ratios
    double GolfSimCamera::ComputeRatioDistance(const std::vector<double> distance_ratios,
        ...
        // Compute the differences between each distance ratio in the pattern of distances as
        // compared to each possible offset of those ratios to the pulse interval ratios.
        for (size_t distance_ratio_index = 0; distance_ratio_index < distance_ratios.size();
             distance_ratio_index++) {
            // If we got more ball distance ratios than we have collapsed (and thus down-sized)
            // pulse ratios, then just stop and return a big error number to drop this comparison
            // from the best-of list.

            if ((distance_ratio_index + distance_pattern_offset) >= pulse_ratios.size()) {
                LoggingTools::Warning(
                    "GolfSimCamera::ComputeRatioDistance received a distance_ratio_index higher "
                    "than the number of pulse ratios.");
            ...
            // std::min(distance_ratio, pulse_ratio);
            double single_ratio_difference = 100. * std::abs(distance_ratio - pulse_ratio);
    ...
    // Set the element of the vector to true when there is a 1 bit (a flag to remove the
    // corresponding strobe pulse) in the vector
    ...
    bool GolfSimCamera::DetermineStrobeIntervals(std::vector<GolfBall>& input_balls,
        ...
        if (distances.empty()) {
            LoggingTools::Warning(
                "GetBallDistancesAndRatios returned no data.  Has the strobe vector been "
                "established?");
            return false;
        }
        ...
        GS_LOG_TRACE_MSG(trace, "About to call GetPulseIntervalsAndRatios with " +
                                    std::to_string(pulse_intervals_from_strobe.size()) +
                                    " pulse intervals");

        bool result = GetPulseIntervalsAndRatios(pulse_intervals_from_strobe, test_pulse_intervals,
                                                 test_pulse_ratios);
        ...
        // Look at each pulse interval ratio that we have and see which is closest to the ratio(s)
        // that exist for distance deltas. NOTE - Assumes near-constant ball speed such that the
        // ratios of strobe intervals is directly proportional to the distances covered during those
        // intervals.
        ...
        if (distance_ratios.size() > 0) {
            // Each interval ratio we have will involve three balls (left/right/middle), and should
            // be roughly equal to the ratio of the distance between the middle and next ball
            // divided by the distance between the ball before the middle and the middle

            // Compute the correlation (closeness) of the ball distance ratio pattern to the pulse
            // ratio pattern.  The distance ratios will always be <= the number of pulse intervals,
            // due to the possibility of losing some highly-overlapped images early in the pattern.

            // There shouldn't be too many missed pulses, but they could occur anywhere, so make
            // sure that we consider possible collapsed pulses all the way to the end
            ...
            // We will now generate all combinations of fitting the <x> 'missing' balls into the <y>
            // possible positions that exist in the pulse vector.  We had originally been assuming
            // there would just be one contiguous group of missing balls, but that turned out not to
            // be true.  So, now go through all combinations
            ...
            if (number_ball_exposures > number_of_strobes) {
                LoggingTools::Warning(
                    "GetBallDistancesAndRatios had more exposures than strobes.  It is possible "
                    "the image being analyzed was generated with a different strobing "
                    "configuration.");
                return false;
            }
            ...
            // This code iterates though all possible numbers of an n-bit number and whenever the
            // number has the correct number of bits, we use that pattern as a possible candidate
            // for how the exposures correllate to the strobes.
                ...
                **/
                ...
                if (!GetPulseIntervalsAndRatiosFromIntervalVector(intervals_to_collapse_vector,
                    ...
                    GS_LOG_TRACE_MSG(error,
                                     "Failed to GetPulseIntervalsAndRatiosFromIntervalVector");
                    return false;
                }

                // Too chatty- LoggingTools::Trace("The (potentially collapsed) pulse_intervals were
                // (ignore last '0' interval): ", pulse_intervals); LoggingTools::Trace("The
                // (potentially collapsed) pulse_ratios were : ", pulse_ratios);

                double delta_to_closest_ratio;

                int best_local_offset_of_distance_ratios = FindClosestRatioPatternMatchOffset(
                    distance_ratios, pulse_ratios, delta_to_closest_ratio,
                    true /* Try any offset */);
                ...
                if (best_local_offset_of_distance_ratios >= 0 &&
                    ...
                    GS_LOG_TRACE_MSG(trace,
                                     "------------> Found best (so far) pulse ratio pattern match. "
                                     " best_ratio_distance= " +
                                         std::to_string(best_ratio_distance) +
                                         " best_final_offset_of_distance_ratios= " +
                                         std::to_string(best_ratio_distance) +
                                         " best_intervals_pattern_index= " +
                                         std::to_string(best_intervals_pattern_index) +
                                         ".  Vector was : ");
                    PrintPulseVector(candidate_intervals_to_collapse_patterns_vector
                                         [best_intervals_pattern_index]);
                }
            }

            // In the unlikely event we didn't find ANY best interval, just default it to the first
            // interval
            if (best_intervals_pattern_index < 0) {
                best_intervals_pattern_index = 0;
            }

            GS_LOG_TRACE_MSG(trace, "------------> Best-fitting pulse vector was number: " +
                                        std::to_string(best_intervals_pattern_index) +
                                        ".  With score of: " + std::to_string(best_ratio_distance) +
                                        ".  Vector was : ");
            PrintPulseVector(
                candidate_intervals_to_collapse_patterns_vector[best_intervals_pattern_index]);

            // If we found a best correllation, re-create the corresponding set of ratios and
            // intervals
            std::vector<double> pulse_ratios;
            std::vector<float> pulse_intervals;
            ...
            if (!GetPulseIntervalsAndRatiosFromIntervalVector(
                    candidate_intervals_to_collapse_patterns_vector[best_intervals_pattern_index],
                    PulseStrobe::GetPulseIntervals(), pulse_intervals, pulse_ratios)) {
            ...
            for (size_t i = 0; i < input_balls.size(); i++) {
                ...
                if (i > 0) {
                    ...
                    if (interval_index < pulse_intervals.size()) {
                        ...
                        GS_LOG_MSG(warning, "Pulse interval index " +
                                                std::to_string(interval_index) +
                                                " out of bounds (size: " +
                                                std::to_string(pulse_intervals.size()) + ")");
            ...
            if (second_ball_index > most_centered_ball_index) {
                ...
                if (interval_index < pulse_intervals.size()) {
                    time_between_ball_images_uS =
                        (long)std::round(1000 * pulse_intervals[interval_index]);
                } else {
                ...
                size_t interval_index =
                    most_centered_ball_index + best_final_offset_of_distance_ratios - 1;
                if (interval_index < pulse_intervals.size()) {
                    time_between_ball_images_uS =
                        (long)std::round(1000 * pulse_intervals[interval_index]);
                } else {
            ...
            if (GolfSimClubs::GetCurrentClubType() == GolfSimClubs::kPutter) {
                GS_LOG_MSG(warning,
                           "DetermineStrobeInterval received only two recognized balls - will make "
                           "a guess that this was the last two exposures.");

                // We only have two valid ball images
                // Special cases where we only have 2 balls initially, or if we ultimately only end
                // up with two. Having only two balls might occur when the ball was hit really fast,
                // and we only got two strobes before the ball went out of the FoV.  In this case,
                // we (MAYBE) can assume that the first interval must be the first strobe delay.
                //    We could do:   time_between_ball_images_uS = pulse_intervals[0];
                // OR...
                // Maybe we trust that we actually managed to identify EACH of the strobes (even
                // though we threw some out earlier) In that case, just count the number of images
                // up to the two balls of interest and pick that pulse length:
                ...
                // We will retrieve the actual pulse interval (in uS) from the list of such
                // intervals. Just need to figure out WHICH interval coresponds to the two balls of
                // interest.
                if (!GetPulseIntervalsAndRatios(PulseStrobe::GetPulseIntervals(), pulse_intervals,
                                                pulse_ratios)) {
                ...
                if (input_balls.size() == 2) {
                    ...
                    time_between_ball_images_uS =
                        (long)std::round(1000 * pulse_intervals[pulse_intervals.size() - 2]);
                ...
                GS_LOG_MSG(warning,
                           "DetermineStrobeInterval received only two recognized balls - will make "
                           "a guess that this was the first two exposures.");
                ...
                // We will retrieve the actual pulse interval (in uS) from the list of such
                // intervals. Just need to figure out WHICH interval corresponds to the two balls of
                // interest.
                if (!GetPulseIntervalsAndRatios(PulseStrobe::GetPulseIntervals(), pulse_intervals,
                                                pulse_ratios)) {
                    GS_LOG_MSG(error, "GetPulseIntervalsAndRatios failed.");
    ...
    bool GolfSimCamera::GetPulseIntervalsAndRatiosFromIntervalVector(
        ...
        const std::vector<float>& initial_pulse_intervals_ms, std::vector<float>& pulse_intervals,
        std::vector<double>& pulse_ratios) {
        std::vector<float> current_pulse_intervals_ms = PulseStrobe::GetPulseIntervals();
        ...
        for (int v = 0; v < (int)intervals_to_collapse_vector.size() - 1; v++) {
            if (intervals_to_collapse_vector[v]) {
                // Ignore the current pulse_ratios for right now as we are building the complete
                // pulse_interval set At the end of this loop, ths pulse_ratios should hold the
                // final ratio set corresponding to the current_pulse_intervals_ms

                bool result = GetPulseIntervalsAndRatios(current_pulse_intervals_ms,
                                                         pulse_intervals, pulse_ratios, 1,
    ...
    bool GolfSimCamera::GetPulseIntervalsAndRatios(
        const std::vector<float>& initial_pulse_intervals_ms,
        std::vector<float>& pulse_pause_intervals, std::vector<double>& pulse_pause_ratios,
        const int number_pulses_to_collapse, const int collapse_offset) {
        GS_LOG_TRACE_MSG(trace,
        ...
        // for it is to treat things as if that strobe pulse never happened, and that the prior
        // strobe consumed the time of the lost pulse as well as the prior pulse.

        if (collapse_offset >= 0 && number_pulses_to_collapse > 0) {
            if (collapse_offset >
                (int)working_pulse_intervals.size() - 1 - number_pulses_to_collapse) {
                GS_LOG_MSG(error, "GolfSimCamera::GetPulseRatios - collapse_offset of " +
            ...
            for (int i = 0; i < number_pulses_to_collapse; i++) {
                ...
                if (collapse_offset + 1 >= (int)working_pulse_intervals.size()) {
                    GS_LOG_MSG(warning,
                               "GetPulseIntervalsAndRatios: Cannot collapse more intervals - " +
                                   std::to_string(i) + " of " +
                                   std::to_string(number_pulses_to_collapse) +
                                   " completed, vector size now " +
                                   std::to_string(working_pulse_intervals.size()));
                    break;
                }

                working_pulse_intervals[collapse_offset] +=
                    working_pulse_intervals[collapse_offset + 1];

                // The erase() will both remove the collapsed element and also move the remaining
                // element(s) over to the left by one to fill in the gap.
                working_pulse_intervals.erase(working_pulse_intervals.begin() + collapse_offset +
                                              1);
            }
        }
        ...
        for (size_t i = 0; i < pulse_pause_intervals.size() - 1; i++) {
            double ratio =
                ((double)(pulse_pause_intervals[i + 1])) / ((double)(pulse_pause_intervals[i]));
    ...
    bool GolfSimCamera::ProcessReceivedCam2Image(const cv::Mat& ball1_mat,
                                                 ...
                                                 GolfBall& result_ball, cv::Vec3d& rotationResults,
        ...
        if (!kUsePreImageSubtraction) {
            ...
            if (camera2_pre_image_.empty()) {
                GS_LOG_MSG(warning,
                           "ProcessReceivedCam2Image - not using kUsePreImageSubtraction, or "
                           "received empty camera2_pre_image_.");
            } else {
                // TBD - For test:
                /*
                ...
                LoggingTools::LogImage("", bgr[0], std::vector < cv::Point >{}, true, "pre_image
                (blue).png"); LoggingTools::LogImage("", bgr[1], std::vector < cv::Point >{}, true,
                "pre_image (green).png"); LoggingTools::LogImage("", bgr[2], std::vector < cv::Point
                ...
                */
                ...
                // LoggingTools::LogImage("", final_pre_image, std::vector < cv::Point >{}, true,
                // "scaled_pre_image.png");

                // Subtract the pre-image from the incoming strobed image to (hopefully) end up with
                // just the golf balls and not all the background clutter
                ...
                LoggingTools::LogImage("", prepared_strobed_ball_mat, std::vector<cv::Point>{},
                                       true, "strobed_img_minus_pre_image.png");
            }
        }
        ...
        auto grayscale_duration =
            std::chrono::duration_cast<std::chrono::microseconds>(grayscale_end - grayscale_start);
        GS_LOG_MSG(info, "Grayscale conversion completed in " +
                             std::to_string(grayscale_duration.count()) + "us");
        ...
        // One set of positions, below describes the relationship of camera2 to itself and the
        // z-plane of the ball. That set does not contain any displacement in the X,Y plane. The
        // offsets are from the origin, which is assumed to be where the ball is placed A second set
        // of positions describes the relationship of camera2 to camera1 so that trajectories may be
        // calculated as between the initial ball image and the second images.
        ...
        bool success = camera_1.GetCalibratedBall(camera_1, ball1_mat, calibrated_ball,
                                                  expectedBallCenter, true /* We expect the ball*/);
        ...
        if (GolfSimOptions::GetCommandLineOptions().system_mode_ == SystemMode::kCamera1Calibrate ||
            ...
            GolfSimOptions::GetCommandLineOptions().system_mode_ ==
                SystemMode::kCamera2BallLocation) {
            GS_LOG_TRACE_MSG(trace,
                             "ProcessReceivedCam2Image returning early, as we are just here to "
                             "find the ball location for testing.");
            return true;
        }

        GS_LOG_TRACE_MSG(
            trace, "ProcessReceivedCam2Image - Calibrated Ball is:\n" + calibrated_ball.Format());
        ...
        const CameraHardware::CameraOrientation camera_2_orientation =
        ...
        success = camera_2.AnalyzeStrobedBalls(strobed_balls_color_image, strobed_balls_gray_image,
                                               calibrated_ball, return_balls_and_timing,
                                               non_overlapping_balls_and_timing, first_strobed_ball,
                                               second_strobed_ball, time_between_balls_uS);
        ...
        // in a sense reversed, and TBD I think the two effects will cancel each other out with
        // respect to spin.
        ...
        if (!ComputeBallDeltas(ball1, ball2, camera_2, camera_2)) {
            GS_LOG_MSG(error,
                       "ProcessReceivedCam2Image - failed to ComputeBallLocation for ball1.");
            return false;
        }
        ...
        GS_LOG_TRACE_MSG(trace,
                         "ProcessReceivedCam2Image - ball2 (with delta information, and after "
                         "accounting for golfer handed-ness) is:\n" +
                             ball2.Format());

        // At this point, ball2 now holds the important delta information from which things like
        // velocity will be computed.  Transfer all of that to the result ball.
        result_ball = ball2;

        // Note - we could've theoretically calculated everything from just those two balls.
        // However, the following code attempts to create better measurements be including (mostly
        // averaged) information using the other balls, including the teed-ball position.  Much of
        // the data in the "result ball" (which is really just carrying the shot parameters as a
        // payload) will be overwritten below.

        // Next, calculate launch and side angles (VLA and HLA) as between the initial, stationary,
        // ball and each of the strobed images.  Given the distance from the initial ball and the
        // later in-flight exposures, the average angles should be pretty accurate, even if, for
        // example, there is some noisy calculations of the radius of the strobed ball exposures.
        // TBD - Because the ball moves in a non-linear manner for the period of time that the ball
        // is in contact with the club, we may want to compute the Ball Deltas using only the
        // later-strobed balls if the balls are still in contact with the club.

        GolfSimConfiguration::SetConstant(
            "gs_config.ball_exposure_selection.kUseOnlyHighQualityBallImagesForHLA",
            kUseOnlyHighQualityBallImagesForHLA);
        GS_LOG_MSG(trace, "ProcessReceivedCam2Image - kUseOnlyHighQualityBallImagesForHLA = " +
                              std::string(kUseOnlyHighQualityBallImagesForHLA ? "True" : "False"));

        std::vector<GolfBall> camera1_average_ball_vector;

        // As we go though the balls to create the set of balls whose properties (e.g., speed, HLA)
        // will be averaged, we can't remove so many overlapping balls that we don't have enough
        // left to get a good quality averaage.

        const int minimum_number_of_balls_to_use_for_averaging =
            3;  // 3 is somewhat arbitrary.  Could probably be 2, but the radii measurements are
                // noisy.
        int max_number_of_balls_to_remove_from_hla_averaging = std::max(
            0,
            ((int)return_balls_and_timing.size() - minimum_number_of_balls_to_use_for_averaging));
        ...
        for (size_t first_index = 0; first_index < return_balls_and_timing.size(); first_index++) {
            ...
            ball2.shot_parameters_.SetParameter(GsShotParameters::ShotParameter::kShotParameterAll,
                                                true);
            ...
            if (!camera_1.ComputeBallDeltas(calibrated_ball, ball2, camera_1, camera_2)) {
                GS_LOG_MSG(error,
                           "ProcessReceivedCam2Image - failed to ComputeBallLocation between "
                           "initial ball and strobed ball.");
                return false;
            }
            ...
            if (kUseOnlyHighQualityBallImagesForHLA &&
                (number_balls_removed_from_hla_averaging <
                 max_number_of_balls_to_remove_from_hla_averaging)) {
                // If we are only using the better balls for calculating HLA, then we need to do
                // some additional processing to see if the ball is a high quality (e.g.,
                // non-overlapping, not too far to the left side of the screen, etc.) and informa
                // the averaging function of whether this (current) ball should be used in the HLA
                // averaging

                // If the ball is overlapping, its radius is likely to be less accurate, and it is
                // also closer to the teed ball that we are using as a second point to determine the
                // ball HLAs. So, if the ball is overlapping, we will exclude it from the HLA
                // averaging, but we will still use it for averaging the VLA and other parameters.

                // One might ask why we don't just remove the overlapping balls from ALL of the
                // averaging.  The reason is that, even if a ball is overlapping, it may still have
                // a pretty good VLA and speed measurement, and we don't want to throw away good
                // data if we don't have to.  So, we will just exclude it from the HLA averaging,
                // which is the most sensitive to having an accurate radius measurement.

                if (!BallIsInBallVector(ball2, non_overlapping_balls_and_timing)) {
                    GS_LOG_MSG(trace,
                               "Removing ball number " + std::to_string(first_index) +
                                   " from HLA calculation because it overlaps with other balls.");
                    ball2.shot_parameters_.SetParameter(GsShotParameters::ShotParameter::kHLA,
                                                        false);
                    number_balls_removed_from_hla_averaging++;
                } else {
                    // Likely high-quality balls will be the balls that are further away from the
                    // teed ball.  If we have sufficient ball images to ignore some of them, do so.
                    // This is an alterative basis for reomoving a ball from the HLA calculation
                    // It's useful because in some cases, the system may not be able to recognize
                    // that two balls are overlapping.
                    if (return_balls_and_timing.size() >=
                        minimum_number_of_balls_to_use_for_averaging) {
                        int balls_to_ignore =
                            static_cast<int>(std::floor(return_balls_and_timing.size() / 2.0f));

                        if (first_index < (size_t)balls_to_ignore) {
                            GS_LOG_MSG(
                                trace,
                                "Removing ball number " + std::to_string(first_index) +
                                    " from HLA calculation because it is one of the closer balls "
                                    "to the tee position, and we have sufficient other balls.");
                            ball2.shot_parameters_.SetParameter(
                                GsShotParameters::ShotParameter::kHLA, false);
                            number_balls_removed_from_hla_averaging++;
                        }
                    }
                }
            ...
            GS_LOG_MSG(trace,
                       "Final Ball2 parameters_to_average: = " + ball2.shot_parameters_.Format());

            // Get ready to average the measurements of this calculated ball with any others
            camera1_average_ball_vector.push_back(ball2);
        }

        // We want to average the launch angles, but we don't want to average any spins or distance
        // or velocities. Doesn't really make sense to average the two pairs of balls' distances,
        // for example, as those would be different by design since it's (usually) different pairs
        GolfBall camera1_averaged_ball;
        GolfBall::AverageBalls(camera1_average_ball_vector, camera1_averaged_ball, false);

        GS_LOG_TRACE_MSG(
            trace, "Averaged angles from the initial, stationary ball to each strobed ball:\n" +
                       camera1_averaged_ball.Format());

        // Initially overwrite the angle information with the (hopefully) more accurate angles
        // formed by the initial ball to each of the strobed balls as just calculated above.
        ...
        // At this point, we also have a more accurate idea of the launch side-angle than we could
        // have derived from just the change in the radius of the ball in the camera2 image.  So,
        // re-calculate the velocity based on that side angle in combination with the other two
        // original angles
        ...
        // TBD - Let's see how the entire group of strobed balls BY THEMSELVES do in terms of HLA,
        // VLA, velocity, etc.
        GolfBall average_of_strobed_ball_data;

        if (!ComputeAveragedStrobedBallData(camera_2, return_balls_and_timing,
                                            average_of_strobed_ball_data)) {
            GS_LOG_MSG(error,
                       "ProcessReceivedCam2Image - failed to ComputeBallLocation between initial "
                       "ball and strobed ball.");
            return false;
        }
        ...
        GS_LOG_TRACE_MSG(trace, "ComputeAveragedStrobedBallData returned ball=:" +
        ...
        // This VLA is likely more accurate because the ball gets moved along from the tee for a
        // short distance before it gets airborne, and that movement is likely to be more accurately
        // captured by the angles between the strobed balls than by the angles between the initial
        // ball and the strobed balls.
        ...
        GolfSimConfiguration::SetConstant(
            "gs_config.golf_simulator_interfaces.kSkipSpinCalculation", kSkipSpinCalculation);
        ...
        if (kSkipSpinCalculation || GolfSimClubs::GetCurrentClubType() == GolfSimClubs::kPutter) {
            ...
            if (non_overlapping_balls_and_timing.size() < 2) {
                std::string error_str =
                    "Could not find two non-overlapping balls to analyze for spin.";
        ...
        if (std::abs(hla_offset) > 0.01 || std::abs(vla_offset) > 0.01) {
            GS_LOG_MSG(trace, "Applying HLA offset of " + std::to_string(hla_offset) +
                                  " and VLA offset of " + std::to_string(vla_offset) +
                                  " to results.");
        }
    ...
    bool GolfSimCamera::ProcessSpin(GolfSimCamera& camera, const cv::Mat& strobed_balls_gray_image,
        ...
        // Try to find the two closest balls while avoiding any balls really close to the edge if we
        // can. Back off if necessary
        if (!FindBestTwoSpinBalls(strobed_balls_gray_image, non_overlapping_balls_and_timing, true,
                                  spin_ball1, spin_ball2, spin_timing_interval_uS)) {
            if (!FindClosestTwoBalls(strobed_balls_gray_image, non_overlapping_balls_and_timing,
                                     false, spin_ball1, spin_ball2, spin_timing_interval_uS)) {
        ...
        ShowAndLogBalls("ProcessSpin - Final Spin Balls", strobed_balls_gray_image, finalSpinBalls,
                        kLogIntermediateExposureImagesToFile);

        // The best spin analysis will likely be between the two closest balls that are
        // non-overlapping
        rotationResults = BallImageProc::GetBallRotation(strobed_balls_gray_image, spin_ball1,
                                                         strobed_balls_gray_image, spin_ball2);
        ...
        result_ball.time_between_angle_measures_for_rpm_uS_ =
            (long)std::round(spin_timing_interval_uS);

        return true;
    }

    bool GolfSimCamera::FindClosestTwoBalls(const cv::Mat& img, const GsBallsAndTimingVector& balls,
                                            const bool use_edge_backoffs, GolfBall& ball1,
                                            GolfBall& ball2, double& timing_interval_uS) {
        int closest_ball1 = -1;
        int closest_ball2 = -1;
        ...
        for (int first_index = (int)balls.size() - 1; first_index > 0 && !found_touching_pair;
             first_index--) {
            for (int second_index = first_index - 1; second_index >= 0 && !found_touching_pair;
                 second_index--) {
    ...
    bool GolfSimCamera::ComputeAveragedStrobedBallData(
        const GolfSimCamera& camera, const GsBallsAndTimingVector& balls_and_timing,
        ...
        for (size_t i = 0; i < balls_and_timing.size() - 1; i++) {
            ...
            for (size_t j = i + 1; j < balls_and_timing.size(); j++) {
                ...
                GS_LOG_MSG(trace,
                           "ComputeAveragedStrobedBallData comparing the following two balls "
                           "(indexes are within the vector): Balls (" +
    ...
    bool GolfSimCamera::FindBestTwoSpinBalls(const cv::Mat& img,
                                             ...
                                             const bool use_edge_backoffs, GolfBall& output_ball1,
                                             GolfBall& output_ball2, double& timing_interval_uS) {
        int closest_ball1 = -1;
        int closest_ball2 = -1;
        double closest_distance_so_far = 100000;
            ...
            for (size_t j = i + 1; j < balls_and_timing.size(); j++) {
                ...
                if (pair_proximity <
                    ...
                    pair_proximity_score =
                        (10.0 * (ball1.measured_radius_pixels_ + ball2.measured_radius_pixels_)) /
                        pair_proximity;
                }
                ...
                double distance_from_screen_center_to_edge =
                    CvUtils::GetDistance(screen_edge, screen_center);
                double ball1_distance_from_screen_center =
                    CvUtils::GetDistance(ball1_center, screen_center);
                double ball2_distance_from_screen_center =
                    CvUtils::GetDistance(ball2_center, screen_center);

                double ball1_middle_proximity_score =
                    5.0 *
                    ((distance_from_screen_center_to_edge - ball1_distance_from_screen_center) /
                     distance_from_screen_center_to_edge);
                double ball2_middle_proximity_score =
                    5.0 *
                    ((distance_from_screen_center_to_edge - ball2_distance_from_screen_center) /
                     distance_from_screen_center_to_edge);

                middle_proximity_score =
                    ball1_middle_proximity_score + ball2_middle_proximity_score;
                ...
                // The 13.6 and 8 just allows the too-big-of-a-radius-change limit to be relative to
                // the number of pixels we have to work with.  Should be about 08.
                radius_similarity_score =
                    std::max(0.0, (img.rows / 13.6) - 3.0 * pow((ball1.measured_radius_pixels_ -
                                                                 ball2.measured_radius_pixels_),
                                                                2.0)) /
                    8.;
        ...
        for (const GsBallPairAndSpinCandidateScoreElement& ball_pair_element : ball_pair_elements) {
            std::string spin_ball_score_text =
                "TOTAL: " + std::to_string(ball_pair_element.total_pair_score) +
                ...
                ", middle_proximity_score: " +
                std::to_string(ball_pair_element.middle_proximity_score) +
                ", leg_proximity_score: " + std::to_string(ball_pair_element.leg_proximity_score) +
                ", radius_similarity_score: " +
                std::to_string(ball_pair_element.radius_similarity_score);

            GS_LOG_TRACE_MSG(trace, "Potential Spin Ball Combination of balls ( " +
                                        std::to_string(ball_pair_element.ball1_index) + ", " +
                                        std::to_string(ball_pair_element.ball2_index) +
                                        ") scored: " + spin_ball_score_text);
        }
        ...
        timing_interval_uS =
            balls_and_timing[index_of_ball_with_interval].time_interval_before_ball_us;
    ...
    void GolfSimCamera::DrawFilterLines(const std::vector<cv::Vec4i>& lines, cv::Mat& image,
        ...
        for (size_t i = 0; i < lines.size(); i++) {
            ...
            double line_length =
                sqrt((pow((double)(pt1.x - pt2.x), 2.) + pow((double)(pt1.y - pt2.y), 2.)));
    ...
    bool GolfSimCamera::CleanExternalStrobeArtifacts(const cv::Mat& image, cv::Mat& output_image,
        ...
        cv::GaussianBlur(
            ...
            cv::Size(kExternallyStrobedEnvPreCannyBlurSize, kExternallyStrobedEnvPreCannyBlurSize),
            0);

        // Get a good picture of the edges of the balls.  Will probably have way too many shaft
        // lines
        cv::Mat cannyOutput_for_balls;
        cv::Canny(image_gray, cannyOutput_for_balls, kExternallyStrobedEnvCannyLower,
                  kExternallyStrobedEnvCannyUpper);
        ...
        // Color Filtering - commented out (TBD) because this hasn't actually worked very well.
        // There's too much of a mix of colors

        /****
        GsColorTriplet lowerHsv{ (float)kExternallyStrobedEnvFilterHsvLowerH,
        (float)kExternallyStrobedEnvFilterHsvLowerS, (float)kExternallyStrobedEnvFilterHsvLowerV };
        GsColorTriplet upperHsv{ (float)kExternallyStrobedEnvFilterHsvUpperH,
        (float)kExternallyStrobedEnvFilterHsvUpperS, (float)kExternallyStrobedEnvFilterHsvUpperV };
        ...
        cv::dilate(color_mask_image, color_mask_image, cv::getStructuringElement(cv::MORPH_ELLIPSE,
        cv::Size(1, 1)), cv::Point(-1, -1), 1);


        // LoggingTools::DebugShowImage("External Strobe Color Mask- color_mask_image",
        color_mask_image);
        ...
        ***/
        ...
        if (kExternallyStrobedEnvBottomIgnoreHeight > 0) {
            ...
            cv::rectangle(output_image, floor_blackout_area.tl(), floor_blackout_area.br(),
    ...
    bool GolfSimCamera::TakeStillPicture(const GolfSimCamera& camera, cv::Mat& color_image) {
        ...
        GS_LOG_TRACE_MSG(
            trace, "TakeStillPicture called with camera number = " + std::to_string(camera_number));
