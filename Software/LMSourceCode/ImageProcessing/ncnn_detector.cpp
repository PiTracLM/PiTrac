...
namespace golf_sim {
    ...
    NCNNDetector::NCNNDetector(const Config& config) : config_(config) {}
    ...
    bool NCNNDetector::Initialize() {
        ...
        letterbox_buf_ =
            cv::Mat(config_.input_height, config_.input_width, CV_8UC3, cv::Scalar(114, 114, 114));

        initialized_ = true;
        GS_LOG_MSG(info, "NCNN detector initialized (" + std::to_string(config_.num_threads) +
                             " threads)");
    ...
    std::vector<NCNNDetector::Detection> NCNNDetector::Detect(const cv::Mat& image,
    ...
    void NCNNDetector::Letterbox(const cv::Mat& image, ncnn::Mat& out) {
        float scale = std::min((float)config_.input_width / image.cols,
    ...
    std::vector<NCNNDetector::Detection> NCNNDetector::PostprocessYOLO(const ncnn::Mat& output) {
    ...
    std::vector<NCNNDetector::Detection> NCNNDetector::NMS(std::vector<Detection>& detections) {
        if (detections.empty())
            return {};
    ...
    void NCNNDetector::WarmUp(int iterations) {
        cv::Mat dummy(config_.input_height, config_.input_width, CV_8UC3,
