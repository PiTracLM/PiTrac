...
std::string LibCameraInterface::kCameraMotionDetectSettings =
    "./assets/motion_detect.json";
...
LibCameraInterface::CropConfiguration
    LibCameraInterface::camera_crop_configuration_ = kCropUnknown;
cv::Vec2i LibCameraInterface::current_watch_resolution_;
cv::Vec2i LibCameraInterface::current_watch_offset_;

LibCameraInterface::CameraConfiguration
    LibCameraInterface::libcamera_configuration_[] = {
 ...
 * continuously watch the area where the ball is by taking images as quickly as
 * possible and comparing each image to the prior image.  See
 * motion_detect_stage.cpp for details on detection. As soon as movement is
 * detected, signals are sent to the camera 2 and strobe to take a picture.
 *
 ...
 * method ended \return True iff no error occurred.
 */
bool WatchForHitAndTrigger(const GolfBall &ball, cv::Mat &image,
  ...
  if (!GolfSimClubData::ProcessClubStrikeData(RecentFrames)) {
    GS_LOG_MSG(
        warning,
        "Failed to GolfSimClubData::ProcessClubStrikeData(RecentFrames().");
    // TBD - Ignore for now
    // return false;
  }
...
bool LibCameraInterface::SendCamera2PreImage(const cv::Mat &raw_image) {
  ...
  LoggingTools::LogImage("", return_image, std::vector < cv::Point >{}, true,
  "log_cam2_last_pre_image.png");
  ***/
  return true;
}

bool WatchForBallMovement(GolfSimCamera &camera, const GolfBall &ball,
                          bool &motion_detected) {
  ...
  // sensor that will be processed in each frame (cropping)
  ...
  if (motion_detected) {
    ...
    for (auto &it : boost::adaptors::reverse(RecentFrames)) {
      ...
      frame_information += "Frame " + std::to_string(frameIndex) +
      ...
      if (mostRecentFrameMat.empty()) {
        GS_LOG_TRACE_MSG(warning, "Sequence No. " +
                                      std::to_string(it.requestSequence) +
                                      " was empty.");
      }
...
bool ConfigCameraForCropping(GolfBall ball, GolfSimCamera &camera,
                             RPiCamEncoder &app) {

  // First, determine the cropping window size

  // watching_crop_width & Height defines the size of the cropping window, which
  // will be a sub-region of the camera's full resolution.
  ...
  // image size beyond whatever small cropping window we would otherwise have
  // used. Doing so is likely to slow down the frame rate, however.
  ...
  // One issue here is that the current GS camera will not crop any smaller than
  // 98x88.  So, if the ball is smaller than that, we still have to go with the
  // 98x88 and deal with the smaller ball later within the ROI processing

  // We will later want to ensure the ball is not so small that the inscribed
  // watching area ( for high FPS ) is larger than the ball and could pick up
  // unrelated movement outside of the ball. We will deal with this when
  // determining the ROI
  ...
  uint largest_inscribed_square_side_length_of_ball =
      (double)(CvUtils::CircleRadius(ball.ball_circle_)) * sqrt(2);
  GS_LOG_TRACE_MSG(
      trace, "largest_inscribed_square_side_length_of_ball is: " +
  ...
  // size.  And if that size is too large, reduce it to the size of the ball.
  if (!GolfSimClubData::kGatherClubData) {
  ...
  // the cropping size to the smalllest possible size (for FPS) and either
  // center the ball within that area, or put the ball in the bottom-right of
  // the viewport if we want to see the club data.

  // Now determine the cropping window's offset within the full camera
  // resolution image This offset will be based on the position of the ball The
  // cropOffset is where, within the full-resolution image, the top-left corner
  // of the cropping window is.

  float ball_x = CvUtils::CircleX(ball.ball_circle_);
  float ball_y = CvUtils::CircleY(ball.ball_circle_);

  // Assume first is that the ball will be centered in the cropping window, then
  // tweak it next if we're in club strike mode. Club strike imaging may require
  // an offset. NOTE - the crop offset is from the bottom right!  Not the
  // top-left.
  ...
  // otherwise, the ball is often off-center.
  ...
  GolfSimConfiguration::SetConstant(
      "gs_config.motion_detect_stage.kCroppedImagePixelOffsetLeft",
      kCroppedImagePixelOffsetLeft);
  GolfSimConfiguration::SetConstant(
      "gs_config.motion_detect_stage.kCroppedImagePixelOffsetUp",
  ...
  float x_scale =
      crop_offset_scale_adjustment_x; // NOT IMPLEMENTED YET
  ...
  crop_offset_x = crop_offset_adjustment_x +
                  (x_scale * (camera.camera_hardware_.resolution_x_ -
                              (ball_x + watching_crop_width / 2.0)));

  float y_scale =
      crop_offset_scale_adjustment_y; // NOT IMPLEMENTED YET
  ...
  crop_offset_y = crop_offset_adjustment_y +
                  (y_scale * (camera.camera_hardware_.resolution_y_ -
                              (ball_y + watching_crop_height / 2.0)));

  // If we're trying to get club images, then skew the cropping so that the ball
  // ends up near the bottom-right such that the the golf ball "watch" ROI will
  // eventually also be all the way at the bottom right (to give more room so
  // see the club)
  if (GolfSimClubData::kGatherClubData) {
    crop_offset_x += (0.5 * watching_crop_width -
                      0.5 * largest_inscribed_square_side_length_of_ball);
    crop_offset_y += (0.5 * watching_crop_height -
                      0.5 * largest_inscribed_square_side_length_of_ball);
  }

  // Check for and correct if the resulting crop window would be outside the
  // full resolution image If we need to correct something, preserve the crop
  // width and correct the offset. NOTE - Camera resolutions are 1 greater than
  // the greatest pixel position
  if ((((camera.camera_hardware_.resolution_x_ - 1) - crop_offset_x) +
       watching_crop_width) >= camera.camera_hardware_.resolution_x_) {
    crop_offset_x =
        (camera.camera_hardware_.video_resolution_x_ - crop_offset_x) - 1;
  }

  if ((((camera.camera_hardware_.resolution_y_ - 1) - crop_offset_y) +
       watching_crop_height) >= camera.camera_hardware_.resolution_y_) {
    crop_offset_y =
        (camera.camera_hardware_.video_resolution_y_ - crop_offset_y) - 1;
  }

  cv::Vec2i watching_crop_size =
      cv::Vec2i((uint)watching_crop_width, (uint)watching_crop_height);
  cv::Vec2i watching_crop_offset =
      cv::Vec2i((uint)crop_offset_x, (uint)crop_offset_y);

  // Check to see if this resolution is the same as we currently have
  if (LibCameraInterface::camera_crop_configuration_ ==
          LibCameraInterface::kCropped &&
    ...
    // Don't reset the crop if we don't need to.  It takes time, especially the
    // kernel-based cropping command lines
    return true;
  }

  // If the camera is flipped, the cropping has to be adjusted accordingly so
  // that the crop offset is flipped vertically
  ...
  if (camera_orientation == CameraHardware::CameraOrientation::kUpsideDown) {
    GS_LOG_TRACE_MSG(trace, "Original watching_crop_offset[1] = " +
                                std::to_string(watching_crop_offset[1]));
    float half_screen_height =
        std::round(camera.camera_hardware_.video_resolution_y_ / 2);
    watching_crop_offset[1] =
        half_screen_height - (watching_crop_offset[1] - half_screen_height);

    // We essentially need to swap the top and bottom of the cropping rectangle
    watching_crop_offset[1] -= watching_crop_size[1];

    GS_LOG_TRACE_MSG(trace, "Flipped watching_crop_offset[1] = " +
                                std::to_string(watching_crop_offset[1]));
  }

  if (!SendCameraCroppingCommand(camera, watching_crop_size,
                                 watching_crop_offset)) {
  ...
  // (and confirm the resolution) The camera would have been stopped after we
  // took the first picture, so need re-start for this call
  cv::Vec2i cropped_resolution;
  uint cropped_frame_rate_fps;
  if (!RetrieveCameraInfo(camera.camera_hardware_.camera_number_,
                          cropped_resolution, cropped_frame_rate_fps, true)) {
    return false;
  }

  if (!ConfigureLibCameraOptions(camera, app, watching_crop_size,
  ...
  // window is of interest in terms of determining ball movement. Unlike the
  // cropping window, Offsets here are from the top-left corner of the cropped
  // window Assume the ball is perfectly round, so the roi is square.  We don't
  // want to watch for movement anywhere but within the ball. NOTE - We have to
  // convert from the center of the ROI to the top-left
  ...
  float size_difference_x =
      largest_inscribed_square_side_length_of_ball - watching_crop_width;
  float size_difference_y =
      largest_inscribed_square_side_length_of_ball - watching_crop_height;
  ...
  if (size_difference_x >= 0.0) {
    ...
    // dealing with club strike data so just have the ROI match the cropping
    // area
    roi_size_x = watching_crop_width;
    roi_offset_x = 0.0;
  } else {
    // The cropping area is larger than a square inscribed in the ball circle,
    // so we want to focus the ROI on just the area of that square.
    roi_size_x = largest_inscribed_square_side_length_of_ball * x_scale;
    // Essentially center the ROI within the image, assuming that the ball is
    // centered in the cropping area (which will likely not be the case if we
    // are widening the cropping area for club strike data)
  ...
  // If the camera is reversed, we need to move the offset in the opposite
  // direction of the original offset
  if (camera_orientation == CameraHardware::CameraOrientation::kUpsideDown) {
    GS_LOG_TRACE_MSG(trace,
                     "Original roi_offset_y = " + std::to_string(roi_offset_y));
    roi_offset_y = -roi_offset_y;
    GS_LOG_TRACE_MSG(trace,
                     "Updated roi_offset_y = " + std::to_string(roi_offset_y));
  }
  ...
  if (camera.camera_hardware_.video_resolution_x_ < 0 ||
    ...
    GS_LOG_TRACE_MSG(error, "camera.camera_hardware_.video_resolution_x_ or "
                            "_y_ have not been set.  Exiting.");
    return false;
  }

  roi_offset_x = std::min(roi_offset_x,
                          (float)camera.camera_hardware_.video_resolution_x_);
  roi_offset_y = std::min(roi_offset_y,
                          (float)camera.camera_hardware_.video_resolution_y_);
...
bool DiscoverCameraLocation(const GsCameraNumber camera_number,
                            int &media_number, int &device_number) {

  // The camera location won't change during the course of a single exceution,
  // so no need to figure this out more than once - re-use the earlier values if
  // we can
  ...
  if (pitrac_root.empty()) {
    GS_LOG_TRACE_MSG(error, "DiscoverCameraLocation - could not get "
                            "PITRAC_ROOT environment variable");
    return false;
  }
  ...
  s += "#!/bin/bash\n";
  s += "rm -f /tmp/discover_media.txt /tmp/discover_device.txt "
       "/tmp/discover_result.txt " +
       kOutputFileName + "\n";
  s += "for ((m = 0; m <= 5; ++m))\n";
  s += "    do\n";
  s += "        rm -f /tmp/discover_result.txt\n";
  s += "        media-ctl -d \"/dev/media$m\" --print-dot | grep imx > "
       "/tmp/discover_media.txt\n";
  s += "        awk -F\"imx296 \" '{print $2}' < /tmp/discover_media.txt | cut "
       "-d- -f1 > /tmp/discover_device.txt\n";
  s += "        echo -n -e \"$m \" > /tmp/discover_result.txt\n";
  s += "        cat /tmp/discover_device.txt >> /tmp/discover_result.txt\n";

  s += "       if  grep imx /tmp/discover_media.txt > /dev/null;  then  cat "
       "/tmp/discover_result.txt >> " +
       kOutputFileName + ";  fi\n";
  s += "            done\n";
  ...
  if (!script_file.is_open()) {
    GS_LOG_TRACE_MSG(error,
                     "DiscoverCameraLocation - failed to open script file " +
                         script_name);
    return false;
  }
  ...
  if (cmdResult != 0) {
    GS_LOG_TRACE_MSG(
        error, "system(DiscoverCameraLocation) failed.  Return value was: " +
  ...
  if (!file.is_open()) {
    GS_LOG_TRACE_MSG(error,
                     "DiscoverCameraLocation - failed to open output file " +
                         kOutputFileName);
    return false;
  }
  ...
  try {
    ...
    if (new_line_position == (int)string::npos) {
      ...
      if (GolfSimOptions::GetCommandLineOptions().run_single_pi_) {
        GS_LOG_TRACE_MSG(
            error, "No expected new line found in camera location output.  "
                   "Missing camera when running in single-pi mode.");
        return false;
      } else {
      ...
      // (the port nearest the LAN port) will correspond to the first line of
      // the returned media-ctl output
    ...
    // Parse out the media and device numbers from what should be the first line
    // of the media-ctl location report
    ...
    if (media_number_str.empty() || device_number_str.empty()) {
      GS_LOG_TRACE_MSG(error,
                       "Failed to parse media and device number strings");
      return false;
    }
...
bool SendCameraCroppingCommand(const GolfSimCamera &camera,
  ...
  std::string mediaCtlCmd = GetCmdLineForMediaCtlCropping(
      camera, cropping_window_size, cropping_window_offset);
...
bool ConfigurePostProcessing(const cv::Vec2i &roi_size,
  ...
  GolfSimConfiguration::SetConstant(
      "gs_config.motion_detect_stage.kDifferenceM", kDifferenceM);
  GolfSimConfiguration::SetConstant(
      "gs_config.motion_detect_stage.kDifferenceC", kDifferenceC);
  GolfSimConfiguration::SetConstant(
      "gs_config.motion_detect_stage.kRegionThreshold", kRegionThreshold);
  GolfSimConfiguration::SetConstant(
      "gs_config.motion_detect_stage.kMaxRegionThreshold", kMaxRegionThreshold);
  GolfSimConfiguration::SetConstant(
      "gs_config.motion_detect_stage.kFramePeriod", kFramePeriod);
  GolfSimConfiguration::SetConstant("gs_config.motion_detect_stage.kHSkip",
                                    kHSkip);
  GolfSimConfiguration::SetConstant("gs_config.motion_detect_stage.kVSkip",
                                    kVSkip);
  ...
  MotionDetectStage::incoming_configuration.use_incoming_configuration =
      true; // Don't use .json file values -- use the following
  ...
  MotionDetectStage::incoming_configuration.max_region_threshold =
      kMaxRegionThreshold;
  MotionDetectStage::incoming_configuration.frame_period = kFramePeriod;
  MotionDetectStage::incoming_configuration.hskip =
      kHSkip; // TBD - don't hard code the skip factor
...
bool ConfigureLibCameraOptions(const GolfSimCamera &camera, RPiCamEncoder &app,
  ...
  options->no_raw =
      true; // See https://forums.raspberrypi.com/viewtopic.php?t=369927 -
  ...
  if (GolfSimClubData::kGatherClubData) {
    ...
    shutter_speed_string =
        std::to_string((int)(GolfSimClubData::kClubImageShutterSpeedMultiplier *
                             (1. / cropped_frame_rate_fps * 1000000.))) +
        "us"; // TBD - should be 1,000,000 for uS setting
  } else {
    camera_gain = LibCameraInterface::kCamera1HighFPSGain;
    shutter_speed_string =
        std::to_string((int)(1. / cropped_frame_rate_fps * 1000000.)) +
        "us"; // TBD - should be 1,000,000 for uS setting
  }

  options->gain = camera_gain;
  options->shutter.set(
      shutter_speed_string); // TBD - should be 1,000,000 for uS setting

  options->saturation = (camera_number == GsCameraNumber::kGsCamera1)
                            ? LibCameraInterface::kCamera1Saturation
                            : LibCameraInterface::kCamera2Saturation;
  ...
  // On the Pi5, there's no hardware H.264 encoding, so let's try to turn it off
  // entirely TBD - See video_options.cpp to consider other options like libav
  options->codec = "yuv420"; // was h.264, but that no longer works on Pi5

  if (GolfSimConfiguration::GetPiModel() ==
      GolfSimConfiguration::PiModel::kRPi5) {
  ...
  // speed, we will use a statically-bound motion_detection module.  See
  // ball_watcher.cpp options->post_process_file =
...
std::string GetCmdLineForMediaCtlCropping(const GolfSimCamera &camera,
  ...
  if (!DiscoverCameraLocation(camera.camera_hardware_.camera_number_,
                              media_number, device_number)) {
  ...
  std::string format =
      (camera.camera_hardware_.camera_is_mono()) ? "Y10_1X10" : "SBGGR10_1X10";

  s += "#!/bin/sh\n";
  s += "if  media-ctl -d \"/dev/media" + std::to_string(media_number) +
       "\" --set-v4l2 \"'imx296 " + std::to_string(device_number) +
       "-001a':0 [fmt:" + format + "/" + std::to_string(croppedHW[0]) + "x" +
       std::to_string(croppedHW[1]) + " crop:(" +
       std::to_string(crop_offset_xY[0]) + "," +
       std::to_string(crop_offset_xY[1]) + ")/" + std::to_string(croppedHW[0]) +
       "x" + std::to_string(croppedHW[1]) +
       "]\" > /dev/null;  then  echo -e \"/dev/media" +
       std::to_string(media_number) + "\" > /dev/null; break;  fi\n";

  return s;
}

bool RetrieveCameraInfo(const GsCameraNumber camera_number,
                        cv::Vec2i &resolution, uint &frameRate,
                        bool restartCamera) {
  ...
  if (restartCamera) {
    ...
    try {
      ...
      if (options->Parse(1, argv)) {
        ...
        // Set the camera number (0 or 1, likely) when we have more than one
        // camera
        options->camera =
            (camera_number == GsCameraNumber::kGsCamera1 ||
             !GolfSimOptions::GetCommandLineOptions().run_single_pi_)
                ? 0
                : 1;
...
cv::Mat
LibCameraInterface::undistort_camera_image(const cv::Mat &img,
                                           const GolfSimCamera &camera) {

  if (!camera.camera_hardware_.use_undistortion_matrix_) {
    GS_LOG_MSG(trace, "undistort_camera_image ignoring camera with no "
                      "undistortion matrix. Returning original image.");
    return img;
  }

  GsCameraNumber camera_number = camera.camera_hardware_.camera_number_;
  CameraHardware::CameraModel camera_model =
      camera.camera_hardware_.camera_model_;
  ...
  cv::Mat cameraDistortionVector_ =
      camera.camera_hardware_.cameraDistortionVector_;
  ...
  if (camera.camera_hardware_.camera_is_mono()) {
    cv::initUndistortRectifyMap(
        cameracalibrationMatrix_, cameraDistortionVector_, cv::Mat(),
        cameracalibrationMatrix_, cv::Size(img.cols, img.rows), CV_8UC1,
        m_undistMap1, m_undistMap2);
  } else {
    cv::initUndistortRectifyMap(
        cameracalibrationMatrix_, cameraDistortionVector_, cv::Mat(),
        cameracalibrationMatrix_, cv::Size(img.cols, img.rows), CV_32FC1,
        m_undistMap1, m_undistMap2);
  }

  cv::remap(img, unDistortedBall1Img, m_undistMap1, m_undistMap2,
            cv::INTER_LINEAR);
...
bool ConfigCameraForFullScreenWatching(const GolfSimCamera &c) {
  ...
  if (width <= 0 || height <= 0) {
    GS_LOG_MSG(error, "ConfigCameraForFullScreenWatching called with camera "
                      "that has no resolution set.");
    return false;
  }

  // Ensure no cropping and full resolution on the camera
  std::string mediaCtlCmd = GetCmdLineForMediaCtlCropping(
      c, cv::Vec2i(width, height), cv::Vec2i(0, 0));
  int cmdResult = system(mediaCtlCmd.c_str());
  ...
  LibCameraInterface::camera_crop_configuration_ =
      LibCameraInterface::kFullScreen;
...
bool SetLibcameraTuningFileEnvVariable(const GolfSimCamera &camera) {
  ...
  if (camera.camera_hardware_.camera_is_mono()) {
    ...
    // regardless of whether the camera is camera 1 or camera 2
...
LibcameraJpegApp *ConfigureForLibcameraStill(const GolfSimCamera &camera) {
  ...
  if (app != nullptr || lci::libcamera_configuration_[hardware_camera_index] ==
  ...
  try {
    ...
    if (GolfSimOptions::GetCommandLineOptions().GetCameraNumber() ==
      ...
      // pictures. BUT, we might be doing a calibration shot, and if so, we'll
      // want to adjust the gain & contrast to a lower value because of the
      ...
      if (GolfSimOptions::GetCommandLineOptions().system_mode_ ==
        ...
        still_shutter_time_uS =
            6 * LibCameraInterface::kCamera2StillShutterTimeuS;
      }
    }

    // Assume camera 1 will be at slot 0 in all cases.  Camera 2 will be at slot
    // 1 only in a single Pi system.
    options->camera = (camera_number == GsCameraNumber::kGsCamera1 ||
                       !GolfSimOptions::GetCommandLineOptions().run_single_pi_)
                          ? 0
                          : 1;

    // Shouldn't need a special gain to take a "normal" picture.   Default will
    // be 1.0 from the command line options.
    options->gain = camera_gain;
    options->saturation = (camera_number == GsCameraNumber::kGsCamera1)
                              ? LibCameraInterface::kCamera1Saturation
                              : LibCameraInterface::kCamera2Saturation;
    ...
    GS_LOG_TRACE_MSG(trace,
                     "Camera denoise option set to: " + options->denoise);
    options->immediate = true; // TBD - Trying this for now.  May have to work
                               // on white balance too
    options->awb = "indoor"; // TBD - Trying this for now.  May have to work on
                             // white balance too
    options->nopreview = true;
    options->viewfinder_width = 0;
    options->viewfinder_height = 0;
    ...
    GS_LOG_MSG(error, "ERROR in ConfigureForLibcameraStill: *** " +
                          std::string(e.what()) + " ***");
    return nullptr;
  }
...
bool DeConfigureForLibcameraStill(const GsCameraNumber camera_number) {
  ...
  if (lci::libcamera_app_[camera_number] == nullptr) {
    GS_LOG_TRACE_MSG(error, "DeConfigureForLibcameraStill called, but "
                            "camera_app was null.  Doing nothing.");
    return false;
  }

  if (lci::libcamera_configuration_[camera_number] !=
      lci::CameraConfiguration::kStillPicture) {
    GS_LOG_TRACE_MSG(
        warning,
        "DeConfigureForLibcameraStill called, but camera_app was in the wrong "
        "configurations (configure was mis-matched). Camera_number was: " +
  ...
  try {
    ...
    GS_LOG_MSG(error, "ERROR in DeConfigureForLibcameraStill: *** " +
                          std::string(e.what()) + " ***");
    return false;
  }

  lci::libcamera_configuration_[camera_number] =
      lci::CameraConfiguration::kNotConfigured;
...
bool TakeLibcameraStill(const GolfSimCamera &camera, cv::Mat &img) {
  ...
  if (!DeConfigureForLibcameraStill(
...
bool TakeRawPicture(const GolfSimCamera &camera, cv::Mat &img) {
  ...
  img =
      golf_sim::LibCameraInterface::undistort_camera_image(initialImg, camera);
...
bool CheckForBallEnhanced(GolfBall &ball, cv::Mat &img) {
  bool use_yolo = (golf_sim::BallImageProc::kBallPlacementDetectionMethod ==
                   "experimental");
  ...
  camera.camera_hardware_.init_camera_parameters(
      camera_number, camera_model, camera_lens_type, camera_orientation);
  ...
  if (use_yolo) {
    if (!golf_sim::BallImageProc::PreloadYOLOModel()) {
      ...
      bool detected = golf_sim::BallImageProc::DetectBallsONNX(
          img, golf_sim::BallImageProc::BallSearchMode::kFindPlacedBall,
          detected_circles);
...
bool CheckForBallLegacy(GolfBall &ball, cv::Mat &img) {
  ...
  camera.camera_hardware_.firstCannedImageFileName =
      std::string("/mnt/VerdantShare/dev/GolfSim/LM/Images/") +
      "FirstWaitingImage";
  ...
  bool expectBall = false;
  bool success = camera.GetCalibratedBall(camera, img, ball, search_area_center,
                                          expectBall);
...
bool WaitForCam2Trigger(cv::Mat &return_image) {
  ...
  GolfSimCamera c;
  c.camera_hardware_.init_camera_parameters(GsCameraNumber::kGsCamera2,
                                            camera_model, camera_lens_type,
                                            camera_orientation);
  ...
  try {
    ...
    // On a two-Pi system, each Pi has just one camera, and that camera will be
    // in slot 0 On a single-pi system the one Pi 5 has both cameras.  And
    // Camera 2 will be in slot 1 because Camera 1 is in slot 0.
    if (GolfSimOptions::GetCommandLineOptions().run_single_pi_) {
      options->camera = 1;
    } else {
      options->camera = 0;
    ...
    GS_LOG_MSG(trace, "Flight camera saturation = " +
                          std::to_string(options->saturation));
    ...
    GS_LOG_TRACE_MSG(trace,
                     "Camera denoise option set to: " + options->denoise);
    options->nopreview = true;
    // TBD - Currently, we are using the viewfinder stream to take the picture.
    // Should be corrected.
    ...
    options->shutter.set(
        "11111us"); // Not actually used for external triggering.  Just needs to
                    // be set to something
    options->info_text = "";

    // We know we are using camera 2
    if (GolfSimCamera::kSystemSlot2CameraOrientation ==
        CameraHardware::CameraOrientation::kUpsideDown) {
      // Tell libcamera to flip the image vertically back to where it should be
      options->transform = libcamera::Transform::VFlip;
      GS_LOG_MSG(trace, "Flipping still picture upside down.");
    } else {
  ...
  return_image =
      golf_sim::LibCameraInterface::undistort_camera_image(raw_image, c);

  if (GolfSimOptions::GetCommandLineOptions().camera_still_mode_) {

    std::string output_fname =
        GolfSimOptions::GetCommandLineOptions().output_filename_;
...
bool PerformCameraSystemStartup() {
  ...
  case SystemMode::kTestSpin: {
    ...
    if (!GolfSimOptions::GetCommandLineOptions().run_single_pi_) {
      std::string trigger_mode_command =
          "sudo "
          "$PITRAC_ROOT/ImageProcessing/CameraTools/"
          "setCameraTriggerInternal.sh";
      ...
      /****
      const CameraHardware::CameraModel  camera_model =
      GolfSimCamera::kSystemSlot1CameraType; if (camera_model ==
      CameraHardware::CameraModel::InnoMakerIMX296GS_Mono) { std::string
      ...
      trigger_mode_command); int command_result =
      system(trigger_mode_command.c_str());
          ...
          GS_LOG_TRACE_MSG(trace, "Running in single-pi mode, so not setting
      camera triggering (internal or external) programmatically.  Instead,
      please see the following discussion on how to setup the
      boot/firmware.config.txt dtoverlays for triggering:
  ...
  case SystemMode::kCamera2TestStandalone: {
    ...
    GolfSimCamera camera;
    camera.camera_hardware_.init_camera_parameters(
        GsCameraNumber::kGsCamera2, GolfSimCamera::kSystemSlot2CameraType,
        GolfSimCamera::kSystemSlot2LensType,
        GolfSimCamera::kSystemSlot1CameraOrientation);
