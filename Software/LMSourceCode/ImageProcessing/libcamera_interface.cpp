...
/*****************************************************************/ /**
                                                                     ...
                                                                     * \brief  Main interface to
                                                                     * both PiTrac cameras (using
...
namespace golf_sim {
    ...
    LibCameraInterface::CropConfiguration LibCameraInterface::camera_crop_configuration_ =
        kCropUnknown;
        ...
        if (motion_detected) {
            ...
            for (auto& it : boost::adaptors::reverse(RecentFrames)) {
                ...
                if (mostRecentFrameMat.empty()) {
                    GS_LOG_TRACE_MSG(warning, "Sequence No. " + std::to_string(it.requestSequence) +
                                                  " was empty.");
                }
    ...
    bool ConfigCameraForCropping(GolfBall ball, GolfSimCamera& camera, RPiCamEncoder& app) {
        ...
        // watching_crop_width & Height defines the size of the cropping window, which will be a
        // sub-region of the camera's full resolution.
        ...
        // One issue here is that the current GS camera will not crop any smaller than 98x88.  So,
        // if the ball is smaller than that, we still have to go with the 98x88 and deal with the
        // smaller ball later within the ROI processing

        // We will later want to ensure the ball is not so small that the inscribed watching area (
        // for high FPS ) is larger than the ball and could pick up unrelated movement outside of
        // the ball. We will deal with this when determining the ROI
        uint largest_inscribed_square_side_length_of_ball =
            (double)(CvUtils::CircleRadius(ball.ball_circle_)) * sqrt(2);

        // Reduce the size of the inscribed square a little bit to ensure the motion detection ROI
        // will be within the ball
        ...
        uint largest_inscribed_square_side_length_of_ball =
            (double)(CvUtils::CircleRadius(ball.ball_circle_)) * sqrt(2);
        GS_LOG_TRACE_MSG(trace, "largest_inscribed_square_side_length_of_ball is: " +
                                    std::to_string(largest_inscribed_square_side_length_of_ball));

        // If we are not gathering club data, then the cropping window is a fixed size.  And if that
        // size is too large, reduce it to the size of the ball.
        if (!GolfSimClubData::kGatherClubData) {
            if (largest_inscribed_square_side_length_of_ball < watching_crop_width) {
                GS_LOG_TRACE_MSG(
                    trace, "Decreasing cropping window width because largest ball square side = " +
                               std::to_string(largest_inscribed_square_side_length_of_ball));
            ...
            if (largest_inscribed_square_side_length_of_ball < watching_crop_height) {
                GS_LOG_TRACE_MSG(
                    trace, "Decreasing cropping window height because largest ball square side = " +
                               std::to_string(largest_inscribed_square_side_length_of_ball));
        ...
        // TBD - After all that, and just for the current GS camera, we'll just set the cropping
        // size to the smalllest possible size (for FPS) and either center the ball within that
        // area, or put the ball in the bottom-right of the viewport if we want to see the club
        // data.
        ...
        GolfSimConfiguration::SetConstant(
            "gs_config.motion_detect_stage.kCroppedImagePixelOffsetUp", kCroppedImagePixelOffsetUp);
        ...
        crop_offset_x =
            crop_offset_adjustment_x + (x_scale * (camera.camera_hardware_.resolution_x_ -
        ...
        crop_offset_y =
            crop_offset_adjustment_y + (y_scale * (camera.camera_hardware_.resolution_y_ -
                                                   (ball_y + watching_crop_height / 2.0)));

        // If we're trying to get club images, then skew the cropping so that the ball ends up near
        // the bottom-right such that the the golf ball "watch" ROI will eventually also be all the
        // way at the bottom right (to give more room so see the club)
        if (GolfSimClubData::kGatherClubData) {
            crop_offset_x +=
                (0.5 * watching_crop_width - 0.5 * largest_inscribed_square_side_length_of_ball);
            crop_offset_y +=
                (0.5 * watching_crop_height - 0.5 * largest_inscribed_square_side_length_of_ball);
        }

        // Check for and correct if the resulting crop window would be outside the full resolution
        // image If we need to correct something, preserve the crop width and correct the offset.
        ...
        if (LibCameraInterface::camera_crop_configuration_ == LibCameraInterface::kCropped &&
            ...
            // Don't reset the crop if we don't need to.  It takes time, especially the kernel-based
            // cropping command lines
            return true;
        }
        ...
        if (camera_orientation == CameraHardware::CameraOrientation::kUpsideDown) {
            GS_LOG_TRACE_MSG(trace, "Original watching_crop_offset = (" +
                                        std::to_string(watching_crop_offset[0]) + ", " +
            ...
            watching_crop_offset[0] =
                half_screen_width - (watching_crop_offset[0] - half_screen_width);
            ...
            GS_LOG_TRACE_MSG(trace, "Flipped watching_crop_offset = (" +
                                        std::to_string(watching_crop_offset[0]) + ", " +
                                        std::to_string(watching_crop_offset[1]) + ")");
        }
        ...
        // Determine what the resulting frame rate is in the resulting camera mode  (and confirm the
        // resolution) The camera would have been stopped after we took the first picture, so need
        // re-start for this call
        cv::Vec2i cropped_resolution;
        uint cropped_frame_rate_fps;
        if (!RetrieveCameraInfo(camera.camera_hardware_.camera_number_, cropped_resolution,
                                cropped_frame_rate_fps, true)) {
            return false;
        }
        ...
        // Unlike the cropping window, Offsets here are from the top-left corner of the cropped
        // window Assume the ball is perfectly round, so the roi is square.  We don't want to watch
        // for movement anywhere but within the ball. NOTE - We have to convert from the center of
        // the ROI to the top-left
        ...
        if (size_difference_x >= 0.0) {
            // The cropped area is already fully inside the ball (assuming we're not dealing with
            // club strike data so just have the ROI match the cropping area
            roi_size_x = watching_crop_width;
            roi_offset_x = 0.0;
        } else {
            // The cropping area is larger than a square inscribed in the ball circle, so we want to
            // focus the ROI on just the area of that square.
            roi_size_x = largest_inscribed_square_side_length_of_ball * x_scale;
            // Essentially center the ROI within the image, assuming that the ball is centered in
            // the cropping area (which will likely not be the case if we are widening the cropping
        ...
        // If the camera is reversed, we need to move the offset in the opposite direction of the
        // original offset
        if (camera_orientation == CameraHardware::CameraOrientation::kUpsideDown) {
    ...
    bool DiscoverCameraLocation(const GsCameraNumber camera_number, int& media_number,
                                int& device_number) {
        ...
        if (pitrac_root.empty()) {
            GS_LOG_TRACE_MSG(
                error, "DiscoverCameraLocation - could not get PITRAC_ROOT environment variable");
            return false;
        }
        ...
        s += "        awk -F\"imx296 \" '{print $2}' < /tmp/discover_media.txt | cut -d- -f1 > "
             "/tmp/discover_device.txt\n";
        ...
        if (cmdResult != 0) {
            GS_LOG_TRACE_MSG(error, "system(DiscoverCameraLocation) failed.  Return value was: " +
        ...
        if (!file.is_open()) {
            GS_LOG_TRACE_MSG(
                error, "DiscoverCameraLocation - failed to open output file " + kOutputFileName);
            return false;
        }
        ...
        try {
            ...
            if (new_line_position == (int)std::string::npos) {
                GS_LOG_TRACE_MSG(error,
                                 "Only one camera detected. Both cameras must be connected.");
                return false;
            } else {
            ...
            // Parse out the media and device numbers from what should be the first line of the
            // media-ctl location report
    ...
    bool SendCameraCroppingCommand(const GolfSimCamera& camera, cv::Vec2i& cropping_window_size,
                                   cv::Vec2i& cropping_window_offset) {
        std::string mediaCtlCmd =
            GetCmdLineForMediaCtlCropping(camera, cropping_window_size, cropping_window_offset);
    ...
    bool ConfigurePostProcessing(const cv::Vec2i& roi_size, const cv::Vec2i& roi_offset) {
        ...
        GolfSimConfiguration::SetConstant("gs_config.motion_detect_stage.kDifferenceM",
                                          kDifferenceM);
        GolfSimConfiguration::SetConstant("gs_config.motion_detect_stage.kDifferenceC",
                                          kDifferenceC);
        ...
        GolfSimConfiguration::SetConstant("gs_config.motion_detect_stage.kMaxRegionThreshold",
                                          kMaxRegionThreshold);
        GolfSimConfiguration::SetConstant("gs_config.motion_detect_stage.kFramePeriod",
                                          kFramePeriod);
    ...
    bool ConfigureLibCameraOptions(const GolfSimCamera& camera, RPiCamEncoder& app,
        ...
        if (GolfSimClubData::kGatherClubData) {
            ...
            shutter_speed_string = std::to_string((int)(1. / cropped_frame_rate_fps * 1000000.)) +
                                   "us";  // TBD - should be 1,000,000 for uS setting
        }

        options->Set().gain = camera_gain;
        options->Set().shutter.set(
            shutter_speed_string);  // TBD - should be 1,000,000 for uS setting

        options->Set().saturation = (camera_number == GsCameraNumber::kGsCamera1)
    ...
    std::string GetCmdLineForMediaCtlCropping(const GolfSimCamera& camera, cv::Vec2i croppedHW,
        ...
        s += "if  media-ctl -d \"/dev/media" + std::to_string(media_number) +
             ...
             std::to_string(croppedHW[1]) + " crop:(" + std::to_string(crop_offset_xY[0]) + "," +
             std::to_string(crop_offset_xY[1]) + ")/" + std::to_string(croppedHW[0]) + "x" +
             std::to_string(croppedHW[1]) + "]\" > /dev/null;  then  echo -e \"/dev/media" +
             std::to_string(media_number) + "\" > /dev/null; break;  fi\n";

        return s;
    }

    bool RetrieveCameraInfo(const GsCameraNumber camera_number, cv::Vec2i& resolution,
                            uint& frameRate, bool restartCamera) {
        ...
        double fps =
            fd_ctrl == cam->controls().end() ? NAN : (1e6 / fd_ctrl->second.min().get<int64_t>());
    ...
    cv::Mat LibCameraInterface::undistort_camera_image(const cv::Mat& img,
                                                       const GolfSimCamera& camera) {
        if (!camera.camera_hardware_.use_undistortion_matrix_) {
            GS_LOG_MSG(trace,
                       "undistort_camera_image ignoring camera with no undistortion matrix. "
        ...
        if (camera.camera_hardware_.camera_is_mono()) {
            cv::initUndistortRectifyMap(cameracalibrationMatrix_, cameraDistortionVector_,
                                        cv::Mat(), cameracalibrationMatrix_,
                                        cv::Size(img.cols, img.rows), CV_8UC1, m_undistMap1,
                                        m_undistMap2);
        } else {
            cv::initUndistortRectifyMap(cameracalibrationMatrix_, cameraDistortionVector_,
                                        cv::Mat(), cameracalibrationMatrix_,
                                        cv::Size(img.cols, img.rows), CV_32FC1, m_undistMap1,
                                        m_undistMap2);
        }
    ...
    bool ConfigCameraForFullScreenWatching(const GolfSimCamera& c) {
        ...
        std::string mediaCtlCmd =
            GetCmdLineForMediaCtlCropping(c, cv::Vec2i(width, height), cv::Vec2i(0, 0));
        int cmdResult = system(mediaCtlCmd.c_str());
    ...
    bool SetLibcameraTuningFileEnvVariable(const GolfSimCamera& camera) {
        ...
        if (camera.camera_hardware_.camera_is_mono()) {
            // If this is a mono camera, than we must use the "mono" tuning file, regardless of
            // whether the camera is camera 1 or camera 2
    ...
    LibcameraJpegApp* ConfigureForLibcameraStill(const GolfSimCamera& camera) {
        ...
        try {
            ...
            if (GolfSimOptions::GetCommandLineOptions().GetCameraNumber() ==
                ...
                // and if so, we'll want to adjust the gain & contrast to a lower value because of
                // the brighter (non-strobed) environment
            ...
            GS_LOG_TRACE_MSG(trace, "Camera denoise option set to: " + options->Set().denoise);
            options->Set().immediate =
                true;  // TBD - Trying this for now.  May have to work on white balance too
            options->Set().awb =
                "indoor";  // TBD - Trying this for now.  May have to work on white balance too
            options->Set().nopreview = true;
            options->Set().viewfinder_width = 0;
            options->Set().viewfinder_height = 0;
            ...
            if (camera_orientation == CameraHardware::CameraOrientation::kUpsideDown) {
                ...
                options->Set().transform =
                    libcamera::Transform::VFlip | libcamera::Transform::HFlip;
                GS_LOG_MSG(trace, "Flipping still picture upside down.");
            } else {
                GS_LOG_MSG(trace, "NOT flipping still picture upside down.");
            }
    ...
    bool CheckForBallEnhanced(GolfBall& ball, cv::Mat& img) {
        ...
        if (use_yolo) {
            ...
            bool detected = golf_sim::BallImageProc::DetectBalls(
                img, golf_sim::BallImageProc::BallSearchMode::kFindPlacedBall, detected_circles,
                false);
    ...
    bool WaitForCam2Trigger(cv::Mat& return_image) {
        ...
        try {
            ...
            GS_LOG_MSG(trace,
                       "Flight camera saturation = " + std::to_string(options->Set().saturation));
            ...
            GS_LOG_TRACE_MSG(trace, "Camera denoise option set to: " + options->Set().denoise);
            options->Set().nopreview = true;
            // TBD - Currently, we are using the viewfinder stream to take the picture.  Should be
            // corrected.
            ...
            options->Set().shutter.set("11111us");  // Not actually used for external triggering.
                                                    // Just needs to be set to something
            options->Set().info_text = "";
