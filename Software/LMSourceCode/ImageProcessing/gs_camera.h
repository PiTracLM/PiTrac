...
namespace golf_sim {
    ...
    class GolfSimCamera {
       public:
        ...
        // FAKE_PHOTO = "./FakePiCameraPhotoOfGolfBall-Clr-White-Flat.png";   // Having problems
        // with this one
        ...
        // Once the ball has been moved (hit), we expect it's radius will be relatively close to the
        // original radius where is started (or close) as a minimum, as well as a little larger or
        // smaller. Max is a little larger to adjust for possible error and stretching out of balls
        // near the edge
        static float kMaxMovedBallRadiusRatio;
        static float kMinMovedBallRadiusRatio;

        // Determines whether and how intermediate processing images (of which there are over a
        // dozen) are logged to the file system.  Doing so is expensive, time-wise.
        ...
        // The remainder of these constants control the way that the (probably overly-complicated)
        // ball-identification processing works.  Best to see how they are used in the rest of the
        // code to understand them. These constants get their initial values from the configuration
        // .json file for the system.
        ...
        // See the HLA and VLA calculations in gs_camera.cpp for more details on the meaning of
        // negative versus positive angles
        static float kHLAOffsetAngleDegrees;
        static float kVLAOffsetAngleDegrees;

        static bool kUseOnlyHighQualityBallImagesForHLA;

        // Refers to the camera_hardware device object associated with this higher-level camera
        // object
        ...
        bool GetCalibratedBall(const GolfSimCamera& camera, const cv::Mat& rgbImg, GolfBall& b,
                               const cv::Vec2i& expectedBallCenter = cv::Vec2i(0, 0),
                               const bool expectBall = true);

        // Currently returns a single ball
        // TBD - Should return a vector of golf ball objects with each ball's current information.
        // A vector could be returned to support, e.g., a multi-strobed picture where the camera
        // shutter is open while the strobe flashes multiple times. Returns true iff the input ball
        // was successfully calibrated camera_positions_from_origin[0] is the position for the
        // calibrated ball, [1] is the position for the camera that took this image
        bool GetCurrentBallLocation(const GolfSimCamera& camera, const cv::Mat& img,
                                    const GolfBall& calibrated_ball, GolfBall& foundBall);

        // In some cases, camera_1 will be the same as for camera_2 (such as comparing two strobed
        // balls from camera 2).
        static bool ComputeBallDeltas(GolfBall& ball1, GolfBall& ball2,
                                      const GolfSimCamera& first_camera,
                                      const GolfSimCamera& second_camera);

        // This will reverse whatever values are necessary to "correct" the processing for
        // left-handed shots
        ...
        // Use the specified time delay and the already-calculated deltas in the ball to determine
        // velocity
        static void CalculateBallVelocity(GolfBall& b, long time_delay_us);

        // Use the specified time delay and the already-calculated deltas in the ball to determine
        // spin rate Also sets the rotation_results into the ball object
        static void CalculateBallSpinRates(GolfBall& b, const cv::Vec3d& rotation_results,
                                           long time_delay_us);
        ...
        bool AnalyzeStrobedBalls(const cv::Mat& strobed_balls_color_image,
                                 ...
                                 GolfBall& face_ball, GolfBall& ball2,
                                 long& time_between_ball_images_ms);

        // Sets up the LoggingTool root cause and prints out an error if there are less than two
        // strobed balls found
        static void ReportBallSearchError(const int number_balls_found);

        static bool ShowAndLogBalls(const std::string& title, const cv::Mat& img,
                                    std::vector<GolfBall>& balls, bool log_image_to_file = true,
        ...
        // The ball closest to the center of the screen (assuming everything else is ok with it)
        // will likely be the ball that will produce the best "face" for spin detection, especially
        // if compared to the view that the first camera has of the initially-positioned ball.
        ...
        // NOTE:  the hsv range may include negative or > 180 Hue numbers due to the hue looping at
        // 180 degrees
        ...
        // The focal distance for each camera should already be known, but this function allows a
        // reverse calculation that takes the meters and imaged ball radius and determines the focal
        // distance from that.
        ...
        // Returned angles (x,y) (degrees):  x is left or right angle to the "origin" (default, at
        // rest) ball.
        //     camera_positions_from_origin assume that the camera is aimed at the origin.  That is,
        //     the origin will
        //          appear in the middle of the camera's view
        //     Input camera positions are in meters.
        //     Positive x angles mean the ball is positioned to the left of the camera, looking out
        //     from the camera
        //         at the ball.
        //     Negative y angles mean the ball is below the camera's axis, looking out at the ball
        //     from the camera
        ...
        static bool ComputeXyzDeltaDistances(const GolfBall& b1, const GolfBall& b2,
                                             cv::Vec3d& position_deltas_ball_perspective,
                                             cv::Vec3d& distance_deltas_camera_perspective);

        // Returns the real-world distances in the X and Y directions from the perspective of the
        // camera, based in part on the input Z-distance from the ball to the camera.  The X,Y
        // origin from which the distances are measured is considered the center of the image.
        // Regardless of the camera angle, the returned distances are as if the camera was facing
        // straight at the plane of the expected ball's line of flight.
        ...
        // Given the current focal length, direct-to-ball distance, and sensed ball radius,
        // determine what a distance in pixels in the X or Y direction is in real-life distance in
        // meters.
        static double convertXDistanceToMeters(const GolfSimCamera& camera, double zDistanceMeters,
                                               double xDistancePixels);
        static double convertYDistanceToMeters(const GolfSimCamera& camera, double zDistanceMeters,
        ...
        bool analyzeShotImages(
            ...
            const std::vector<cv::Vec3d>&
                camera_positions_from_origin,  // Same X,Y,Z coordinate system as for camera(s)
            GolfBall& result_ball, const cv::Vec2i& expectedBallCenter = cv::Vec2i(0, 0));
        ...
        // Because the strobe timing between different images of the ball can differ from ball-pair
        // to pair, this method helps figure what those intervals are.
        bool DetermineStrobeIntervals(std::vector<GolfBall>& input_balls,
                                      int most_centered_ball_index, int second_ball_index,
                                      long& time_between_ball_images_ms,
                                      GsBallsAndTimingVector& return_balls_and_timing);
        ...
        bool GetPulseIntervalsAndRatiosFromIntervalVector(
            ...
            std::vector<float>& pulse_intervals, std::vector<double>& pulse_ratios);
        ...
        double GetPerpendicularDistanceFromLine(double x, double y, double x1, double y1, double x2,
                                                double y2);
        ...
        bool FindBestBallOnLineOfFlight(const std::vector<GolfBall>& balls,
                                        ...
                                        const GolfBall& line_ball1, const GolfBall& line_ball2);

        // Returns a score of the closeness of the vector of distance_ratios within the pulse_ratios
        // at an offset of the distance_ratios from the beginning of the pulse_ratios
        ...
        void RemoveLowScoringBalls(std::vector<GolfBall>& initial_balls,
                                   const int max_balls_to_retain);

        // Erases any balls whose distance (at a right angle) from the line between the best and
        // second-best ball is greater than the max_distance_from_trajectory
        void RemoveOffTrajectoryBalls(std::vector<GolfBall>& initial_balls,
                                      const double max_distance_from_trajectory,
                                      const GolfBall& best_ball, const GolfBall& second_best_ball);
        ...
        void RemoveWrongColorBalls(const cv::Mat& img, std::vector<GolfBall>& initial_balls,
                                   const GolfBall& expected_best_ball,
        ...
        void DetermineSecondBall(std::vector<GolfBall>& return_balls,
                                 const int most_centered_ball_index, int& second_ball_index);

        // The img is used as a frame to ensure that the balls that are chosen are not too close to
        // the edges
        static bool FindClosestTwoBalls(const cv::Mat& img, const GsBallsAndTimingVector& balls,
        ...
        static bool FindBestTwoSpinBalls(const cv::Mat& img, const GsBallsAndTimingVector& balls,
                                         const bool use_edge_backoffs, GolfBall& ball1,
                                         GolfBall& ball2, double& timing_interval_uS);
        ...
        uint RemoveOverlappingBalls(const std::vector<GolfBall>& initial_balls,
                                    ...
                                    std::vector<GolfBall>& return_balls, const GolfBall& best_ball,
                                    const GolfBall& second_best_ball,
                                    const bool preserve_high_quality_balls = true);

        double AdjustDistanceForSlowing(const double initial_right_distance);

        // Analyze the ball exposures in the image and return ball2 with the trajectory, spin, etc.
        // information exposures_image returns an image of the ball exposures that were identified.
        ...
        static bool ProcessSpin(GolfSimCamera& camera, const cv::Mat& strobed_balls_gray_image,
                                const GsBallsAndTimingVector& non_overlapping_balls_and_timing,
                                GolfBall& result_ball, cv::Vec3d& rotationResults);

        static void DrawFilterLines(const std::vector<cv::Vec4i>& lines, cv::Mat& image,
                                    const cv::Scalar& color, const int thickness = 1);

        // Returns the lines used to try to remove the golf club shaft artifacts
        static bool CleanExternalStrobeArtifacts(const cv::Mat& image, cv::Mat& output_image,
        ...
        static constexpr float EXTRA_MAX_HUE_RANGE_RATIO =
            1.1f;  // An extra boost for the HUE ? TBD

        // Min values will be subtracted from the average HSV values, max will be added
        static constexpr int H_MIN_CAL_COLOR_WIDENING_AMOUNT =
            60;  // 12 worked for green ball, but white seems to need more
        ...
        static int GetExpectedBallRadiusPixelsUsingKnownFocalLength(
            const CameraHardware& camera_hardware, const int resolution_x_, const double distance);

        static bool BallIsInBallVector(const GolfBall& search_ball,
