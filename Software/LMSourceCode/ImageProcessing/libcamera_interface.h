...
namespace golf_sim {
    ...
    class LibCameraInterface {
       public:
        ...
        // True (or set to a non-negative number) if we've already figured out the media and device
    ...
    // Sets up the rpicam-app-based post-processing pipeline so that the motion-detection stage
    // knows how to analyze the cropped image
    bool ConfigurePostProcessing(const cv::Vec2i& roi_size, const cv::Vec2i& roi_offset);

    // Sets up a libcamera encoder with options necessary for a high FPS video loop in a cropped
    ...
    std::string GetCmdLineForMediaCtlCropping(const GolfSimCamera& camera_number,
                                              cv::Vec2i croppedHW, cv::Vec2i cropOffsetXY);
    ...
    bool RetrieveCameraInfo(const GsCameraNumber camera_number, cv::Vec2i& resolution,
                            uint& frameRate, bool restartCamera = false);
