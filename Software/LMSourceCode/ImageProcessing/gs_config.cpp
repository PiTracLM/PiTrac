...
// Having to set the constants in this way creates more entanglement than we'd like.  TBD -
// Re-architect
...
namespace golf_sim {
    ...
    GolfSimConfiguration::EnclosureType GolfSimConfiguration::kEnclosureVersion =
        GolfSimConfiguration::EnclosureType::kEnclosureVersion_Unknown;
    ...
    bool GolfSimConfiguration::Initialize(const std::string& configuration_filename) {
        try {
            ...
            GS_LOG_MSG(error, "GolfSimConfiguration::Initialize failed. ERROR: *** " +
                                  std::string(e.what()) + " ***");
            return false;
        }
    ...
    bool GolfSimConfiguration::ReadShotInjectionData(std::vector<GsResults>& shots,
        ...
        try {
            ...
            GS_LOG_MSG(error, "GolfSimConfiguration::ReadShotInjectionData failed. ERROR: *** " +
                                  std::string(e.what()) + " ***");
            return false;
        }
        return true;
    ...
    bool GolfSimConfiguration::ReadValues() {
        ...
        SetConstant("gs_config.cameras.kCamera1HighFPSGain",
                    LibCameraInterface::kCamera1HighFPSGain);
        ...
        SetConstant("gs_config.cameras.kCamera2PuttingGain",
                    LibCameraInterface::kCamera2PuttingGain);
        ...
        // The web server share directory isn't really a value we want to use from the .json
        // configuration file anymore, but for now, let's allow it as a fall-back to the command
        // line
        ...
        if (!GsUISystem::kWebServerShareDirectory.empty() &&
            GsUISystem::kWebServerShareDirectory.back() != '/') {
            GsUISystem::kWebServerShareDirectory += '/';
        }
        ...
        GS_LOG_TRACE_MSG(
            info, "GolfSimConfiguration - PITRAC_SLOT1_CAMERA_TYPE environment variable was: " +
                      slot1_env);
        if (slot1_env.empty()) {
            GS_LOG_TRACE_MSG(info,
                             "GolfSimConfiguration - PITRAC_SLOT1_CAMERA_TYPE environment variable "
                             "was not set.  Assuming default of: " +
            ...
            GolfSimCamera::kSystemSlot1CameraType =
                CameraHardware::string_to_camera_model(slot1_env);
        }

        std::string slot2_env = safe_getenv("PITRAC_SLOT2_CAMERA_TYPE");
        GS_LOG_TRACE_MSG(
            info, "GolfSimConfiguration - PITRAC_SLOT2_CAMERA_TYPE environment variable was: " +
                      slot2_env);
        if (slot2_env.empty()) {
            ...
            GolfSimCamera::kSystemSlot2CameraType =
                CameraHardware::string_to_camera_model(slot2_env);
        }

        std::string slot1_lens_env = safe_getenv("PITRAC_SLOT1_LENS_TYPE");
        GS_LOG_TRACE_MSG(
            info, "GolfSimConfiguration - PITRAC_SLOT1_LENS_TYPE environment variable was: " +
                      slot1_lens_env);
        if (slot1_lens_env.empty()) {
            GS_LOG_TRACE_MSG(info,
                             "GolfSimConfiguration - PITRAC_SLOT1_LENS_TYPE environment variable "
                             "was not set.  Assuming default of: " +
                                 std::to_string(GolfSimCamera::kSystemSlot1LensType));
        } else {
#ifndef __unix__  // Ignore in Windows environment
            // Ensure we don't have any trailing spaces.  Visual Studio seems to add them?
            ...
            GolfSimCamera::kSystemSlot1LensType =
                CameraHardware::string_to_lens_type(slot1_lens_env);
        }

        std::string slot2_lens_env = safe_getenv("PITRAC_SLOT2_LENS_TYPE");
        GS_LOG_TRACE_MSG(
            info, "GolfSimConfiguration - PITRAC_SLOT2_LENS_TYPE environment variable was: " +
                      slot2_lens_env);
        if (slot2_lens_env.empty()) {
            ...
            GolfSimCamera::kSystemSlot2LensType =
                CameraHardware::string_to_lens_type(slot2_lens_env);
        }

        std::string slot1_camera_orientation_env = safe_getenv("PITRAC_SLOT1_CAMERA_ORIENTATION");
        GS_LOG_TRACE_MSG(
            info,
            "GolfSimConfiguration - PITRAC_SLOT1_CAMERA_ORIENTATION environment variable was: " +
                slot1_camera_orientation_env);
        if (slot1_camera_orientation_env.empty()) {
            GS_LOG_TRACE_MSG(info,
                             "GolfSimConfiguration - PITRAC_SLOT1_CAMERA_ORIENTATION environment "
                             "variable was not set.  Assuming default of: " +
                                 std::to_string(GolfSimCamera::kSystemSlot1CameraOrientation));
        } else {
#ifndef __unix__  // Ignore in Windows environment
            // Ensure we don't have any trailing spaces.  Visual Studio seems to add them?
            ...
            GolfSimCamera::kSystemSlot1CameraOrientation =
                CameraHardware::string_to_camera_orientation(slot1_camera_orientation_env);
        }

        std::string slot2_camera_orientation_env = safe_getenv("PITRAC_SLOT2_CAMERA_ORIENTATION");
        GS_LOG_TRACE_MSG(
            info,
            "GolfSimConfiguration - PITRAC_SLOT2_CAMERA_ORIENTATION environment variable was: " +
                slot2_camera_orientation_env);
        if (slot2_camera_orientation_env.empty()) {
            ...
            GolfSimCamera::kSystemSlot2CameraOrientation =
                CameraHardware::string_to_camera_orientation(slot2_camera_orientation_env);
        }
    ...
    void GolfSimConfiguration::SetConstant(const std::string& tag_name, bool& constant_value) {
        ...
        if (config_mgr.HasKey(tag_name)) {
            ...
            if (val != constant_value) {
                GS_LOG_TRACE_MSG(trace, "Override from ConfigurationManager: " + tag_name + " = " +
                                            (val ? "true" : "false"));
                constant_value = val;
                return;
            }
        ...
        try {
            ...
            GS_LOG_MSG(error, "GolfSimConfiguration::SetConstant failed. ERROR: *** " +
                                  std::string(e.what()) + " ***");
    ...
    void GolfSimConfiguration::SetConstant(const std::string& tag_name,
        ...
        if (config_mgr.HasKey(yaml_key)) {
            ...
            if (val != constant_value) {
                GS_LOG_TRACE_MSG(trace,
                                 "Override from ConfigurationManager: " + tag_name + " = " + val);
                constant_value = val;
                return;
            }
        }
    ...
    bool GolfSimConfiguration::RemoveTreeNode(const std::string& tag_name) {
        try {
            ...
            GS_LOG_MSG(error, "GolfSimConfiguration::RemoveTreeNode failed. ERROR: *** " +
                                  std::string(e.what()) + " ***");
            return false;
        }
    ...
    bool GolfSimConfiguration::SetTreeValue(const std::string& tag_name, const cv::Vec2d& vec) {
        ...
        try {
            ...
            GS_LOG_MSG(error, "GolfSimConfiguration::SetTreeValue failed. ERROR: *** " +
                                  std::string(e.what()) + " ***");
            return false;
        }
    ...
    bool GolfSimConfiguration::WriteTreeToFile(const std::string& file_name) {
        GS_LOG_MSG(trace,
                   "GolfSimConfiguration::WriteTreeToFile called for file_name = " + file_name);
