...
bool GsSimInterface::InitializeSims() {
  ...
  if (GsOpenGolfSimInterface::InterfaceIsPresent()) {
    ...
    if (!ogs_sim->Initialize()) {
      GS_LOG_MSG(error,
                 "OpenGolfSim simulator interface could not be initialized.");
      return false;
    }
  }
...
void GsSimInterface::DeInitializeSims() {
  ...
  for (auto interface : interfaces_) {
    if (interface == nullptr) {
      GS_LOG_MSG(error,
                 "GsSimInterface::DeInitializeSims() found a null interface");
      continue;
    }
...
int GsSimInterface::SendSimMessage(const std::string &message) {
  GS_LOG_MSG(warning,
             "GsSimInterface::SendSimMessage - message was:\n" + message);
  return 0;
}
...
bool GsSimInterface::SendResultsToGolfSims(const GsResults &input_results) {
  ...
  if (results.speed_mph_ > 200.0) {
    GS_LOG_MSG(warning, "GsSimInterface::SendResultsToGolfSim got out of "
                        "bounds speed_mph.  Settting to 200.");
...
GsSimInterface *
GsSimInterface::GetSimInterfaceByType(GolfSimulatorType sim_type) {
...
void GsSimInterface::IncrementShotCounter() { shot_counter_++; }
...
bool GsSimInterface::SendResults(const GsResults &results) {
  GS_LOG_TRACE_MSG(trace, "GsSimInterface::SendResults - No Golf Sim connected "
                          "to Launch Monitor.  Results are: " +
                              results.Format());
  return true;
}

std::string
GsSimInterface::GenerateResultsDataToSend(const GsResults &results) {
  return results.Format();
}

bool GsSimInterface::ProcessReceivedData(const std::string received_data) {
  GS_LOG_TRACE_MSG(
      trace, "GsSimInterface::ProcessReceivedData - No Golf Sim connected to "
