...
namespace golf_sim {
        ...
        if (BallStabilizationCheckTimerThread == nullptr) {
            BallStabilizationCheckTimerThread =
                new TimedCallbackThread("BallStabilizationCheckTimerThread",
                                        kBallStabilizationTime * 1000, queueBallStabilizationCheck);
    ...
    void queueCam2ImageReceivedCheck() {
        ...
        if (GolfSimGlobals::golf_sim_running_) {
            GolfSimEventElement checkForCam2ImageReceivedEvent{
                new GolfSimEvent::CheckForCam2ImageReceived{}};
    ...
    void setupCam2ImageReceivedCheckTimer() {
        GS_LOG_TRACE_MSG(trace, "setupCam2ImageReceivedCheckTimer - Setting call back for " +
        ...
        if (ReceivedCam2ImageCheckTimerThread == nullptr) {
            ReceivedCam2ImageCheckTimerThread = new TimedCallbackThread(
                "setupCam2ImageReceivedCheckTimerThread", kMaxCam2ImageReceivedTimeMs * 1000,
    ...
    GolfSimState onEvent(const state::InitializingCamera1System& initializing,
        ...
        GS_LOG_MSG(debug,
                   "GolfSim state transition: Initializing - Received Restart - Next state "
                   "WaitingForSimArmed or WaitingForBall. ");
        ...
        if (GsSimInterface::GetAllSystemsArmed()) {
            GolfSimEventElement beginWaitingForBallPlacedEvent{
                new GolfSimEvent::BeginWaitingForBallPlaced{}};
            GolfSimEventQueue::QueueEvent(beginWaitingForBallPlacedEvent);

            return state::WaitingForBall{
                std::chrono::steady_clock::now(),
                false /* have not sent the waiting-for-ball IPC message yet */};
        }

        GolfSimEventElement beginWaitingForSimulatorArmedEvent{
    ...
    GolfSimState onEvent(
        const state::WaitingForBall& waitingForBallState,
        const GolfSimEvent::BeginWaitingForBallPlaced& beginWaitingForBallPlacedEvent) {
        GS_LOG_MSG(trace,
                   "State: WaitingForBall - Received BeginWaitingForBallPlacedEvent - Now waiting "
                   "for ball to show up.");
        ...
        if (found) {
            ...
            // Schedule the timer for a determined (short) time in the future.  When the timer goes
            // off, an CheckForBallStable event will be injected Create a thread that will queue an
            // event-loop queueBallStabilizationCheck in the future
        ...
        // Create an image that the monitor can show the player in order to see where the LM is
        // looking for the ball, if there is a search area set. TBD - Figure out a clearer way to
        // pass back the search-area information

        if (ball.search_area_radius_ > 0.01) {
            cv::Scalar c1(255, 255, 255);
            cv::circle(img, cv::Point(ball.search_area_center_[0], ball.search_area_center_[1]),
                       ball.search_area_radius_, c1, 2);
        }
        ...
        GolfSimEventElement newBeginWaitingForBallPlacedEvent{
            new GolfSimEvent::BeginWaitingForBallPlaced{}};
    ...
    GolfSimState onEvent(const state::WaitingForBallStabilization& waitingForBallStabilization,
        ...
        GS_LOG_MSG(debug,
                   "GolfSim state transition: WaitingForBallStabilization - Received "
                   "CheckForBallStableEvent ");
        ...
        if (found) {
            ballMoved =
                ball.CheckIfBallMoved(waitingForBallStabilization.cam1_ball_,
                                      10 /* max center move pixels */, 6 /* % radius change */);
        } else {
            GS_LOG_MSG(info,
        ...
        if (!found || ballMoved) {
            GS_LOG_MSG(info,
                       "=============== Ball Moved (or was lost) Before Stabilizing - Will look "
                       "for ball again.");

            // This event will cause the WaitingForBall state to begin waiting for the ball to
            // appear teed up again
            GolfSimEventElement beginWaitingForBallPlaced{
                new GolfSimEvent::BeginWaitingForBallPlaced{}};
            GolfSimEventQueue::QueueEvent(beginWaitingForBallPlaced);

            return state::WaitingForBall{std::chrono::steady_clock::now(),
                                         false /* send the ball-waiting message again*/};
        }
        ...
        bool use_fast_speed =
            (GolfSimClubs::GetCurrentClubType() == GolfSimClubs::GsClubType::kDriver);
        ...
        // LoggingTools::LogImage("", img, std::vector < cv::Point >{ cv::Vec2i(1456 / 2, 1088 / 2)
        // }, true, "log_view_last_ball_found_img.png");
        ...
        if (GolfSimOptions::GetCommandLineOptions().artifact_save_level_ !=
            ...
            if (GolfSimCamera::kLogDiagnosticImagesToUniqueFiles) {
                ...
                LoggingTools::LogImage(kWebServerLastTeedBallImage + "_", img,
                                       std::vector<cv::Point>{}, false, "",
                                       "_Shot_" + std::to_string(GsSimInterface::GetShotCounter()));
            }

            // In any case, save the image with a non-unique name that will be overwritten on the
            // next show, but that the GUI will be able to depend on the name of.
            LoggingTools::LogImageWithCircles("", img, std::vector<GsCircle>{ball.ball_circle_},
                                              true, kWebServerLastTeedBallImage + ".png");
        }
        ...
        return state::WaitingForBallHit{std::chrono::steady_clock::now(),
                                        ...
                                        waitingForBallStabilization.ball_image_, empty_mat};
    }
    ...
    GolfSimState onEvent(
        ...
        GolfSimEventElement nextBeginWaitingForSimulatorArmed{
            new GolfSimEvent::BeginWaitingForSimulatorArmed{}};
    ...
    GolfSimState onEvent(const state::WaitingForSimulatorArmed& waitingForSimulatorArmed,
        ...
        GolfSimEventElement beginWaitingForSimulatorArmed{
            new GolfSimEvent::BeginWaitingForSimulatorArmed{}};
    ...
    GolfSimState onEvent(const state::BallHitNowWaitingForCam2Image& BallHitNowWaitingForCam2Image,
        ...
        GS_LOG_MSG(debug,
                   "GolfSim state transition: BallHitNowWaitingForCam2Image - Received "
        ...
        if (!GolfSimCamera::ProcessReceivedCam2Image(
                BallHitNowWaitingForCam2Image.ball_image_, cam2_mat,
                BallHitNowWaitingForCam2Image.camera2_pre_image_, result_ball, rotation_results,
            ...
            GsUISystem::SendIPCErrorStatusMessage(
                "GolfSim FSM could not ProcessReceivedCam2Image.");

            GS_LOG_MSG(
                info,
                "BALL_HIT_CSV, " + std::to_string(GsSimInterface::GetShotCounter()) +
                    ", (carry - Error), (Total - Error), (Side Dest - Error), (Smash Factor - "
                    "Error), (Club Speed - Error), " +
            ...
            GS_LOG_TRACE_MSG(trace,
                             "Received and processed cam2ImageReceived.  Now sending Results to "
                             "any connected Golf Simulator");
            ...
            auto velocity_time_period_string =
                GS_FORMATLIB_FORMAT("{: <6.2f}", velocity_time_period);
            s = " Time between chosen images for velocity calculation: " +
                velocity_time_period_string + " ms.";
            ...
            GsUISystem::SaveWebserverImage(GsUISystem::kWebServerResultBallExposureCandidates,
                                           exposures_image, exposure_balls);
            GsHttpClient::PostImageReady(GsUISystem::kWebServerResultBallExposureCandidates +
                                         ".png");
    #endif
        }
    ...
    GolfSimState onEvent(const state::BallHitNowWaitingForCam2Image& BallHitNowWaitingForCam2Image,
        ...
        GS_LOG_MSG(debug,
                   "GolfSim state transition: BallHitNowWaitingForCam2Image - Received "
                   "CheckForCam2ImageReceived - Will restart ");
    ...
    /* TBD
        GolfSimState onEvent(const auto&,
            ...
            GS_LOG_TRACE_MSG(trace, "GolfSim state transition: <Any State> - Received Exit -
       Exiting. ");
        ...
        GolfSimState onEvent(const state::Exiting& exiting,
            ...
            GS_LOG_TRACE_MSG(trace, "GolfSim state transition: Exiting - Received <Any event> -
       Still Exiting. "); return state::Exiting{ };
        }
    */
    ...
    class GolfSimStateMachine {
       public:
        void restartSim(const GolfSimState& starting_state) { state_ = starting_state; }

        void processEvent(const PossibleEvent& event) {
            state_ = std::visit(helper::overload{[](const auto& state, const auto& evt) {
        ...
        void reportCurrentState() {
            ...
            std::visit(
                helper::overload{
                    ...
                    [](const state::Exiting& exiting) { GS_LOG_TRACE_MSG(trace, "Exiting."); },
    ...
    bool ProcessControlMessageEvent(GolfSimEvent::ControlMessage& control_message) {
        ...
        GS_LOG_TRACE_MSG(trace, "Processing ControlMessage of type: " +
    ...
    bool RunGolfSimFsm(const GolfSimState& starting_state) {
        ...
        while (GolfSimGlobals::golf_sim_running_) {
            ...
            GS_LOG_TRACE_MSG(trace, "       Event Queue size = " +
            ...
            try {
                ...
                if (GolfSimEventQueue::EventIsShutdownEvent(eventElement.e_)) {
                    GS_LOG_TRACE_MSG(
                        trace, "----------- Shutting Down - Received Exit Event -------------");
                    GolfSimGlobals::golf_sim_running_ = false;
                ...
                GS_LOG_TRACE_MSG(trace,
                                 "Exception! - " + std::string(ex.what()) + ".  Restarting...");
    ...
    bool PerformSystemStartupTasks() {
        ...
        bool skip_camera =
            (mode == SystemMode::kTest || mode == SystemMode::kTestSpin ||
             mode == SystemMode::kTestExternalSimMessage || mode == SystemMode::kTestGSProServer ||
        ...
        if (!skip_camera) {
            ...
            GS_LOG_MSG(info,
                       "Skipping camera initialization for test mode: " + std::to_string(mode));
        }
        ...
        GolfSimConfiguration::SetConstant("gs_config.modes.kStartInPuttingMode",
                                          kStartInPuttingMode);
