...
namespace golf_sim {
    ...
    GsCameraNumber GolfSimOptions::GetCameraNumber() {
        ...
        if (system_mode_ == kCamera1 || system_mode_ == kCamera1TestStandalone ||
            system_mode_ == kTest || system_mode_ == kCamera1Calibrate ||
            system_mode_ == kCamera1AutoCalibrate || system_mode_ == kCamera1BallLocation) {
    ...
    bool GolfSimOptions::Parse(int argc, char* argv[]) {
        ...
        std::map<std::string, int> log_level_table = {
            {"trace", LoggingLevel::kTrace}, {"debug", LoggingLevel::kDebug},
            {"info", LoggingLevel::kInfo},   {"warn", LoggingLevel::kWarn},
            {"error", LoggingLevel::kError}, {"none", LoggingLevel::kNone},
        };
