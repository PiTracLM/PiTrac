...
namespace golf_sim {
    ...
    void GsGSProConnection::Start() {
        ...
        while (true) {
            ...
            GS_LOG_TRACE_MSG(
                trace, "Received the following message from the Launch Monitor: " + buffer_string);

            message_ = GenerateResponseString();

            GS_LOG_TRACE_MSG(
                trace,
                "Sending the following message from the GSPro simulated server: " + message_);
    ...
    GsGSProTestServer::GsGSProTestServer(boost::asio::io_context& io_context, int port_number)
        : io_context_(io_context), acceptor_(io_context, tcp::endpoint(tcp::v4(), port_number)) {
        port_number_ = port_number;
        StartAccept();
    }

    void GsGSProTestServer::StartAccept() {
        GS_LOG_TRACE_MSG(trace, "GsGSProTestServer::StartAccept.  port_number_: " +
