...
ED::ED(Mat _srcImage, GradientOperator _op, int _gradThresh, int _anchorThresh, int _scanInterval,
    ...
    edgeImage = Mat(
        ...
        Scalar(0));  // initialize edge Image//CV_8UC1:8bites Unsign C1:灰度图，Scalar(0):初始化值
...
ED::ED(short* _gradImg, uchar* _dirImg, int _width, int _height, int _gradThresh, int _anchorThresh,
    ...
    if (selectStableAnchors) {
        ...
        // Compute anchors with the user supplied parameters
        ComputeAnchorPoints();  // anchorThresh used as given as argument. No validation applied.
                                // (No stable anchors.)
    }  // end-else
...
void ED::ComputeGradient() {
    ...
    for (int i = 1; i < height - 1; i++) {
        for (int j = 1; j < width - 1; j++) {
            ...
            //
            // For Scharr
            // Then: gx = 3*(com1 + com2) + 10*(E-D) = 3*(H-A) + 3*(C-F) + 10*(E-D) = 3*(C-A) +
            // 10*(E-D) + 3*(H-F)
            //       gy = 3*(com1 - com2) + 10*(G-B) = 3*(H-A) - 3*(C-F) + 10*(G-B) = 3*(F-A) +
            //       10*(G-B) + 3*(H-C)
            //
            // For LSD
            // A B
    ...
    StartOfWhile:
        ...
        if (len - duplicatePixelCount < minPathLen) {
            ...
            if (totalLen > 1) {
                ...
                // Copy these chains in the forward direction. Skip the first pixel of the first
                // chain due to repetition with the last pixel of the previous chain
