...
namespace golf_sim {
    ...
    public:
        ...
        BOOST_LOG_API void consume(boost::log::record_view const& rec, string_type const&
    formatted_message) {
    ...
    ***/
    ...
    void LoggingTools::InitLogging() {
        ...
        switch (GolfSimOptions::GetCommandLineOptions().logging_level_) {
            ...
            default: {
                std::cout << "WARNING - Received unknown logging level.  Setting to Trace"
                          << std::endl;
                boost::log::core::get()->set_filter(boost::log::trivial::severity >=
                                                    boost::log::trivial::trace);
                break;
        ...
        boost::log::formatter logFmtWithLineNumbers =
            boost::log::expressions::format("[%1%] (%2%) [%3%] [%4%] %5%") % fmtTimeStamp %
    ...
    void LoggingTools::InternalLog(boost::log::trivial::severity_level log_level,
    ...
    // Get a window size (x, y) appropriate to the image, limited to something reasonable -- 600
    // pixels
    ...
    void LoggingTools::ShowImage(std::string name, const cv::Mat& img,
        ...
        for (auto& point : pointFeatures) {
            cv::circle(imgToShow, point, 2, cv::Scalar{0, 0, 0},
                       dot_width);  // Nominal width was 24
        }
        ...
        if (logging_tool_wait_for_keypress_) {
            ...
            if (keyPressed == 115)  // "s"
            {
                ...
                InternalLog(trace, "Saved to file: " + std::string(kDefaultSaveFileName) +
    ...
    bool LoggingTools::LogImageWithCircles(const std::string& fileNameTag, const cv::Mat& img,
        ...
        return LogImage(fileNameTag, imgToLog, std::vector<cv::Point>({}), forceFixedFileName,
                        fixedFileName);
    }
    ...
    bool LoggingTools::LogImage(const std::string& fileNameTag, const cv::Mat& img,
                                const std::vector<cv::Point>& pointFeatures,
                                bool forceFixedFileName, const std::string& fixedFileName,
                                const std::string& suffix) {
        if (img.empty()) {
        ...
        // To ensure that the GUI has a fixed file name to use to display for the user, we may later
        // save both a fixed AND (if requested) a dynamic, time-stamped file name.
    ...
    void LoggingTools::ShowRectangleOnImage(std::string name, const cv::Mat& baseImage,
    ...
    void LoggingTools::GetOneImage(std::vector<cv::Mat> images) {
        ...
        try {
            ...
            GS_LOG_TRACE_MSG(warning, "Exception! - failed to imwrite with fname = " +
                                          std::string{kDefaultSaveFileName} + " - " + ex.what());
        }
    }

    void LoggingTools::DrawCircleOutlineAndCenter(cv::Mat& img, GsCircle circle, std::string label,
                                                  int ordinal, bool de_emphasize) {
        ...
        cv::putText(img, label,
                    ...
                    cv::FONT_HERSHEY_SIMPLEX, 1, c3, 2, cv::LINE_AA);
    }

    std::string LoggingTools::SummarizeImage(const cv::Mat& img) {
        std::string s =
            "(sizeX, sizeY) = (" + std::to_string(img.cols) + ", " + std::to_string(img.rows) + ")";
        return s;
    }
