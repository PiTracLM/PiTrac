...
namespace golf_sim {
    ...
    bool GsSimSocketInterface::Initialize() {
        // Derived classes must set the socket connection address and port before calling this
        // function
        ...
        // Derived classes will need to deal with any initial messaging after the socket is
        // established.
    ...
    void GsSimSocketInterface::ReceiveSocketData() {
        ...
        while (GolfSimGlobals::golf_sim_running_) {
            ...
            try {
                // Read_some should be blocking, but if the socket has closed, it will return
                // immediately
                len = socket_->read_some(boost::asio::buffer(buf), error);
            } catch (std::exception& e) {
                GS_LOG_MSG(error,
                           "GsSimSocketInterface::ReceiveSocketData failed to read from socket - "
                           "Error was: " +
                               std::string(e.what()));
            }
    ...
    void GsSimSocketInterface::DeInitialize() {
        ...
        try {
            ...
            GS_LOG_MSG(error, "Failed GsSimSocketInterface::DeInitialize() - Error was: " +
                                  std::string(e.what()));
        }
    ...
    int GsSimSocketInterface::SendSimMessage(const std::string& message) {
        ...
        try {
            ...
            GS_LOG_MSG(error, "Failed TestE6Message - Error was: " + std::string(e.what()) +
    ...
    bool GsSimSocketInterface::SendResults(const GsResults& results) {
        ...
        if (receive_thread_exited_) {
            GS_LOG_MSG(error,
                       "GsSimSocketInterface::SendResults called before the interface was "
                       "intialized - trying to re-initialize.");
    ...
    bool GsSimSocketInterface::ProcessReceivedData(const std::string received_data) {
        GS_LOG_TRACE_MSG(
            trace,
            "GsSimSocketInterface::ProcessReceivedData - No Scoket-based Golf Sim connected to "
            "Launch Monitor, so not doing anything with data.  Data was:\n" +
                received_data);
        return true;
    }
