...
#define c_P15 imgIT(y + 2, x + 2) == 1
    ...
    void cvCanny3(const cv::Mat& srcarr, cv::Mat& dstarr, cv::Mat& dxarr, cv::Mat& dyarr,
                  int aperture_size) {
        // cv::Ptr<cv::Mat> dx, dy;
        ...
        if (srcarr.type() != CV_8UC1 || dstarr.type() != CV_8UC1 || dxarr.type() != CV_16SC1 ||
            dyarr.type() != CV_16SC1)
        ...
        // golf_sim::LoggingTools::DebugShowImage("cvCanny3 - dxarr", dxarr);
        // golf_sim::LoggingTools::DebugShowImage("cvCanny3 - dyarr", dyarr);
        ...
        for (i = 0; i < size.height; ++i) {
            ...
            for (j = 0; j < size.width; ++j) {
                val = float(abs(dxarr.at<short>(i, j)) +
                            abs(dyarr.at<short>(i, j)));  // TBD - should be short!
                _pmag[j] = val;
...
#define TG22 (int)(0.4142135623730950488016887242097 * (1 << CANNY_SHIFT) + 0.5)
        ...
        // golf_sim::LoggingTools::DebugShowImage("cvCanny3 - dstarr", dstarr);
    };

    void Canny3(InputArray& image, OutputArray& _edges, OutputArray& _sobel_x,
        ...
        cvCanny3(c_src, c_dst, c_dx, c_dy, apertureSize + (L2gradient ? CV_CANNY_L2_GRADIENT : 0));
    };
