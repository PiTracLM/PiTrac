...
namespace golf_sim {
    ...
    static std::string BuildResultJson(int result_type, const std::string& message,
                                       ...
                                       float side_deg = 0, int back_spin = 0, int side_spin = 0,
                                       int carry_m = 0,
    ...
    void GsUISystem::SendIPCErrorStatusMessage(const std::string& error_message) {
        ...
        GsHttpClient::PostResult(BuildResultJson(static_cast<int>(GsIPCResultType::kError), msg));
    }
        ...
        switch (message_type) {
            ...
            case GsIPCResultType::kWaitingForBallToAppear:
                if (GolfSimOptions::GetCommandLineOptions().system_mode_ ==
                    ...
                    msg = "Waiting for ball to be teed up at " +
                          std::to_string(GolfSimCamera::kCamera1CalibrationDistanceToBall) +
            ...
            case GsIPCResultType::kCalibrationResults:
                ...
                break;
            default:
                GS_LOG_TRACE_MSG(trace, "SendIPCStatusMessage received unknown GsIPCResultType : " +
    ...
    void GsUISystem::SendIPCHitMessage(const GolfBall& result_ball,
        ...
        GS_LOG_MSG(info, "BALL_HIT_CSV, " + std::to_string(GsSimInterface::GetShotCounter()) +
                             ", (carry - NA), (Total - NA), (Side Dest - NA), (Smash Factor - NA), "
                             "(Club Speed - NA), " +
                             std::to_string(CvUtils::MetersPerSecondToMPH(speed)) + ", " +
                             std::to_string(back_spin) + ", " + std::to_string(side_spin) + ", " +
                             std::to_string(launch) + ", " + std::to_string(side) +
                             ", (Descent Angle-NA), (Apex-NA), (Flight Time-NA), (Type-NA)");

        GsHttpClient::PostResult(BuildResultJson(static_cast<int>(GsIPCResultType::kHit), msg,
                                                 speed, launch, side, back_spin, side_spin, carry,
                                                 images));
    }

    bool GsUISystem::SaveWebserverImage(const std::string& input_file_name, const cv::Mat& img,
                                        bool suppress_diagnostic_saving) {
        GS_LOG_MSG(trace,
                   "GsUISystem::SaveWebserverImage called with file name = " + input_file_name);
        ...
        if (GolfSimCamera::kLogDiagnosticImagesToUniqueFiles && !suppress_diagnostic_saving) {
            LoggingTools::LogImage(file_name + "_", img, std::vector<cv::Point>{}, false, "",
    ...
    bool GsUISystem::SaveWebserverImage(const std::string& file_name, const cv::Mat& img,
