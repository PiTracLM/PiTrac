...
namespace golf_sim {
    ...
    // Using convention: direction right handed golfer is facing = +x, ball flight direction = +y,
    up = +z
    */
    ...
    class GolfBall {
       ...
       public:
        // TBD - In the long term, need to figure out how to manage x, y, radius, and the ball
        // circle so that we are not carrying redundant information.  We couuld probably derive the
        // circle from the others. center of the ball
        long x() const {
            return x_;
        };  // Position on screen.  In pixels in openCV coordinate system
        long y() const { return y_; };  // In pixels in openCV coordinate system

        void set_x(long x) {
            x_ = x;
            ball_circle_[0] = (float)x;
        };
        void set_y(long y) {
            y_ = y;
            ball_circle_[1] = (float)y;
        };
        ...
        // The circle where the ball exists on the relevant image.  Could (in theory?) be different
        // from the more definitive x and y and radius_at_calibration_pixels_!
        ...
        // These next few variables hold information relative to this ball's relationship to another
        // ball.  Typically, the other ball is the same physical ball, but earlier in time.
        cv::Vec3d position_deltas_ball_perspective_;  // Important - the ball movement in the real
                                                      // world between this ball and the prior
        ...
        double measured_radius_pixels_ =
            0.0;  // In pixels.  This is the currently-known radius, which might differ from
        ...
        enum BallColor {
            kCalibrated = 0,  // If set to this calibrated psuedo-color, then the ball's
                              // average_color_ is the actual best description of the color
            kWhite = 1,
            kOrange = 2,
            kYellow = 3,
            kOpticGreen = 4,
        ...
        // If the ball's hsv range is known (at it really better be!), ball_color_ should be set to
        // kCalibrated
        ...
        // The screen-image region in which the ball is expected to be found.  May not be set, and
        // if so will be all 0's
        cv::Rect expected_ROI_;

        double distance_at_calibration_ = -1.0;  // In meters at the time of calibration
        double radius_at_calibration_pixels_ =
            -1.0;  // In pixels.  Should only be set after calibration
        double calibrated_focal_length_ =
            -1.0;  // In mm.  This is set if we can get a precise, known distance to the ball
        ...
        // This next variable may be important to help create a good colorMask that will remove
        // unwanted parts of the image while still preserving the likely ball portion of the iamge
        ...
        bool CheckIfBallMoved(const GolfBall& ball_to_compare, const int max_center_move_pixels,
                              const int max_radius_change_percent);

        double PixelDistanceFromBall(const GolfBall& ball2) const;

        // Balls that have shot_parameter_ values other than kShotParameterAll will only be used for
        // the purposes that the shot_parameters_ specify.
        static void AverageBalls(const std::vector<GolfBall>& ball_vector, GolfBall& averaged_ball,
                                 bool average_all_parameters = true);
