...
namespace golf_sim {
    ...
    void CEllipseDetectorYaed::SetParameters(Size szPreProcessingGaussKernelSize,
                                             double dPreProcessingGaussSigma, float fThPosition,
                                             float fMaxCenterDistance, int iMinEdgeLength,
                                             float fMinOrientedRectSide,
                                             float fDistanceToEllipseContour, float fMinScore,
                                             float fMinReliability, int iNs) {
    ...
    float CEllipseDetectorYaed::GetMedianSlope(vector<Point2f>& med, Point2f& M,
                                               vector<float>& slopes) {
    ...
    void CEllipseDetectorYaed::GetFastCenter(vector<Point>& e1, vector<Point>& e2,
                                             EllipseData& data) {
        data.isValid = true;
    ...
    void CEllipseDetectorYaed::DetectEdges13(cv::Mat& DP, VVP& points_1, VVP& points_3) {
        ...
        for (int i = 0; i < iContoursSize; ++i) {
            ...
            if (iCountBottom > iCountTop) {  // 1
                points_1.push_back(edgeSegment);
            } else if (iCountBottom < iCountTop) {  // 3
    ...
    void CEllipseDetectorYaed::FindEllipses(Point2f& center, VP& edge_i, VP& edge_j, VP& edge_k,
        ...
        // A low reliability indicate that the arcs form an elliptic shape by chance, but do not
        // underlie an actual ellipse. The value is normalized between 0 and 1. The default value is
        // 0.4.
    ...
    // Get the coordinates of the center, given the intersection of the estimated lines. See Fig.
    // [8] in Sect [3.2.3] in the paper.
    ...
    void CEllipseDetectorYaed::Triplets124(VVP& pi, VVP& pj, VVP& pk,
    ...
    void CEllipseDetectorYaed::Triplets231(VVP& pi, VVP& pj, VVP& pk,
                                           unordered_map<uint, EllipseData>& data,
                                           vector<Ellipse>& ellipses) {
        ushort sz_i = ushort(pi.size());
        ushort sz_j = ushort(pj.size());
    ...
    void CEllipseDetectorYaed::Triplets342(VVP& pi, VVP& pj, VVP& pk,
                                           unordered_map<uint, EllipseData>& data,
                                           vector<Ellipse>& ellipses) {
        ushort sz_i = ushort(pi.size());
        ushort sz_j = ushort(pj.size());
    ...
    void CEllipseDetectorYaed::Triplets413(VVP& pi, VVP& pj, VVP& pk,
                                           unordered_map<uint, EllipseData>& data,
                                           vector<Ellipse>& ellipses) {
        ushort sz_i = ushort(pi.size());
        ushort sz_j = ushort(pj.size());
    ...
    void CEllipseDetectorYaed::PreProcessing(cv::Mat& I, cv::Mat& DP, cv::Mat& DN) {
    ...
    void CEllipseDetectorYaed::DetectAfterPreProcessing(vector<Ellipse>& ellipses, cv::Mat& E,
                                                        Mat1f& PHI) {
        // Set the image size
        _szImg = E.size();
        ...
        // Other temporary
        VVP points_1, points_2, points_3,
            points_4;  // vector of points, one for each convexity class
        unordered_map<uint, EllipseData>
            centers;  // hash map for reusing already computed EllipseData
    ...
    void CEllipseDetectorYaed::Detect(cv::Mat& I, vector<Ellipse>& ellipses) {
        golf_sim::LoggingTools::DebugShowImage("CEllipseDetectorYaed::Detect - Input Image I", I);
        ...
        */
        ...
        // Preprocessing
        // From input image I, find edge point with coarse convexity along positive (DP) or negative
        // (DN) diagonal
        ...
        // golf_sim::LoggingTools::DebugShowImage("DP", DP);
        // golf_sim::LoggingTools::DebugShowImage("DN", DN);
        ...
        golf_sim::LoggingTools::DebugShowImage("out", out);
    ...
    void CEllipseDetectorYaed::ClusterEllipses(vector<Ellipse>& ellipses) {
        ...
        for (int i = 1; i < iNumOfEllipses; ++i) {
            ...
            for (int j = 0; j < sz_clusters; ++j) {
                ...
                // Centers
                float Dc =
                    ((e1._xc - e2._xc) * (e1._xc - e2._xc) + (e1._yc - e2._yc) * (e1._yc - e2._yc));
                if (Dc > th_Dc) {
                    // not same cluster
                    continue;
                }
    ...
    void CEllipseDetectorYaed::DrawDetectedEllipses(Mat3b& output, vector<Ellipse>& ellipses,
        ...
        for (int i = 0; i < n; ++i) {
            ...
            ellipse(output, Point(cvRound(e._xc), cvRound(e._yc)),
                    Size(cvRound(e._a), cvRound(e._b)), e._rad * 180.0 / CV_PI, 0.0, 360.0, color,
                    thickness);
        }
    }
