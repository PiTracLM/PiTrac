...
void Output::OutputReady(void* mem, size_t size, int64_t timestamp_us, bool keyframe) {
    ...
    if (!options_->Get().metadata.empty()) {
        ...
        write_metadata(buf_metadata_, options_->Get().metadata_format, metadata,
                       !metadata_started_);
...
Output* Output::Create(VideoOptions const* options) {
    ...
    if (!libav && (strncmp(out_file.c_str(), "udp://", 6) == 0 ||
                   strncmp(out_file.c_str(), "tcp://", 6) == 0))
...
void write_metadata(std::streambuf* buf, std::string fmt, libcamera::ControlList& metadata,
                    bool first_write) {
    std::ostream out(buf);
    ...
    if (fmt == "txt") {
        ...
        for (auto const& [id, val] : metadata) {
            ...
            out << (first_done ? "," : "") << std::endl
                << "    \"" << id_map->at(id)->name() << "\": " << arg_quote << val.toString()
                << arg_quote;
            first_done = true;
        }
        out << std::endl << "}";
