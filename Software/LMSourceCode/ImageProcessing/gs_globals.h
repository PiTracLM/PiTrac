...
namespace golf_sim {
    ...
    using GsCircle =
        cv::Vec3f;  // Certain openCV calls require Vec3fs, even though we'd probably prefer Vec3d.
                    // Probably overkill, but saves the need for a lot of explicit type conversions
    using GsEllipse = cv::RotatedRect;
    // Note that the GsColorTriplet may need to go negative to handle loop-around situations for the
    // hue in an HSV value.  A Vec3b would not work for that, so we use a Scalar
