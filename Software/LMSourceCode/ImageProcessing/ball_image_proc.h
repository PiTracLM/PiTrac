...
namespace golf_sim {
    ...
    class BallImageProc {
       public:
        enum YOLOImageTypeToUse { kUseYOLOWithColorImages = 0, kUseYOLOWithMonochromeImages = 1 };
        ...
        // TBD - Some of these are redundant - put 'em all in ball_image_proc or in gs_camera, but
        // not both
        ...
        // Shows the ball that was identified with a circle and center point on top of original
        // image
        ...
        //      circle
        // If chooseLargestFinalBall is set true, then even a poorer-matching final ball candidate
        // will be chosen over a smaller, better-scored candidate. If report_find_failures is true,
        // then the system will not be as picky when trying to find a ball. and will not log
        // warnings or errors if the ball is not found.  Otherwise, if false (when the system does
        // not know if a ball will be present), the system will require a more perfect ball in order
        // to reduce false positives.
        bool GetBall(const cv::Mat& img, const GolfBall& baseBallWithSearchParams,
                     std::vector<GolfBall>& return_balls, cv::Rect& expectedBallArea,
                     BallSearchMode search_mode, bool chooseLargestFinalBall = false,
        ...
        static bool DetermineBestCircle(const cv::Mat& gray_image, const GolfBall& reference_ball,
                                        bool choose_largest_final_ball, GsCircle& final_circle);

        // Waits for movement behind the ball (i.e., the club) and returns the first image
        // containing the movement Ignores the first <X> seconds for movement.
        static bool WaitForBallMovement(GolfSimCamera& c, cv::Mat& firstMovementImage,
        ...
        static cv::Vec3d GetBallRotation(const cv::Mat& full_gray_image1, const GolfBall& ball1,
                                         const cv::Mat& full_gray_image2, const GolfBall& ball2);
        ...
        static cv::Vec2i CompareRotationImage(const cv::Mat& img1, const cv::Mat& img2,
                                              const int index = 0);

        static cv::Mat MaskAreaOutsideBall(cv::Mat& ball_image, const GolfBall& ball,
                                           float mask_reduction_factor,
                                           const cv::Scalar& maskValue = (255, 255, 255));

        static void GetRotatedImage(const cv::Mat& gray_2D_input_image, const GolfBall& ball,
                                    const cv::Vec3i rotation, cv::Mat& outputGrayImg);

        static bool RemoveSmallestConcentricCircles(std::vector<GsCircle>& circles);

        // Img would be a constant reference, but we need to perform sub-imaging on it, so keep
        // non-const for now reference_ball_circle is the circle around where the best approximation
        ...
        cv::Mat GetColorMaskImage(const cv::Mat& hsvImage, const GolfBall& ball,
        ...
        // Detection Methods
        static bool DetectBalls(const cv::Mat& preprocessed_img, BallSearchMode search_mode,
                                std::vector<GsCircle>& detected_circles, bool report_find_failures);
        ...
        static bool DetectBallsNCNN(const cv::Mat& preprocessed_img, BallSearchMode search_mode,
                                    std::vector<GsCircle>& detected_circles,
       ...
       private:
        ...
        // as well as information about the difference between the ball's average/median/std color
        // versus the expected color. The following constants identify where in each element the
        // information is
        ...
        // This is an early attempt to remove lines from an image, such as those caused when using
        // the system with another strobe-based launch monitor
        ...
        // Updates the input ball1 to reflect the new position of the ball within the isolated image
        // we are returning.
        ...
        // Will set pixels that were over-saturated in the original_image to be the special "ignore"
        // kPixelIgnoreValue value in the filtered_image.
        static void RemoveReflections(const cv::Mat& original_image, cv::Mat& filtered_image,
                                      const cv::Mat& mask);

        // If prior_binary_threshold < 0, then there is no prior threshold and a new one will be
        // determined and returns in the calibrated_binary_threshold variable.
        static cv::Mat ApplyGaborFilterToBall(const cv::Mat& img, const GolfBall& ball,
        ...
        static cv::Mat ComputeGaborAccumulation(const cv::Mat& img_f32, const int kernel_size,
        ...
        static cv::Mat ThresholdGaborAccumulation(const cv::Mat& accumGray, float binary_threshold,
                                                  int& white_percent);

        // Legacy: runs both stages (backward compat for any external callers)
        static cv::Mat ApplyTestGaborFilter(const cv::Mat& img_f32, const int kernel_size,
                                            double sig, double lm, double th, double ps, double gm,
                                            float binary_threshold, int& white_percent);

        static cv::Mat CreateGaborKernel(int ks, double sig, double th, double lm, double gm,
                                         double ps);

        static cv::Mat Project2dImageTo3dBall(const cv::Mat& image_gray, const GolfBall& ball,
                                              const cv::Vec3i& rotation_angles_degrees,
                                              bool force_serial = false);

        static void Unproject3dBallTo2dImage(const cv::Mat& src3D, cv::Mat& destination_image_gray,
                                             const GolfBall& ball);

        // Given a grayscale (0-255) image and a percentage, this returns in brightness_cutoff from
        // 0-255 that represents the value at which brightness_percentage of the pixels in the image
        // are at or below that value
        static void GetImageCharacteristics(const cv::Mat& img, const int brightness_percentage,
