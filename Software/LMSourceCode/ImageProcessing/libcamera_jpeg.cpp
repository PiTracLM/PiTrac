...
void SetExternalTrigger(bool& flag) {
    ...
    // This will take a moment to complete, so the waiting time to deal with it is dealt with
    // elsehwere in the code
    if (!flag && camera_model == gs::CameraHardware::CameraModel::InnoMakerIMX296GS_Mono) {
        ...
        GS_LOG_TRACE_MSG(trace, "ball_flight_camera_event_loop - Camera 2 trigger_mode_command = " +
...
bool cam2_run_event_loop(LibcameraJpegApp& app, cv::Mat& returnImg, bool send_priming_pulses) {
    ...
    long kQuiesceTimeMs =
        (gs::PulseStrobe::kNumberPrimingPulses) * (1000 / gs::PulseStrobe::kPrimingPulseFPS) + 10;
    ...
    GS_LOG_TRACE_MSG(trace, "kQuiesceTimeMs to wait for the priming pulses to arrive = " +
                                std::to_string(kQuiesceTimeMs) + " milliseconds");

    // Set the starting time to now, even though we will override it when the first trigger is
    // received
    ...
    // Check here, once, to see if we are going to expect to produce a pre-image for later
    // subtraction
    golf_sim::GolfSimConfiguration::SetConstant(
    ...
    // We want to make sure we are externally triggered here every time just in case we're using an
    // InnoMaker camera
        ...
        switch (state) {
            case kWaitingForFinalImageTrigger: {
                GS_LOG_TRACE_MSG(trace,
                                 "Received Final Image Trigger - Image will be de-queued after "
                                 "next (flush) trigger.");
            ...
            case kWaitingForFinalImageFlush: {
                ...
                GS_LOG_TRACE_MSG(trace,
                                 "About to create Mat frame in kWaitingForFinalImageFlush.  "
                                 "Info.height, width = " +
                                     std::to_string(info.height) + ", " +
                                     std::to_string(info.width) +
                                     ". Stride = " + std::to_string(info.stride));
                ...
                // THE FOLLOWING CREATES A SEGMENTATION FAULT: returnImg = cv::Mat(info.height,
                // info.width, CV_8UC3, image, info.stride); So, that's why the frame is being
                // cloned.
                GS_LOG_TRACE_MSG(trace, "Returning (Final, Strobed) Viewfinder captured image");
                // golf_sim::LoggingTools::LogImage("", returnImg, std::vector < cv::Point >{},
                // true, "Cam2_Strobed_Image.png");
            ...
            case kWaitingForFirstPrimingTimeEnd: {
                ...
                auto timeLapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                                      timeOfCurrentTrigger - timeOfFirstTrigger)
                                      .count();

                GS_LOG_TRACE_MSG(trace, "		Time since last trigger: " +
                                            std::to_string(timeLapsed) + " ms.");
                ...
                if (timeLapsed < kQuiesceTimeMs) {
                    ...
                    if (!golf_sim::GolfSimCamera::kUsePreImageSubtraction) {
                        if (!golf_sim::GolfSimCamera::kCameraRequiresFlushPulse) {
                            ...
                            GS_LOG_TRACE_MSG(
                                trace, "Priming period complete.  Ready for Final Image Trigger.");
                            state = kWaitingForFinalImageFlush;
                        } else {
                            GS_LOG_TRACE_MSG(trace,
                                             "Priming period complete.  Ready for Final Image "
                                             "Trigger (before flush).");
            ...
            case kWaitingForPreImageTrigger: {
                ...
                GS_LOG_TRACE_MSG(trace,
                                 "Received Pre-Image Trigger - Image will be de-queued after next "
                                 "(flush) trigger.");
            ...
            case kWaitingForPreImageFlush: {
                ...
                CompletedRequestPtr& payload = std::get<CompletedRequestPtr>(msg.payload);
                            ...
                            cv::Mat frame = cv::Mat(info.height, info.width, CV_8UC3, image,
                info.stride);
            ...
            // This state is not curently used.  Instead, the system goes directly from the
            // pre-message flush to waiting for the final image trigger
...
bool still_image_event_loop(LibcameraJpegApp& app, cv::Mat& returnImg) {
    ...
    for (;;) {
        ...
        if (msg.type == RPiCamApp::MsgType::Timeout) {
            GS_LOG_MSG(error, "ERROR: Device timeout detected, attempting a restart.");
            GS_LOG_MSG(error,
                       "		Check to make sure the .yaml file in use by libcamera has a long "
                       "timeout set, for example,  \"camera_timeout_value_ms\": 86400000,  in the "
                       "appropriate file.");
            GS_LOG_MSG(error,
                       "			On a Pi 4, check both "
                       "/usr/local/share/libcamera/pipeline/rpi/vc4/rpi_apps.yaml and "
                       "/usr/share/libcamera/pipeline/rpi/vc4/rpi_apps.yaml");
            GS_LOG_MSG(error,
                       "			On a Pi 5, check both "
                       "/usr/local/share/libcamera/pipeline/rpi/pisp/rpi_apps.yaml and "
        ...
        if (msg.type == RPiCamApp::MsgType::Quit) {
            ...
            GS_LOG_MSG(error,
                       "Unrecognised camera message type in still_image_event_loop, aborting.");
            app.StopCamera();
            return false;
        }
        ...
        if (app.ViewfinderStream()) {
            ...
            if (options->Set().timeout &&
                now - start_time > std::chrono::milliseconds(
                                       options->Set().timeout.get<std::chrono::milliseconds>())) {
        ...
        // In still capture mode, save a jpeg and quit.
            ...
            GS_LOG_TRACE_MSG(trace, "Still image (width, height) = (" + std::to_string(w) + "," +
