...
void MotionDetectStage::Read(boost::property_tree::ptree const& params) {
    ...
    GS_LOG_MSG(trace,
               "    config_.max_region_threshold " + std::to_string(config_.max_region_threshold));
...
void MotionDetectStage::Configure() {
    ...
    if (gs::GolfSimClubData::kGatherClubData) {
        ...
        GS_LOG_MSG(trace, "Circular frame buffer size re-set to: " +
                              std::to_string(final_frame_buffer_size));
    }
    ...
    GS_LOG_MSG(trace, "After decimating, config_.roi_x = " + std::to_string(config_.roi_x) +
                          ", config_.roi_y = " + std::to_string(config_.roi_y));
    GS_LOG_MSG(trace,
               "roi_x_ = " + std::to_string(roi_x_) + ", roi_y_ = " + std::to_string(roi_y_));
    ...
    if (config_.verbose)
        LOG(1, "Sampled (vskip/hskip) Image x,y (smaller): "
                   << info.width << "x" << info.height << " roi: (" << roi_x_ << "," << roi_y_
                   << ") ROI Width/height: " << roi_width_ << "x" << roi_height_
                   << " threshold: " << region_threshold_);
    ...
    if (gs::GolfSimOptions::GetCommandLineOptions().artifact_save_level_ !=
        gs::ArtifactSaveLevel::kNoArtifacts) {
...
bool MotionDetectStage::Process(CompletedRequestPtr& completed_request) {
    ...
    if (config_.frame_period && completed_request->sequence % config_.frame_period) {
        GS_LOG_MSG(trace,
                   "config_.frame_period && completed_request->sequence % config_.frame_period. "
                   "config_.frame_period= " +
                       std::to_string(config_.frame_period));
        return false;
    }
    ...
    if (first_time_) {
        ...
        for (unsigned int y = 0; y < roi_height_; y++) {
            uint8_t* new_value_ptr =
                image + ((roi_y_ + y) * sampledFrameStride) + (roi_x_ * config_.hskip);
    ...
    for (unsigned int y = 0; !local_motion_detected && y < roi_height_; y++) {
        ...
        for (unsigned int x = 0; x < roi_width_; x++, new_value_ptr += config_.hskip) {
            ...
            // TBD - Remove
            // LOG(1, "sampledFrameStride: " << sampledFrameStride << ". old= " << old_value << "
            // new= " << new_value << " .New_ptr: " << (long)new_value_ptr << ".Old_ptr : " <<
            // (long)old_value_ptr << ".");

            *(old_value_ptr++) = new_value;
            if (std::abs(new_value - old_value) >
                (config_.difference_m * (float)old_value + config_.difference_c)) {
                regions++;
            }
        }
        ...
        if (local_motion_detected) {
            // std::cout << "Motion detected - Regions = " << regions << ", y row value was: " << y
            // << std::endl;
            break;
        }
    }
    ...
    if (local_motion_detected && !detectionPaused_) {
        ...
        if (gs::GolfSimOptions::GetCommandLineOptions().system_mode_ !=
            gs::kCamera1TestStandalone) {
        ...
        GS_LOG_MSG(trace, "Will save an additional " + std::to_string(postMotionFramesToCapture_) +
                              " frames.");
    }
    ...
    if (postMotionFramesToCapture_ > 1) {
        GS_LOG_MSG(trace,
                   "Post-motion frames > 0 - setting result local_motion_detected to false.");
        completed_request->post_process_metadata.Set("motion_detect.result", false);
    } else {
        // TBD - Too Much Logging - GS_LOG_MSG(trace, "No post-motion frames after this one -
        // setting result local_motion_detected of: " + std::to_string(local_motion_detected) +
        // ".");
    ...
    // Save the current frame image information if we are still in a mode to be capturing these
    // images either pre- or post-motion detection
    ...
    if ((!detectionPaused_ || postMotionFramesToCapture_ > 0)) {
        ...
        if (config_.showroi) {
            ...
            cv::Point endPoint = cv::Point((roi_x_ + roi_width_) * config_.hskip,
                                           (roi_y_ + roi_height_) * config_.vskip);
        ...
        cv::putText(mat, frame_label, cv::Point(text_x, text_y), cv::FONT_HERSHEY_SIMPLEX, 0.8,
                    c_label, 2, cv::LINE_AA);

        // TBD - Too Much Logging - GS_LOG_MSG(trace, "Pushing Post-Motion Frame No. " +
        // std::to_string(postMotionFramesToCapture_) + " - Seq. No. " +
        // std::to_string(completed_request->sequence));
        ...
        if (need_to_log_first_image_) {
            gs::LoggingTools::LogImage("", mat, std::vector<cv::Point>{}, true,
                                       "gs_log_first_cropped_image_of_teed_ball.png");
