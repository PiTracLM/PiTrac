...
TfStage::TfStage(RPiCamApp* app, int tf_w, int tf_h)
    : PostProcessingStage(app), tf_w_(tf_w), tf_h_(tf_h) {
    if (tf_w_ <= 0 || tf_h_ <= 0)
...
void TfStage::Configure() {
    ...
    if (lores_stream_) {
        ...
        if (config_->verbose)
            LOG(1, "TfStage: Low resolution stream is " << lores_info_.width << "x"
                                                        << lores_info_.height);
    ...
    if (main_stream_) {
        ...
        if (config_->verbose)
            LOG(1, "TfStage: Main stream is " << main_stream_info_.width << "x"
                                              << main_stream_info_.height);
    } else if (config_->verbose)
...
void TfStage::runInference() {
    ...
    if (interpreter_->tensor(input)->type == kTfLiteUInt8) {
        ...
        for (unsigned int i = 0; i < rgb_image.size(); i++)
            tensor[i] =
                (rgb_image[i] - config_->normalisation_offset) / config_->normalisation_scale;
    }
