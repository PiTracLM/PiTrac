...
bool FaceDetectCvStage::Process(CompletedRequestPtr& completed_request) {
...

    {
        ...
        if (completed_request->sequence % refresh_rate_ == 0 &&
            (!future_ptr_ ||
             future_ptr_->wait_for(std::chrono::seconds(0)) == std::future_status::ready)) {
    ...
    if (draw_features_) {
        ...
        uint8_t* ptr = (uint8_t*)buffer.data();
        Mat image(full_stream_info_.height, full_stream_info_.width, CV_8U, ptr,
                  full_stream_info_.stride);
        drawFeatures(image);
    }
...
void FaceDetectCvStage::detectFeatures(CascadeClassifier& cascade) {
    ...
    cascade.detectMultiScale(image_, temp_faces, scaling_factor_, min_neighbors_,
                             CASCADE_SCALE_IMAGE, Size(min_size_, min_size_),
                             Size(max_size_, max_size_));
...
void FaceDetectCvStage::drawFeatures(Mat& img) {
    const static Scalar colors[] = {Scalar(255, 0, 0), Scalar(255, 128, 0), Scalar(255, 255, 0),
    ...
    for (size_t i = 0; i < faces_.size(); i++) {
        ...
        if (0.75 < aspect_ratio && aspect_ratio < 1.3) {
            ...
            rectangle(img, Point(cvRound(r.x), cvRound(r.y)),
                      Point(cvRound(r.x + r.width - 1), cvRound(r.y + r.height - 1)), color, 3, 8,
                      0);
    }
}
