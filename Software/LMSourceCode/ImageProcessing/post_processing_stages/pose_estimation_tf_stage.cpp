...
void PoseEstimationTfStage::interpretOutputs() {
    // This code has been adapted from the "Qengineering/TensorFlow_Lite_Pose_RPi_32-bits"
    // repository and can be found here:
    // "https://github.com/Qengineering/TensorFlow_Lite_Pose_RPi_32-bits/blob/master/Pose_single.cpp"
    ...
    for (int i = 0; i < FEATURE_SIZE; i++) {
        ...
        location_coord.x =
            (x * main_stream_info_.width) / (HEATMAP_DIMS - 1) + offsets[j + FEATURE_SIZE];
