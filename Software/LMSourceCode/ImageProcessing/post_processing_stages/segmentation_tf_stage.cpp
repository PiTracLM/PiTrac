...
class SegmentationTfStage : public TfStage {
   public:
    SegmentationTfStage(RPiCamApp* app)
        : TfStage(app, WIDTH, HEIGHT), segmentation_(WIDTH * HEIGHT) {
...
void SegmentationTfStage::applyResults(CompletedRequestPtr& completed_request) {
    ...
    completed_request->post_process_metadata.Set(
        "segmentation.result", Segmentation(WIDTH, HEIGHT, labels_, segmentation_));
...
void SegmentationTfStage::interpretOutputs() {
    ...
    std::generate(hist.begin(), hist.end(),
                  [i = 0]() mutable { return std::pair<size_t, int>(0, i++); });
