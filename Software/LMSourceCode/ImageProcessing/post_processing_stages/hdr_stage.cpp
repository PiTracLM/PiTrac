...
static void forward_pass(std::vector<double>& fwd_pixels, std::vector<double>& fwd_weight_sums,
                         ...
                         std::vector<double>& threshold, int width, int height, int size,
                         double strength)
...
HdrImage HdrImage::LpFilter(LpFilterConfig const& config) const {
    ...
    std::thread fwd_pass(forward_pass, std::ref(fwd_pixels), std::ref(fwd_weight_sums),
                         std::ref(*this), std::ref(weights), std::ref(threshold), width, height,
                         size, strength);
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++, off++)
            out.P(off) =
                (fwd_pixels[off] * fwd_weight_sums[off] + rev_pixels[off] * rev_weight_sums[off]) /
...
void HdrStage::Read(boost::property_tree::ptree const& params) {
    ...
    // A strength of 1 should give the value in the function; a strength of 0 should give the
    // value 1.
...
bool HdrStage::Process(CompletedRequestPtr& completed_request) {
    ...
    if (!config_.jpeg_filename.empty()) {
        ...
        if (options)
            jpeg_save(buffers, info_, completed_request->metadata, filename, app_->CameraModel(),
                      options);
        else
