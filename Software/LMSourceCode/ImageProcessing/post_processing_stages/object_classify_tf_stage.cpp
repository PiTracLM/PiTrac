...
void ObjectClassifyTfStage::readExtras(boost::property_tree::ptree const& params) {
    ...
    std::string default_path =
        home ? std::string(home) + "/models/labels.txt" : "/usr/share/pitrac/models/labels.txt";
...
void ObjectClassifyTfStage::interpretOutputs() {
    ...
    getTopResults(interpreter_->typed_output_tensor<uint8_t>(0), output_size,
...
void ObjectClassifyTfStage::getTopResults(uint8_t* prediction, int prediction_size,
                                          size_t num_results) {
