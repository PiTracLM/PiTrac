...
static std::map<std::string, ExifIfd> exif_ifd_map = {{"EXIF", EXIF_IFD_EXIF},
                                                      {"IFD0", EXIF_IFD_0},
                                                      {"IFD1", EXIF_IFD_1},
...
void exif_read_tag(ExifData* exif, char const* str) {
    ...
    if (!entry)
        throw std::runtime_error("failed to create entry for EXIF tag " + tag_string +
                                 ", please try without this tag");
    if (entry->format == 0) {
    ...
    if (entry->format == EXIF_FORMAT_UNDEFINED) {
        if (exif_exceptions.count(tag)) {
            ...
            LOG_ERROR("WARNING: libexif format for tag " << tag_name
                                                         << " undefined - treating as ASCII");
            entry->format = EXIF_FORMAT_ASCII;
        }
    }
...
static void YUV420_to_JPEG(const uint8_t* input, StreamInfo const& info,
    ...
    while (cinfo.next_scanline < output_height) {
        ...
        unsigned int offset_uv =
            (((cinfo.next_scanline / 2) * info.height) / output_height) * (info.stride / 2);
        for (unsigned int k = 0; k < output_width3; k += 3) {
...
static void YUV_to_JPEG(const uint8_t* input, StreamInfo const& info, const int output_width,
                        const int output_height, const int quality, const unsigned int restart,
                        uint8_t*& jpeg_buffer, jpeg_mem_len_t& jpeg_len) {
    if (info.pixel_format == libcamera::formats::YUYV)
        YUYV_to_JPEG(input, info, output_width, output_height, quality, restart, jpeg_buffer,
                     jpeg_len);
    else if (info.pixel_format == libcamera::formats::YUV420)
        YUV420_to_JPEG(input, info, output_width, output_height, quality, restart, jpeg_buffer,
                       jpeg_len);
    else
...
static void create_exif_data(std::vector<libcamera::Span<uint8_t>> const& mem,
    ...
    try {
        ...
        if (options->Get().thumb_quality) {
            ...
            LOG(2, "Thumbnail dimensions are " << options->Get().thumb_width << " x "
                                               << options->Get().thumb_height);
            ...
            ExifEntry* thumb_length_entry =
                exif_create_tag(exif, EXIF_IFD_1, EXIF_TAG_JPEG_INTERCHANGE_FORMAT_LENGTH);
            ...
            for (; q > 0; q -= 5) {
                YUV_to_JPEG((uint8_t*)(mem[0].data()), info, options->Get().thumb_width,
                            options->Get().thumb_height, q, 0, thumb_buffer, thumb_len);
...
void jpeg_save(std::vector<libcamera::Span<uint8_t>> const& mem, StreamInfo const& info,
    ...
    try {
        ...
        create_exif_data(mem, info, metadata, cam_model, options, exif_buffer, exif_len,
        ...
        YUV_to_JPEG((uint8_t*)(mem[0].data()), info, info.width, info.height,
                    options->Get().quality, options->Get().restart, jpeg_buffer, jpeg_len);
