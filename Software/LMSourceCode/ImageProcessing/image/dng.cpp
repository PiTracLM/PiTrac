...
struct Matrix {
    ...
    Matrix operator*(Matrix const& other) const {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
                result.m[i * 3 + j] = m[i * 3] * other.m[j] + m[i * 3 + 1] * other.m[3 + j] +
                                      m[i * 3 + 2] * other.m[6 + j];
        return result;
    }
