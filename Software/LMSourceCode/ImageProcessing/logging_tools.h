...
// Both an abstraction layer to the boost logging system, and also a collection of
// helper methods for logging more complex data, such as lists of golf balls?
...
namespace golf_sim {
    ...
    struct LoggingTools {
        ...
        static std::string current_error_root_cause_;

        static std::string kBaseImageLoggingDir;
        ...
        // Shortcut calls for debug and error messages
        ...
        static void Debug(const std::string msg, const std::vector<GolfBall> list);

        static void Trace(const std::string msg, const std::vector<unsigned int> list);
        static void Trace(const std::string msg, const std::vector<int> list);
        static void Trace(const std::string msg, const std::vector<double> list);
        static void Trace(const std::string msg, const std::vector<float> list);
        static void Trace(const std::string msg, const std::vector<GolfBall> list);

        static bool DisplayIntermediateImages();

        static cv::Vec2i GetImageWindowSize(const cv::Mat& img);

        static void ShowImage(std::string name, const cv::Mat& img,
        ...
        static bool LogImage(const std::string& fileNameTag, const cv::Mat& img,
                             const std::vector<cv::Point>& pointFeatures,
                             bool forceFixedFileName = false,
                             const std::string& fixedFileName = std::string(""),
                             const std::string& suffix = std::string(""));

        static bool LogImageWithCircles(const std::string& fileNameTag, const cv::Mat& img,
        ...
        static void ShowRectangleOnImage(std::string name, const cv::Mat& baseImage,
                                         cv::Point startPoint, cv::Point endPoint);
        ...
        static void DrawCircleOutlineAndCenter(cv::Mat& img, GsCircle circle, std::string label,
                                               int ordinal = 0, bool de_emphasize = false);

        // Only shows the image if the logging level is at or below debug
        static void DebugShowImage(std::string name, const cv::Mat& img,
                                   const std::vector<cv::Point>& pointFeatures = {});

        // Prints out the basic information about an image, but not all the data
        static std::string SummarizeImage(const cv::Mat& img);

        // The following format_xxx functions prepare a human-readable output that represents the
        // given object
        ...
        static std::vector<std::string> GetRecentLogMessages();

        static boost::circular_buffer<std::string>& GetRecentLogMessagesQueue();

       protected:
        // Circular queue to hold the last <n> log messages.
        // The messages can be used to provide a trace-back for the user
        // after an error.
        static boost::circular_buffer<std::string> RecentLogMessages;
    };

// Used as a define so that we can get file/line-numbers in our tracing if we want
#define GS_LOG_MSG(LEVEL, MSG) \
    BOOST_LOG_FUNCTION();      \
    BOOST_LOG_TRIVIAL(LEVEL) << MSG

// Trace logging is everywhere, so this macro allows just that macro to be undefined
// in order to increase performance
#define GS_LOG_TRACE_MSG(LEVEL, MSG) \
    BOOST_LOG_FUNCTION();            \
    BOOST_LOG_TRIVIAL(LEVEL) << MSG
