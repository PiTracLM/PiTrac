...
namespace golf_sim {
    ...
    bool GolfSimClubData::Configure() {
        ...
        if (kGatherClubData) {
            ...
            GolfSimConfiguration::SetConstant(
                "gs_config.club_data.kClubImageShutterSpeedMultiplier",
    ...
    bool GolfSimClubData::ProcessClubStrikeData(
    ...
    bool GolfSimClubData::CreateClubStrikeVideo(
        boost::circular_buffer<RecentFrameInfo>& frame_info) {
        GS_LOG_TRACE_MSG(trace, "GolfSimClubData::CreateClubStrikeVideo with " +
                                    std::to_string(frame_info.size()) + " frames.");

        if (!kGatherClubData) {
            GS_LOG_TRACE_MSG(warning,
                             "GolfSimClubData::CreateClubStrikeVideo called, but kGatherClubData "
                             "was not set to true. Cannot generate video.");
            return false;
        }
        ...
        for (auto& it : frame_info) {
            ...
            if (next_frame_mat.empty()) {
                GS_LOG_TRACE_MSG(warning, "GolfSimClubData::CreateClubStrikeVideo -- " +
                                              frame_image_name + " was empty.");
            } else {
                LoggingTools::LogImage("", next_frame_mat, std::vector<cv::Point>{}, true,
                                       frame_image_name);
            }
        ...
        std::string make_movie_command =
            "ffmpeg -framerate 2 -pattern_type glob -i '" + LoggingTools::kBaseImageLoggingDir +
            "Club*.png' -c:v libx264 -pix_fmt yuv420p " + LoggingTools::kBaseImageLoggingDir +
            "ClubStrike_" + unique_time_tag + ".mp4";
