...
namespace golf_sim {
     ...
     */
    bool GsAutomatedTesting::AbsResultsPass(const cv::Vec2d& expected, const cv::Vec2d& result,
                                            const cv::Vec2d& abs_tolerances) {
        bool pass = true;
     ...
     * and the expected values are <= each of the respective tolerances.
     */
    bool GsAutomatedTesting::AbsResultsPass(const cv::Vec3d& expected, const cv::Vec3d& result,
                                            const cv::Vec3d& abs_tolerances) {
        bool pass = true;
    ...
    bool GsAutomatedTesting::AbsResultsPass(const float expected, const float result,
                                            const float abs_tolerances) {
        return !(std::abs(expected - result) > abs_tolerances);
    }

    bool GsAutomatedTesting::AbsResultsPass(const int expected, const int result,
                                            const int abs_tolerances) {
        return !(std::abs(expected - result) > abs_tolerances);
    }
     ...
     */
    void GsAutomatedTesting::ConvertInchesToMeters(const cv::Vec3d& expectedPositionsInches,
    ...
    bool GsAutomatedTesting::ReadExpectedResults(const std::string& expected_results_filename,
                                                 std::vector<FinalResultsTestScenario>& shots) {
        std::ifstream file(expected_results_filename);
        std::string line;

        if (!file.is_open()) {
            GS_LOG_MSG(error,
                       "ReadExpectedResults - could not open file: " + expected_results_filename);
            return false;
        }
        ...
        if (!std::getline(file, line)) {
            GS_LOG_MSG(error,
                       "ReadExpectedResults - file " + expected_results_filename + " was empty.");
            return false;
        }
    ...
    std::vector<std::string> GsAutomatedTesting::get_files_by_wildcard(
        const std::string& dir_path, const std::string& wildcard_pattern) {
    ...
    bool GsAutomatedTesting::TestFinalShotResultData() {
        ...
        GolfSimConfiguration::SetConstant("gs_config.user_interface.kWebServerLastTeedBallImage",
                                          kWebServerLastTeedBallImageFilenamePrefix);
        GolfSimConfiguration::SetConstant("gs_config.user_interface.kWebServerCamera2Image",
                                          kWebServerCamera2ImageFilenamePrefix);

        std::string kAutomatedTestSuiteDirectory;
        std::string kAutomatedTestExpectedResultsCSV;
        GsResults tolerances;

        GolfSimConfiguration::SetConstant("gs_config.testing.kAutomatedTestSuiteDirectory",
                                          kAutomatedTestSuiteDirectory);
        GolfSimConfiguration::SetConstant("gs_config.testing.kAutomatedTestExpectedResultsCSV",
                                          kAutomatedTestExpectedResultsCSV);
        GolfSimConfiguration::SetConstant("gs_config.testing.kAutomatedTestToleranceBallSpeedMPH",
                                          tolerances.speed_mph_);
        GolfSimConfiguration::SetConstant("gs_config.testing.kAutomatedTestToleranceHLA",
                                          tolerances.hla_deg_);
        GolfSimConfiguration::SetConstant("gs_config.testing.kAutomatedTestToleranceVLA",
                                          tolerances.vla_deg_);
        GolfSimConfiguration::SetConstant("gs_config.testing.kAutomatedTestToleranceBackSpin",
                                          tolerances.back_spin_rpm_);
        GolfSimConfiguration::SetConstant("gs_config.testing.kAutomatedTestToleranceSideSpin",
                                          tolerances.side_spin_rpm_);

        // Create absolute file path(s)
        kAutomatedTestExpectedResultsCSV =
            kAutomatedTestSuiteDirectory + kAutomatedTestExpectedResultsCSV;
        ...
        for (FinalResultsTestScenario& r : tests) {
            // Perform a wild-card search to find the right file.  It basically just needs to have a
            // particular prefix and the shot number
            std::string wildcarded_teed_ball_filename =
                kLogImagePrefix + kWebServerLastTeedBallImageFilenamePrefix + "_Shot_" +
                std::to_string(r.shot_number) + "_(.*)png";

            std::vector<std::string> teed_ball_filenames =
                get_files_by_wildcard(kAutomatedTestSuiteDirectory, wildcarded_teed_ball_filename);

            std::string wildcarded_strobed_ball_filename =
                kLogImagePrefix + kWebServerCamera2ImageFilenamePrefix + "_Shot_" +
                std::to_string(r.shot_number) + "_(.*)png";

            std::vector<std::string> wildcarded_strobed_ball_filenames = get_files_by_wildcard(
                kAutomatedTestSuiteDirectory, wildcarded_strobed_ball_filename);
        ...
        // The pulses must be setup so that we can determine, e.g., pulse-ratios for distance and
        // time measurements
        ...
        GolfSimConfiguration::SetConstant("gs_config.testing.kAutomatedTestResultsCSV",
                                          kAutomatedTestResultsCSV);

        std::ofstream testing_results_csv_file(kAutomatedTestSuiteDirectory +
                                               kAutomatedTestResultsCSV);
        GS_LOG_TRACE_MSG(trace, "Writing CSV result data to: " + kAutomatedTestResultsCSV);
        testing_results_csv_file << "Shot ID,,Comparison (PiTrac value minus Uneekor),,,,,,System "
                                    "Data,,,,,,,,,,,,,,,,,,,"
                                 << std::endl;
        testing_results_csv_file
            << "Ball, PiTrac Shot, Speed ? (mph), VLA ? , HLA ? °, Back Spin ? (rpm), Side Spin ? "
               "(rpm), , Uneekor Speed, PiTrac Speed, , Uneekor VLA°, PiTrac VLA°, , Uneekor HLA°, "
               "PiTrac HLA°, , Uneekor Back Spin, PiTrac Back Spin, , Uneekor Side Spin, PiTrac "
               "Side Spin, , Ball ID Picture, Spin Ball 1, Spin Ball 2, Test Result Ball, Notes"
            << std::endl;
        ...
        for (auto& test : tests) {
            ...
            if (test.ignore_shot) {
                GS_LOG_TRACE_MSG(info,
                                 "Ignoring Test No. " + std::to_string(test.test_index) + ".");
            ...
            // The (simulated) camera and ball position will depend on the test images we use,
            // above.
            ...
            if (!GsAutomatedTesting::ReadTestImages(
                    teed_ball_image_filename, strobed_balls_image_filename, teed_ball_ImgGray,
                    strobed_balls_ImgGray, teed_ball_ImgColor, strobed_balls_ImgColor, camera_model,
                    false /* No undistort)*/, true /* do_not_alter_filenames */)) {
                GS_LOG_TRACE_MSG(warning, "Failed to read valid images for Test No. " +
                                              std::to_string(test.test_index));
                numTestsFailed++;
                continue;
            }
            ...
            if (!GolfSimCamera::ProcessReceivedCam2Image(
                    teed_ball_ImgColor, strobed_balls_ImgColor, dummy_pre_image, result_ball,
                    rotation_results, exposures_image, exposure_balls)) {
                GS_LOG_TRACE_MSG(warning, "Failed to ProcessReceivedCam2Image() for Test No. " +
                                              std::to_string(test.test_index));
                numTestsFailed++;
                continue;
            }
            ...
            if (!AbsResultsPass((float)CvUtils::MetersPerSecondToMPH((float)result_ball.velocity_),
                                test.expected_results.speed_mph_, (float)tolerances.speed_mph_)) {
                GS_LOG_TRACE_MSG(info, "Test No. " + std::to_string(test.test_index) +
                                           " - Failed ball shot speed measurement.");
                test_passed = false;
            }

            if (!AbsResultsPass((float)result_ball.angles_ball_perspective_[0],
                                test.expected_results.hla_deg_, (float)tolerances.hla_deg_)) {
                GS_LOG_TRACE_MSG(info, "Test No. " + std::to_string(test.test_index) +
                                           " - Failed ball HLA measurement.");
                test_passed = false;
            }

            if (!AbsResultsPass((float)result_ball.angles_ball_perspective_[1],
                                test.expected_results.vla_deg_, (float)tolerances.vla_deg_)) {
                GS_LOG_TRACE_MSG(info, "Test No. " + std::to_string(test.test_index) +
                                           " - Failed ball VLA measurement.");
                test_passed = false;
            }

            if (!AbsResultsPass((int)result_ball.rotation_speeds_RPM_[2],
                                test.expected_results.back_spin_rpm_, tolerances.back_spin_rpm_)) {
                GS_LOG_TRACE_MSG(info, "Test No. " + std::to_string(test.test_index) +
                                           " - Failed ball back spin measurement.");
                test_passed = false;
            }

            if (!AbsResultsPass((int)result_ball.rotation_speeds_RPM_[0],
                                test.expected_results.side_spin_rpm_, tolerances.side_spin_rpm_)) {
                GS_LOG_TRACE_MSG(info, "Test No. " + std::to_string(test.test_index) +
                                           " - Failed ball side spin measurement.");
                test_passed = false;
            }
            ...
            float speed_mph_delta =
                (float)CvUtils::MetersPerSecondToMPH((float)result_ball.velocity_) -
                test.expected_results.speed_mph_;
            float hla_deg_delta =
                (float)(result_ball.angles_ball_perspective_[0] - test.expected_results.hla_deg_);
            float vla_deg_delta =
                (float)(result_ball.angles_ball_perspective_[1] - test.expected_results.vla_deg_);
            int back_spin_rpm_delta =
                (int)(result_ball.rotation_speeds_RPM_[2] - test.expected_results.back_spin_rpm_);
            int side_spin_rpm_delta =
                (int)(result_ball.rotation_speeds_RPM_[0] - test.expected_results.side_spin_rpm_);
            ...
            testing_results_csv_file << test.expected_results.speed_mph_ << ","
                                     << CvUtils::MetersPerSecondToMPH((float)result_ball.velocity_)
                                     << ", ,";
            testing_results_csv_file << test.expected_results.vla_deg_ << ","
                                     << result_ball.angles_ball_perspective_[1] << ", ,";
            testing_results_csv_file << test.expected_results.hla_deg_ << ","
                                     << result_ball.angles_ball_perspective_[0] << ", ,";
            testing_results_csv_file << test.expected_results.back_spin_rpm_ << ","
                                     << result_ball.rotation_speeds_RPM_[2] << ", ,";
            testing_results_csv_file << test.expected_results.side_spin_rpm_ << ","
                                     << result_ball.rotation_speeds_RPM_[0] << ",";
    ...
    cv::Mat GsAutomatedTesting::UndistortImage(
        const cv::Mat& img, CameraHardware::CameraModel camera_model,
        CameraHardware::LensType lens_type, CameraHardware::CameraOrientation camera_orientation) {
    ...
    bool GsAutomatedTesting::ReadTestImages(const std::string& img_1_base_filename,
                                            const std::string& img_2_base_filename,
                                            cv::Mat& ball1Img, cv::Mat& ball2Img,
                                            cv::Mat& ball1ImgColor, cv::Mat& ball2ImgColor,
                                            CameraHardware::CameraModel camera_model,
                                            bool undistort, bool do_not_alter_filenames) {
        ...
        if (!do_not_alter_filenames) {
            ...
            GolfSimConfiguration::SetConstant("gs_config.testing.kBaseTestImageDir",
                                              separate_base_test_dir);
        ...
        if (undistort) {
            ...
            unDistortedBall1Img = GsAutomatedTesting::UndistortImage(
                ball1Img, camera_model, CameraHardware::LensType::Lens_6mm,
                CameraHardware::CameraOrientation::kUpsideUp);
            unDistortedBall2Img = GsAutomatedTesting::UndistortImage(
                ball2Img, camera_model, CameraHardware::LensType::Lens_6mm,
                CameraHardware::CameraOrientation::kUpsideUp);

            // Show the center point to help aim the camera
            LoggingTools::DebugShowImage(
                "Undistorted " + img1FileName, unDistortedBall1Img,
                std::vector<cv::Point>({cv::Point(ball1Img.cols / 2, ball1Img.rows / 2)}));
            LoggingTools::DebugShowImage(
                "Undistorted " + img2FileName, unDistortedBall2Img,
                std::vector<cv::Point>({cv::Point(ball1Img.cols / 2, ball1Img.rows / 2)}));
        } else {
    ...
    bool GsAutomatedTesting::TestBallPosition() {
        ...
        LocationAndSpinTestScenario tests[]{
            ...
            {1, "test_3280w_BS_camoff04x_8.5y_20z_ball00z_02y_00degx_spin00z_v1.png",
             "test_3280w_camoff04x_8.5y_20z_ball10z_03y_15degx_spin30z_v1.png",
             CameraHardware::CameraModel::PiCam2,
             std::vector<cv::Vec3d>(
                 {cv::Vec3d(0.1016, 0.2159, 0.508), cv::Vec3d(0.1016, 0.2159, 0.508)}),
             cv::Vec2i(1100, 1000), cv::Vec3d(3, 2, 10), cv::Vec2d(45, 7), cv::Vec3d(0, 0, 30)},

            {2, "test_pos_4056w_cam04offx_14y_17z_Ball_0inz_00degx_00iny_00Zsp_00.png",
             "test_pos_4056w_cam04offx_14y_17z_Ball_10inz_10degx_00iny_30Zsp_00.png",
             CameraHardware::CameraModel::PiHQCam6mmWideLens,
             std::vector<cv::Vec3d>(
                 {cv::Vec3d(0.1016, 0.3556, 0.4318), cv::Vec3d(.1016, 0.3556, 0.4318)}),
             cv::Vec2i(1100, 2000), cv::Vec3d(6.5, -2, -1.5), cv::Vec2d(13, 0),
             cv::Vec3d(4, -2, 30)},

            {3, "IRtest02.1-filter.png", "IRtest02.2-filter.png",
             CameraHardware::CameraModel::PiHQCam6mmWideLens,
             std::vector<cv::Vec3d>({cv::Vec3d(.0914, 0.0953, 0), cv::Vec3d(.0914, 0.0953, 0)}),
             cv::Vec2i(1400, 800), cv::Vec3d(6.5, -2, -1.5), cv::Vec2d(13, 0),
             cv::Vec3d(4, -2, 30)},
        };
        ...
        for (auto& t : old_tests) {
            ...
            std::vector<cv::Vec3d> camera_positions_from_origin =
                t.camera_positions_from_origin;  // In meters
            ...
            if (!GsAutomatedTesting::ReadTestImages(
                    teed_ball_image_filename, strobed_balls_image_filename, ball1ImgGray,
                    ball2ImgGray, ball1ImgColor, ball2ImgColor, t.camera_model)) {
                GS_LOG_TRACE_MSG(trace, "Failed to read valid images for Test No. " +
                                            std::to_string(t.test_index));
                numTestsFailed++;
                continue;
            }
            ...
            c.camera_hardware_.firstCannedImageFileName =
                kAutomatedBaseTestDir + teed_ball_image_filename;
            c.camera_hardware_.secondCannedImageFileName =
                kAutomatedBaseTestDir + strobed_balls_image_filename;
            ...
            if (!c.analyzeShotImages(c, ball1ImgColor, ball2ImgColor, timeDelayuS,
                                     camera_positions_from_origin, result_ball,
            ...
            if (!AbsResultsPass(t.expected_xyz_rotation_degrees,
                                result_ball.ball_rotation_angles_camera_ortho_perspective_,
                                kRotationAngleToleranceAbs)) {
                GS_LOG_TRACE_MSG(trace, "Test No. " + std::to_string(t.test_index) +
                                            " - Failed ball rotation measurement.");
                GS_LOG_TRACE_MSG(trace,
                                 "    Expected X,Y,Z rotation angles (in degrees) are: " +
                                     std::to_string((t.expected_xyz_rotation_degrees[0])) + ", " +
                                     std::to_string((t.expected_xyz_rotation_degrees[1])) + ", " +
                                     std::to_string((t.expected_xyz_rotation_degrees[2])));
                test_passed = false;
            }

            cv::Vec3d expectedPositionsMeters;
            ConvertInchesToMeters(t.expected_position_deltas_ball_perspective_,
                                  expectedPositionsMeters);

            if (!AbsResultsPass(expectedPositionsMeters,
                                result_ball.position_deltas_ball_perspective_,
                                kDeltaLocationBallToleranceAbs)) {
                GS_LOG_TRACE_MSG(trace, "Test No. " + std::to_string(t.test_index) +
                                            " - Failed ball delta location measurement.");
                GS_LOG_TRACE_MSG(trace,
                                 "    Expected X,Y,Z deltas (ball perspective in inches) are: " +
                                     std::to_string(CvUtils::MetersToInches(
                                         t.expected_position_deltas_ball_perspective_[0])) +
                                     ", " +
                                     std::to_string(CvUtils::MetersToInches(
                                         t.expected_position_deltas_ball_perspective_[1])) +
                                     ", " +
                                     std::to_string(CvUtils::MetersToInches(
                                         t.expected_position_deltas_ball_perspective_[2])));
                test_passed = false;
            }

            if (!AbsResultsPass(t.expected_xy_ball_angle_degrees,
                                result_ball.angles_ball_perspective_, kLaunchAngleToleranceAbs)) {
                GS_LOG_TRACE_MSG(trace, "Test No. " + std::to_string(t.test_index) +
                                            " - Failed ball launch angle measurement.");
                GS_LOG_TRACE_MSG(
                    trace, "    Expected X,Y launch angles (ball perspective) (in degrees) are: " +
                               std::to_string(t.expected_xy_ball_angle_degrees[0]) + ", " +
