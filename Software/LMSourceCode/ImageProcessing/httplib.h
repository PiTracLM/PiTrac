...
namespace httplib {
    ...
    namespace detail {
        ...
        inline uint64_t get_header_value_u64(const Headers& headers, const std::string& key,
                                             uint64_t def, size_t id, bool& is_invalid_value) {
...
#else
    #ifdef SO_REUSEPORT
        setsockopt(sock, SOL_SOCKET, SO_REUSEPORT, reinterpret_cast<const void*>(&opt),
                   sizeof(opt));
    #else
        ...
        socket_t create_socket(const std::string& host, const std::string& ip, int port,
            ...
            for (auto rp = result; rp; rp = rp->ai_next) {
                ...
                if (tcp_nodelay) {
                    auto opt = 1;
#ifdef _WIN32
                    setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&opt),
                               sizeof(opt));
#else
                    setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const void*>(&opt),
                               sizeof(opt));
#endif
                }
