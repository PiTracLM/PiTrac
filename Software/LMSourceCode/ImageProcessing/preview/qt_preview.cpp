    ...
    virtual void Show(int fd, libcamera::Span<uint8_t> span, StreamInfo const& info) override {
        ...
        if (info.colour_space == libcamera::ColorSpace::Smpte170m) {
            ...
            if (info.colour_space != libcamera::ColorSpace::Sycc)
                LOG(1, "QtPreview: unexpected colour space "
                           << libcamera::ColorSpace::toString(info.colour_space));
        }
        ...
        for (unsigned int y = 0; y < window_height_; y++) {
            ...
            memcpy(U_row, Y_start + ((4 * info.height + row) >> 1) * (info.stride >> 1),
                   info.stride >> 1);
            memcpy(V_row, Y_start + ((5 * info.height + row) >> 1) * (info.stride >> 1),
                   info.stride >> 1);
