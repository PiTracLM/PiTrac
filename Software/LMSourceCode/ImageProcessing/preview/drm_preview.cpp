...
void DrmPreview::findCrtc() {
    ...
    if (!conId_) {
        ...
        for (i = 0; i < res->count_connectors; i++) {
            ...
            LOG(2, "Connector " << con->connector_id << " (crtc " << (crtc ? crtc->crtc_id : 0)
                                << "): type " << con->connector_type << ", "
                                << (crtc ? crtc->width : 0) << "x" << (crtc ? crtc->height : 0)
...
static void get_colour_space_info(std::optional<libcamera::ColorSpace> const& cs,
                                  char const*& encoding, char const*& range) {
...
static int drm_set_property(int fd, int plane_id, char const* name, char const* val) {
    ...
    for (unsigned int i = 0; i < properties->count_props; i++) {
        ...
        for (int j = 0; j < prop->count_enums; j++) {
            ...
            ret = drmModeObjectSetProperty(fd, plane_id, DRM_MODE_OBJECT_PLANE, prop_id,
                                           prop->enums[j].value);
            if (ret < 0)
...
static void setup_colour_space(int fd, int plane_id,
...
void DrmPreview::makeBuffer(int fd, size_t size, StreamInfo const& info, Buffer& buffer) {
    ...
    if (drmModeAddFB2(drmfd_, info.width, info.height, out_fourcc_, bo_handles, pitches, offsets,
                      &buffer.fb_handle, 0))
...
void DrmPreview::Show(int fd, libcamera::Span<uint8_t> span, StreamInfo const& info) {
    ...
    if (drmModeSetPlane(drmfd_, planeId_, crtcId_, buffer.fb_handle, 0, x_off + x_, y_off + y_, w,
                        h, 0, 0, buffer.info.width << 16, buffer.info.height << 16))
