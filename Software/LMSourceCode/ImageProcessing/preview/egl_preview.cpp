...
static void gl_setup(int width, int height, int window_width, int window_height) {
    ...
    static const float verts[] = {-w_factor, -h_factor, w_factor,  -h_factor,
                                  w_factor,  h_factor,  -w_factor, h_factor};
...
void EglPreview::makeWindow(char const* name) {
    ...
    window_ = XCreateWindow(display_, root, x_, y_, width_, height_, 0, visinfo->depth, InputOutput,
    ...
    egl_surface_ = eglCreateWindowSurface(egl_display_, config,
                                          reinterpret_cast<EGLNativeWindowType>(window_), NULL);
    if (!egl_surface_)
...
static void get_colour_space_info(std::optional<libcamera::ColorSpace> const& cs, EGLint& encoding,
                                  EGLint& range) {
...
void EglPreview::makeBuffer(int fd, size_t size, StreamInfo const& info, Buffer& buffer) {
    ...
    EGLImage image =
        eglCreateImageKHR(egl_display_, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, attribs);
    if (!image)
