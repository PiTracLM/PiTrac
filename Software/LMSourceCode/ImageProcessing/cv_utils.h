...
namespace golf_sim {
    ...
    struct CvUtils {
        const static std::byte kOpenCvHueMax{
            180};  // We use a 0-180 value range because 360 degrees is more than can fit in an 8 -
                   // bit uchar
        ...
        static std::vector<GsColorTriplet> GetBallColorRgb(const cv::Mat& img,
                                                           const GsCircle& circle);

        static cv::Mat GetAreaMaskImage(int resolution_x_, int resolution_y_, int expected_ball_X,
                                        int expected_ball_Y, int mask_radius,
                                        cv::Rect& mask_dimensions, bool use_square = false);
        ...
        static void BrightnessAndContrastAutoAlgo1(const cv::Mat& src, cv::Mat& dst,
        ...
        static cv::Mat GetSubImage(const cv::Mat& full_image, cv::Rect& ball_ROI_rect,
