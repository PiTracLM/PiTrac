...
namespace golf_sim {
    ...
    SpinPredictor::SpinPredictor(const Config& config) : config_(config) {}
    ...
    SpinPredictor::Result SpinPredictor::Predict(const cv::Mat& dimple_edges_1,
    ...
    void SpinPredictor::TernaryToTwoChannel(const cv::Mat& gabor_img, float* out_data, int size) {
    ...
    void SpinPredictor::Rotation6DToEuler(const float* r6d, double& x_deg, double& y_deg,
                                          double& z_deg) {
        double R[3][3];
        GramSchmidt(r6d, R);
