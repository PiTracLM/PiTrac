...
namespace golf_sim {
    ...
    bool ball_watcher_event_loop(RPiCamEncoder& app, bool& motion_detected) {
        ...
        if (pthread_setschedparam(pthread_self(), SCHED_FIFO, &sp) != 0) {
            GS_LOG_MSG(warning,
                       "Could not set SCHED_FIFO — trigger latency may have jitter. Grant "
                       "CAP_SYS_NICE to pitrac_lm.");
        }
        ...
        app.SetMetadataReadyCallback(
            std::bind(&Output::MetadataReady, output.get(), std::placeholders::_1));
        ...
        for (unsigned int count = 0;; count++) {
            ...
            if (msg.type == RPiCamEncoder::MsgType::Quit) {
                ...
                GS_LOG_MSG(
                    error,
                    "Unrecognised camera message type in ball_watcher_event_loop, aborting.");
                app.StopCamera();
                app.StopEncoder();
                sp.sched_priority = 0;
                pthread_setschedparam(pthread_self(), SCHED_OTHER, &sp);
            ...
            bool mdResult = false;
            int getStatus =
                completed_request->post_process_metadata.Get("motion_detect.result", mdResult);
            if (getStatus == 0) {
                if (mdResult) {
