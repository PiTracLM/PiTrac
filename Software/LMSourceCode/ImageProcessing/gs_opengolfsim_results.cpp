...
std::string GsOpenGolfSimResults::Format() const
{
    ...
    const double spinSpeedRpm = std::sqrt(backSpinRpm * backSpinRpm + sideSpinRpm * sideSpinRpm);
    ...
    if (!std::isfinite(vLaunchDeg) || !std::isfinite(hLaunchDeg) || !std::isfinite(spinAxisDeg))
        return "";
