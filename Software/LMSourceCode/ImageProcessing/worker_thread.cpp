...
#define MSG_TIMER 3
    ...
    GsThread::GsThread(const std::string& thread_name)
        : m_thread_(nullptr), thread_name_(thread_name) {}
    ...
    TimedCallbackThread::TimedCallbackThread(const std::string& thread_name, long wait_time_ms,
                                             void (*callback_function)(), bool repeat_timer)
        : GsThread(thread_name) {
