...
namespace golf_sim {
    ...
    void GolfBall::InitMembers() {
        if (BallHSVRangeDict.size() == 0) {
            BallColorRange cr{cv::Vec3b(30, 0, 100), cv::Vec3b(170, 100, 255),
                              cv::Vec3b(90, 0, 255)};

            BallHSVRangeDict[BallColor::kWhite] = BallColorRange(
                cv::Vec3b(30, 0, 100), cv::Vec3b(170, 100, 255), cv::Vec3b(90, 0, 255));
            BallHSVRangeDict[BallColor::kOrange] = BallColorRange(
                cv::Vec3b(0, 30, 80), cv::Vec3b(35, 255, 255),
                cv::Vec3b(5, 225, 222));  // Very touchy, much higher Hmax and things fail
            BallHSVRangeDict[BallColor::kYellow] = BallColorRange(
                cv::Vec3b(20, 50, 70), cv::Vec3b(70, 255, 255), cv::Vec3b(12, 123, 210));
            BallHSVRangeDict[BallColor::kOpticGreen] = BallColorRange(
                cv::Vec3b(10, 80, 130), cv::Vec3b(35, 165, 255), cv::Vec3b(20, 124, 208));
            BallHSVRangeDict[BallColor::kUnknown] =
                BallColorRange(cv::Vec3b(0, 0, 40), cv::Vec3b(180, 255, 255), cv::Vec3b(0, 0, 0));
            BallHSVRangeDict[BallColor::kModelDetected] =
                BallColorRange(cv::Vec3b(0, 0, 40), cv::Vec3b(180, 255, 255), cv::Vec3b(0, 0, 0));
        }
    ...
    std::string GolfBall::Format() const {
        /*
            auto f1 = GS_FORMATLIB_FORMAT("[Ball(x,y)=({: >4},{: <4}), r={: <6.1f} | Circle{: <20} |
           avgC:{: <15} stdC:{: <15} | cal={: <6} | DistFromLens={: <4.3f}in. | CalFocLen={: <3.3f}
           | TBD]\n", x_, y_, measured_radius_pixels_, LoggingTools::FormatCircle(ball_circle_),
                LoggingTools::FormatGsColorTriplet(average_color_),
           LoggingTools::FormatGsColorTriplet(std_color_), calibrated,
                CvUtils::MetersToInches(distance_to_z_plane_from_lens_),
                calibrated_focal_length_);

            auto f2 = GS_FORMATLIB_FORMAT("  (all inches)      [DistDeltasBall(x,y,z)=({: <6.1f},{:
        ...
        */
        auto f1 = GS_FORMATLIB_FORMAT(
            "[Ball No. {: >4}  (x,y)=({: >4},{: <4}), r={: <6.2f} | Circle{: <20} | cal={: <6} | "
            "DistFromLens={: <4.3f}m | CalFocLen={: <3.3f} | TBD]\n",
            quality_ranking, x_, y_, measured_radius_pixels_,
            LoggingTools::FormatCircle(ball_circle_), calibrated, distance_to_z_plane_from_lens_,
            calibrated_focal_length_);
        /**** FOR INCHES - TBD
        ...
        ****/
        ...
        auto f2 = GS_FORMATLIB_FORMAT(
            "        [BallAngles(x,y)=({: <6.3f},{: <6.3f})]\n        [DistDeltasBall(x,y,z)=({: "
            "<6.3f},{: <6.3f},{: <6.3f})  (all inches)\n        DistDeltasCam(x,y,z)=({: <6.3f},{: "
            "<6.3f},{: <6.3f})\n        DistCam(x,y,z)=({: <6.3f},{: <6.3f},{: <6.3f})\n        "
            "[AnglesCam(x,y)=({: <6.3f},{: <6.3f})]\n",
            angles_ball_perspective_[0], angles_ball_perspective_[1],
            (position_deltas_ball_perspective_[0]), (position_deltas_ball_perspective_[1]),
            (position_deltas_ball_perspective_[2]), (distance_deltas_camera_perspective_[0]),
            (distance_deltas_camera_perspective_[1]), (distance_deltas_camera_perspective_[2]),
            (distances_ortho_camera_perspective_[0]), (distances_ortho_camera_perspective_[1]),
            (distances_ortho_camera_perspective_[2]), angles_camera_ortho_perspective_[0],
            angles_camera_ortho_perspective_[1]);
    ...
    void GolfBall::PrintBallFlightResults() const {
        ...
        GS_LOG_TRACE_MSG(
            ...
            "Calculated X,Y,Z location deltas (ball perspective in inches) are: " +
                std::to_string(CvUtils::MetersToInches(ball.position_deltas_ball_perspective_[0])) +
                ", " +
                std::to_string(CvUtils::MetersToInches(ball.position_deltas_ball_perspective_[1])) +
                ", " +
                std::to_string(CvUtils::MetersToInches(ball.position_deltas_ball_perspective_[2])));
        ...
        GS_LOG_TRACE_MSG(
            trace, "Calculated ball velocity (m/s)= " + std::to_string(ball.velocity_) + ", or " +
                       std::to_string(ball.velocity_ * 2.237) + " mph.");

        GS_LOG_TRACE_MSG(trace, "Calculated ball spin (x,y,z) in RPM = " +
    ...
    void GolfBall::AverageBalls(const std::vector<GolfBall>& ball_vector, GolfBall& averaged_ball,
        ...
        // All other properties will be averaged for all balls, so they will use the number_balls
        // divisor and will be computed using a running average.
        ...
        for (const GolfBall& b : ball_vector) {
            if (average_all_parameters || b.shot_parameters_.ParameterIsPresent(
                                              GsShotParameters::ShotParameter::kBallVelocity)) {
            ...
            // HLA
            if (average_all_parameters ||
                b.shot_parameters_.ParameterIsPresent(GsShotParameters::ShotParameter::kHLA)) {
            ...
            // VLA
            if (average_all_parameters ||
                b.shot_parameters_.ParameterIsPresent(GsShotParameters::ShotParameter::kVLA)) {
                averaged_ball.angles_ball_perspective_[1] += b.angles_ball_perspective_[1];
                number_balls_averaged_for_VLA++;
            }

            averaged_ball.angles_camera_ortho_perspective_[0] +=
                b.angles_camera_ortho_perspective_[0] / number_balls;
            averaged_ball.angles_camera_ortho_perspective_[1] +=
                b.angles_camera_ortho_perspective_[1] / number_balls;
        ...
        // Now finalize the averages for those properties that we may not have assessed for every
        // ball.
    ...
    double GolfBall::PixelDistanceFromBall(const GolfBall& ball2) const {
        double x_distance =
            std::abs(CvUtils::CircleX(ball_circle_) - CvUtils::CircleX(ball2.ball_circle_));
        double y_distance =
            std::abs(CvUtils::CircleY(ball_circle_) - CvUtils::CircleY(ball2.ball_circle_));
