...
void PostProcessor::LoadModules(const std::string& lib_dir) {
    ...
    for (auto const& p : fs::recursive_directory_iterator(path)) {
        ...
        if (p.path().string().find("hailo-postproc") != std::string::npos) {
            // Special case where we need to load libhailort.so as the Hailo postprocessing stages
            // rely on symbols within it.
    ...
    for (auto const& key_and_value : root) {
        if (key_and_value.first == "rpicam-apps") {
            ...
            if (node.find("lores") != node.not_found()) {
                ...
                LOG(1, "Postprocessing requested lores: " << lores_width << "x" << lores_height
                                                          << " " << lores_format);
            }
        } else {
...
void PostProcessor::Process(CompletedRequestPtr& request) {
    ...
    // Queue the futures to ensure we have correct ordering in the output thread. The promise/future
    // return value tells us when all the streams for this request have been processed and
    // output_ready_callback_ can be called.
