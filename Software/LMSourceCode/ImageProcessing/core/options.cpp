...
static bool set_imx708_subdev_hdr_ctrl(int en, const std::string& cam_id) {
    for (unsigned int i = 0; i < 16; i++) {
        const fs::path test_dir{"/sys/class/video4linux/v4l-subdev" + std::to_string(i) +
                                "/device"};
...
Options::Options()
    : options_(std::make_unique<boost::program_options::options_description>("Valid options are",
                                                                             120, 80)),
      app_(nullptr) {
...
bool OptsInternal::Parse(boost::program_options::variables_map& vm, RPiCamApp* app) {
    ...
    if (version) {
        ...
        std::cout << "rpicam-apps capabilites: "
                  << RPiCamAppsCapabilities(preview_libs, encoder_libs) << std::endl;
    ...
    if (camera < cameras.size()) {
        ...
        if (cam_id.find("imx708") != std::string::npos) {
            ...
            if (hdr == "sensor" || hdr == "auto") {
                // Turn on sensor HDR.  Reset the camera manager if we have switched the value of
                // the control.
    ...
    if (list_cameras) {
        ...
        if (cameras.size() != 0) {
            ...
            for (auto const& cam : cameras) {
                ...
                std::unique_ptr<CameraConfiguration> config = cam->generateConfiguration(
                    {libcamera::StreamRole::Raw, libcamera::StreamRole::Viewfinder});
                if (!config)
                ...
                for (const auto& pix : formats.pixelformats()) {
                    ...
                    for (const auto& size : formats.sizes(pix)) {
                        ...
                        auto crop_ctrl =
                            cam->controls().at(&controls::ScalerCrop).max().get<Rectangle>();
    ...
    if (sscanf(afWindow.c_str(), "%f,%f,%f,%f", &afWindow_x, &afWindow_y, &afWindow_width,
               &afWindow_height) != 4)
    ...
    if (!ccm.empty() && sscanf(ccm.c_str(), "%f,%f,%f,%f,%f,%f,%f,%f,%f", &ccm_values[0],
                               &ccm_values[1], &ccm_values[2], &ccm_values[3], &ccm_values[4],
                               &ccm_values[5], &ccm_values[6], &ccm_values[7], &ccm_values[8]) != 9)
...
void OptsInternal::Print() const {
    ...
    if (roi_width == 0 || roi_height == 0)
        ...
        std::cerr << "    roi: " << roi_x << "," << roi_y << "," << roi_width << "," << roi_height
                  << std::endl;
    if (shutter)
    ...
    std::cerr << "    tuning-file: " << (tuning_file == "-" ? "(libcamera)" : tuning_file)
              << std::endl;
...
bool OptsInternal::ParseVideo() {
    ...
    double mbps =
        ((width + 15) >> 4) * ((height + 15) >> 4) * framerate.value_or(DEFAULT_FRAMERATE);
    if ((codec == "h264" || (codec == "libav" && libav_video_codec == "libx264")) &&
        mbps > 245760.0) {
