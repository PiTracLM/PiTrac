...
RPiCamApp::~RPiCamApp() {
    if (!options_->Get().help)
        LOG(2, "Closing RPiCam application" << "(frames displayed " << preview_frames_displayed_
...
Mode RPiCamApp::selectMode(const Mode& mode) const {
    ...
    for (const auto& sensor_mode : sensor_modes_) {
        ...
        if (mode.framerate)
            score +=
                penalty_FPS * std::abs(mode.framerate - std::min(sensor_mode.fps, mode.framerate));
...
void RPiCamApp::ConfigureViewfinder(unsigned int flags) {
    ...
    if (!options_->Get().no_raw) {
        ...
        configuration_->at(raw_stream_num).pixelFormat =
            mode_to_pixel_format(options_->Get().viewfinder_mode);
...
void RPiCamApp::StartCamera() {
    ...
    if (!controls_.get(controls::ScalerCrop) && !controls_.get(controls::rpi::ScalerCrops)) {
        ...
        if (options_->Get().lores_width != 0 && options_->Get().lores_height != 0 &&
            !options_->Get().lores_par) {
    ...
    if (!controls_.get(controls::FrameDurationLimits)) {
        if (StillStream())
            ...
            int64_t frame_time =
                1000000 / options_->Get().framerate.value_or(DEFAULT_FRAMERATE);  // in us
            controls_.set(controls::FrameDurationLimits,
    ...
    if (!controls_.get(controls::ExposureTime) && options_->Get().shutter) {
        ...
        controls_.set(controls::ExposureTime,
                      options_->Get().shutter.get<std::chrono::microseconds>());
    }
    ...
    if (!controls_.get(controls::ColourGains) && options_->Get().awb_gain_r &&
        options_->Get().awb_gain_b)
        controls_.set(controls::ColourGains,
                      libcamera::Span<const float, 2>(
                          {options_->Get().awb_gain_r, options_->Get().awb_gain_b}));
    if (!controls_.get(controls::ColourCorrectionMatrix) && !options_->Get().ccm.empty()) {
        if (!controls_.get(controls::ColourGains))
            LOG_ERROR(
                "WARNING: cannot set colour correction matrix without explicit AWB gains "
                "(--awbgains)");
        else {
    ...
    if (!controls_.get(controls::HdrMode) &&
        (options_->Get().hdr == "auto" || options_->Get().hdr == "single-exp"))
    ...
    if (options_->Get().flicker_period && !controls_.get(controls::AeFlickerMode) &&
        ...
        controls_.set(controls::AeFlickerPeriod,
                      options_->Get().flicker_period.get<std::chrono::microseconds>());
    }
