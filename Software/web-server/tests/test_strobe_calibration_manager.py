...
class TestStartCalibration:
    ...
    async def test_sets_safe_dac_on_failure(self, _sleep):
        ...
        mgr._open_hardware = Mock()
        mgr._close_hardware = Mock()
        mgr._spi_dac = Mock()  # _park_dac_safe only acts when the SPI handle exists
        ...
        # DAC is inverse -> safe fallback is DAC_MAX (min current)
        assert mgr.SAFE_DAC_VALUE == mgr.DAC_MAX
        assert mgr.SAFE_DAC_VALUE in dac_calls

    @pytest.mark.asyncio
    @patch("strobe_calibration_manager.time.sleep")
    async def test_parks_dac_safe_on_cancel(self, _sleep):
        from strobe_calibration_manager import StrobeCalibrationManager
        ...
        def _cancel_midsweep(*_a, **_k):
            mgr._cancel_requested = True
            return (False, -1, -1)
        mgr._calibrate = Mock(side_effect=_cancel_midsweep)
        dac_calls = []
        mgr._set_dac = lambda v: dac_calls.append(v)
    ...
    @pytest.mark.asyncio
    @patch("strobe_calibration_manager.time.sleep")
    async def test_parks_dac_safe_on_exception(self, _sleep):
        from strobe_calibration_manager import StrobeCalibrationManager
