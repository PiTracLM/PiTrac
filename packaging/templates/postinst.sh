...
case "$1" in
    configure)
        ...
        if [ -f /usr/lib/pitrac/boot-config.sh ]; then
            ...
            . /usr/lib/pitrac/boot-config.sh

            CONFIG_FILE=$(get_config_txt_path)

            if [ -f "$CONFIG_FILE" ]; then
                # Create backup before modifications
                backup_config_txt "$CONFIG_FILE" || true

                safe_config_set "$CONFIG_FILE" "camera_auto_detect" "1"
                safe_config_set "$CONFIG_FILE" "force_turbo" "1"

                if [ "$PI_MODEL" = "pi5" ]; then
                    safe_config_set "$CONFIG_FILE" "arm_boost" "1"
                else
                    safe_config_set "$CONFIG_FILE" "gpu_mem" "256"
                fi

                set_dtparam "$CONFIG_FILE" "i2c_arm" "on" "all"
                set_dtparam "$CONFIG_FILE" "spi" "on" "all"

                if ! validate_config_file "$CONFIG_FILE" 2>/dev/null; then
                    echo "Warning: config.txt validation found issues, but continuing"
                fi

                echo "Boot configuration complete"
            fi
        else
            echo "Warning: boot-config.sh not found, skipping boot configuration"
            echo "This should not happen in a proper installation"
        fi
